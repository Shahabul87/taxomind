...
export const VALIDATED_DISTRIBUTIONS: ValidatedDistribution[] = [
  {
    ...
    source: {
      ...
      year: 2009,
      title:
        "Cognitive Rigor: Blending the Strengths of Bloom's Taxonomy and Webb's Depth of Knowledge to Improve Teaching",
...
export function getValidatedDistribution(
  ...
  if (normalizedType === 'technical') {
    return (
      VALIDATED_DISTRIBUTIONS.find((d) => d.courseType === 'STEM') ??
...
export function getCitationString(distribution: ValidatedDistribution): string {
  ...
  const authors =
    s.authors.length > 2 ? `${s.authors[0]} et al.` : s.authors.join(' & ');
 ...
 */
export function getAllCitations(): ResearchCitation[] {
 ...
 */
export function calculateDistributionAlignment(
...
export function recommendDistribution(metadata: {
  ...
  const indicators: Array<{ pattern: RegExp; type: CourseType; weight: number }> = [
  ...
  let bestMatch: { type: CourseType; score: number } = { type: 'intermediate', score: 0 };
