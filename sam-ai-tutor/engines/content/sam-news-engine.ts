...
import { samNewsRankingEngine, RankedNewsArticle } from '@/sam/engines/advanced/sam-news-ranking-engine';
import { samRealNewsFetcher } from '@/sam/utils/sam-real-news-fetcher';
    ...
    try {
      ...
      if (useRealData) {
        ...
        try {
          ...
          const realArticles = await samRealNewsFetcher.fetchNews();

          // If we got real articles, transform them to match our NewsArticle interface
          if (realArticles && realArticles.length > 0) {
            const transformedArticles: NewsArticle[] = realArticles.map(article => ({
              ...
              source: {
                name: article.source,
                url: article.url,
                credibility: article.relevanceScore || 0.5,
                type: 'media' as const,
                country: 'US'
              },
