...
exports.validateCoreEnvVars = validateCoreEnvVars;
 ...
 */
function isPlaceholderValue(value, varName) {
 ...
 */
function validateEnvironmentVariables(throwOnError = true) {
    ...
    for (const config of ENV_CONFIGS) {
        ...
        // Track configured services
        if (config.name === 'DATABASE_URL')
            hasDatabase = true;
        if (config.name.includes('AUTH') || config.name.includes('NEXTAUTH'))
            hasAuth = true;
        if (config.name.includes('OPENAI') || config.name.includes('ANTHROPIC'))
            hasAI = true;
        if (config.name.includes('CLOUDINARY'))
            hasMedia = true;
        if (config.name.includes('REDIS') || config.name.includes('UPSTASH'))
            hasCaching = true;
        if (config.name.includes('SENTRY'))
            hasMonitoring = true;
    }
    ...
    const isStripePlaceholder = stripeSecret && (stripeSecret.includes('your_stripe_secret_key') ||
        stripeSecret === 'sk_test_your_stripe_secret_key_here' ||
        stripeSecret.startsWith('sk_test_your_stripe_secret_key'));
    const isStripePublicPlaceholder = stripePublic && (stripePublic.includes('your_stripe_publishable_key') ||
...
function validateCoreEnvVars() {
    validateRequiredEnvVars([
        exports.ENV_VARS.DATABASE_URL,
        exports.ENV_VARS.NEXTAUTH_SECRET,
    ]);
}
