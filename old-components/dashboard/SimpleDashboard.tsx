...
export function SimpleDashboard({ user }: SimpleDashboardProps) {
  // Determine which capabilities the user has
  const hasTeachingCapability = !!user.isTeacher;
  const hasAffiliateCapability = !!user.isAffiliate;
  const hasMultipleCapabilities = hasTeachingCapability || hasAffiliateCapability;

  // Default to learning tab, or first available capability
  ...
  // Dynamic welcome message based on capabilities
  const getWelcomeMessage = () => {
    if (hasTeachingCapability && hasAffiliateCapability) {
      return "Your complete dashboard for learning, teaching, and earning";
    } else if (hasTeachingCapability) {
      return "Manage your courses and continue learning";
    } else if (hasAffiliateCapability) {
      return "Track your earnings and continue learning";
    }
    return "Continue your learning journey and unlock your potential";
  };

  // Dynamic icon based on primary capability
  ...
  const WelcomeIcon = getWelcomeIcon();

  // If user has multiple capabilities, show tabbed interface
  if (hasMultipleCapabilities) {
    return (
      <motion.div
      ...
      >
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight bg-gradient-to-r from-blue-500 via-purple-500 to-indigo-500 bg-clip-text text-transparent">
                  Welcome back, {user.name}!
                </h1>
                <motion.div
                  animate={{
                    scale: [1, 1.2, 1],
                    rotate: hasTeachingCapability && hasAffiliateCapability ? [0, 360] : [0, 10, -10, 0]
                  }}
                  transition={{
                    duration: hasTeachingCapability && hasAffiliateCapability ? 3 : 2,
                    repeat: Infinity,
                    repeatDelay: hasTeachingCapability && hasAffiliateCapability ? 2 : 3
                  }}
                >
                  <WelcomeIcon className="h-6 w-6 sm:h-7 sm:w-7 text-purple-500" />
                </motion.div>
              </div>
        ...
        <motion.div variants={itemVariants}>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6 sm:space-y-8">
            <TabsList className={`grid w-full ${hasTeachingCapability && hasAffiliateCapability ? 'grid-cols-3 max-w-[650px]' : 'grid-cols-2 max-w-[450px]'} bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-1.5 rounded-xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm`}>
              {/* Learning Tab - Everyone gets this */}
              <TabsTrigger
                value="learning"
                className="flex items-center gap-2 px-3 sm:px-4 py-3 data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-indigo-500 data-[state=active]:text-white data-[state=active]:shadow-md text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all duration-200 rounded-lg font-medium text-sm"
              >
              ...
              {/* Teaching Tab - Only if isTeacher */}
              {hasTeachingCapability && (
                <TabsTrigger
                  value="teaching"
                  className="flex items-center gap-2 px-3 sm:px-4 py-3 data-[state=active]:bg-gradient-to-r data-[state=active]:from-orange-500 data-[state=active]:to-red-500 data-[state=active]:text-white data-[state=active]:shadow-md text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all duration-200 rounded-lg font-medium text-sm"
                >
              ...
              {/* Affiliate Tab - Only if isAffiliate */}
              {hasAffiliateCapability && (
                <TabsTrigger
                  value="affiliate"
                  className="flex items-center gap-2 px-3 sm:px-4 py-3 data-[state=active]:bg-gradient-to-r data-[state=active]:from-green-500 data-[state=active]:to-emerald-500 data-[state=active]:text-white data-[state=active]:shadow-md text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all duration-200 rounded-lg font-medium text-sm"
                >
            ...
            {/* Learning Content - Everyone gets this */}
            <TabsContent value="learning" className="space-y-6">
              <LearnerDashboard user={user} />
            </TabsContent>

            {/* Teaching Content - Only if isTeacher */}
            {hasTeachingCapability && (
            ...
            {/* Affiliate Content - Only if isAffiliate */}
            {hasAffiliateCapability && (
  ...
  // Single capability view (Learning only + optional CTA for becoming instructor)
  return (
    <motion.div
      variants={containerVariants}
      initial="hidden"
    ...
    >
      ...
      {/* Become an Instructor CTA - Only show if not already a teacher */}
      {!hasTeachingCapability && (
        <motion.div
