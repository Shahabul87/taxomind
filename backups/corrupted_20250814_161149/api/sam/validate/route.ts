...
interface ValidationResult {
  ...
  suggestions: {
    type: 'improvement' | 'alternative' export const POST = withAuth(async (request, context) => {
...
async function validateLearningGoals(goals: string[], context: any): Promise<ValidationResult> {
  ...
  if (goals.length < 3) {
    issues.push({
      type: 'warning' as 'error' | 'warning' | 'suggestion',
  ...
  if (goals.some(goal => goal.length < 20)) {
    issues.push({
      type: 'suggestion' as 'error' | 'warning' | 'suggestion',
