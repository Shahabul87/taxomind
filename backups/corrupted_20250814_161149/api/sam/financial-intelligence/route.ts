...
export async function POST(req: NextRequest) {
  try {
    ...
    // TODO: metadata field doesn't exist in User model - using defaults
    const organizationId = null; // session.user.metadata?.organizationId;
    const isOrgAdmin = false; // session.user.metadata?.isOrganizationAdmin;
    const isFinanceRole = false; // session.user.metadata?.role === "finance";
  ...
  const courseRevenue = await Promise.all(
    revenueStreams.map(async (stream) => {
      ...
      return {
        ...
        revenue: (course?.price || 0) * (stream._count.id || 0),
        purchases: stream._count.id || 0,
      };
    })
  );
...
async function handleCostAnalysis(
  ...
  // TODO: metadata field doesn't exist - counting all users in date range
...
async function handleProfitabilityAnalysis(
  ...
  if (courseId) {
    ...
    // TODO: completedAt field doesn't exist in Enrollment model - using all enrollments
