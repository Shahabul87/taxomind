...
import {
  StudentProfile,
  LearningHistory,
  PerformanceMetricsexport const POST = withAuth(async (request, context) => {
  const body = await request.json();
    ...
    let result;
      case "predict-outcomes":
        result = await handlePredictOutcomes(data, context.context.user.id);
        break;

      case "identify-at-risk":
        result = await handleIdentifyAtRisk(data, context.user);
        break;

      case "recommend-interventions":
        result = await handleRecommendInterventions(data, context.context.user.id);
        break;

      case "optimize-velocity":
        result = await handleOptimizeVelocity(data, context.context.user.id);
        break;

      case "calculate-probability":
        result = await handleCalculateProbability(data, context.context.user.id);
        break;
...
async function buildStudentProfile(
  ...
  const assessmentScores = extractAssessmentScores(progress);
