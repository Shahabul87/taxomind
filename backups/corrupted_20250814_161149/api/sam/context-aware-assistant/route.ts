...
- Workflow optimization`
  ...
  return (pageType in capabilities) 
    ? capabilities[pageType as keyof typeof capabilities] 
    : capabilities.other;
}
...
function generateContextualSuggestions(pageType: string, message: string): string[] {
  ...
  return (pageType in suggestions)
    ? suggestions[pageType as keyof typeof suggestions]
    : suggestions.other;
}
      ...
      if (descMatch) {
        return {
          ...
          details: {
            ...
            message: 'Update chapter description'
          }
        };
      }
    }
