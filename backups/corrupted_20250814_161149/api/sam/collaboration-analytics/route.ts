...
async function handleJoinSession(data: any, userId: string) {
  ...
  if (!dbSession || !dbSession.isActive || !dbSession.courseId) {
    throw new Error("Session not found, inactive, or missing course ID");
  }
...
async function getCourseTopContributors(courseId: string, startDate: Date) {
  // First get sessions for the course
...
async function handleGetSocialAnalytics(data: any, userId: string) {
  ...
  const activeNow = await db.user_progress.count({
    where: {
      courseId,
      updatedAt: {
