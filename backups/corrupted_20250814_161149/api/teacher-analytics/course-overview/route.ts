...
async function generateCourseAnalytics(course: any, timeFilter: Date, includeDetailed: boolean): Promise<CourseAnalytics> {
  ...
  // Get unique students
  const uniqueStudents = [...new Set(allAttempts.map((attempt: any) => attempt.userId))];
  ...
  const allScores = allAttempts.map((attempt: any) => attempt.scorePercentage || 0);
  const classAverage = allScores.length > 0 ? allScores.reduce((sum: number, score: number) => sum + score, 0) / allScores.length : 0;
