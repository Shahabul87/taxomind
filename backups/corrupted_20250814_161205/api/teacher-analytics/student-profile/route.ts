...
function calculateBloomsBreakdown(attempt: any) {
  ...
  bloomsLevels.forEach(level => {
    const levelAnswers = attempt.UserAnswer.filter((a: any) => a.ExamQuestion.bloomsLevel === level);
...
function calculateDifficultyBreakdown(attempt: any) {
  ...
  difficulties.forEach(difficulty => {
    const difficultyAnswers = attempt.UserAnswer.filter((a: any) => a.ExamQuestion.difficulty === difficulty);
...
function calculateLearningPatterns(attempts: any[]) {
  ...
  const preferredHour = Object.entries(hourCounts)
    .reduce((max: {hour: string, count: number}, [hour, count]: [string, unknown]) => (count as number) > max.count ? { hour, count: count as number } : max, { hour: '0', count: 0 });
