...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.jexport const POST = withAuth(async (request, context) => {
...
Make the insights practical and actionable for teachers.`;
  ...
  const response = await anthropic.messages.create({
    ...
    system: systemPrompt,
    messages: [
      { role: 'user' as const, content: 'Generate comprehensive class overview insights for this teacher.' }
    ]
  });
...
Focus on actionable recommendations to improve student engagement.`;
  ...
  const response = await anthropic.messages.create({
    ...
    system: systemPrompt,
    messages: [
      { role: 'user' as const, content: 'Analyze the engagement patterns and provide actionable insights.' }
    ]
  });
...
Focus on data-driven recommendations for improving student outcomes.`;
  ...
  const response = await anthropic.messages.create({
    ...
    system: systemPrompt,
    messages: [
      { role: 'user' as const, content: 'Analyze the performance data and provide actionable insights.' }
    ]
  });
...
Focus on prevention and early intervention strategies.`;
  ...
  const response = await anthropic.messages.create({
    ...
    system: systemPrompt,
    messages: [
      { role: 'user' as const, content: 'Analyze at-risk student patterns and provide intervention strategies.' }
    ]
  });
...
Focus on how to adapt teaching methods to match student learning patterns.`;
  ...
  const response = await anthropic.messages.create({
    ...
    system: systemPrompt,
    messages: [
      { role: 'user' as const, content: 'Analyze learning patterns and suggest teaching adaptations.' }
    ]
  });
...
Focus on data-driven content optimization strategies.`;
  ...
  const response = await anthropic.messages.create({
    ...
    system: systemPrompt,
    messages: [
      { role: 'user' as const, content: 'Analyze content effectiveness and provide optimization suggestions.' }
    ]
  });
