...
const detectAction = (message: string): string => {
  ...
  if (lowerMessage.includes('help') || lowerMessage.includes(export const POST = withAuth(async (request, context) => {
  return 'help';
}, {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const errorResponse: SamResponse = {
      response: "I'm experiencing technical difficulties. Please try again in a moment. If the issue persists, check your internet connection or contact support.",'
