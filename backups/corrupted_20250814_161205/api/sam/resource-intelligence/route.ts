...
import {
  Topic,
  Resourcexport const POST = withAuth(async (request, context) => {
  const body = await request.json();
    ...
    let result;
      ...
      case "roi-analysis":
        result = await handleROIAnalysis(data, context.context.user.id);
        break;

      case "personalize":
        result = await handlePersonalize(data, context.context.user.id);
        break;
...
async function buildLearnerProfile(userId: string): Promise<StudentResourceProfile> {
  ...
  const videoCount = activities.filter((a) => (a.metadata as any)?.contentType === "VIDEO").length;
  const articleCount = activities.filter(
    (a) => (a.metadata as any)?.contentType === "ARTICLE"
  ).length;
