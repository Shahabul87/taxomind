...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch security events'export const POST = withAdminAuth(async (request, context) => {
  );
...
// POST /api/enterprise/security - Create security event
  ...
  auditLog: true
    ...
    return NextResponse.json(
      { error: 'Failed to create security event' export const PATCH = withAdminAuth(async (request, context) => {
  );
