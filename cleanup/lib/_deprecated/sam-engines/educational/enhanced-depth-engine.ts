      ...
      if (existing && existing.contentHash === contentHash) {
        ...
        return {
          courseLevel: {
            ...
            dokDistribution: (existingData.dokDistribution as WebbDOKDistribution) ?? webbDOKAnalyzer.bloomsToEstimatedDOK(existing.bloomsDistribution as unknown as Record<string, number>),
            ...
            courseType: (existingData.courseType as CourseType) ?? 'intermediate',
            courseTypeMatch: (existingData.courseTypeMatch as number) ?? 50,
          },
          chapterAnalysis: [],
          objectivesAnalysis: (existingData.objectiveAnalysis as ObjectiveAnalysis[]) ?? [],
          objectiveDeduplication: {
            totalObjectives: 0,
            uniqueClusters: 0,
            duplicateGroups: [],
          ...
          assessmentQuality: (existingData.assessmentQuality as AssessmentQualityMetrics) ?? assessmentQualityAnalyzer.analyzeAssessments([]),
    ...
    try {
      await db.courseBloomsAnalysis.upsert({
        ...
        update: {
          ...
          contentHash,
          analyzedAt: new Date(),
          // Note: dokDistribution, courseType, courseTypeMatch, assessmentQuality, objectiveAnalysis
          // are not in the current Prisma schema - removed to fix build
        },
        create: {
          courseId,
          bloomsDistribution: response.courseLevel.bloomsDistribution,
