import { BookOpen, GraduationCap, Home, ChevronRight, User, Users, Star } from 'lucide-react';
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-400">
              ...
              {course.category && (
                <>
                  <ChevronRight className="h-3 w-3" />
                  <span className="text-slate-300">{course.category.name}</span>
                </>
              )}
            </div>
          ...
          <div className="relative pt-12 lg:pt-16">
            {secureImageUrl ? (
              ...
              <div className="relative aspect-video rounded-2xl overflow-hidden shadow-2xl border border-slate-700/50 bg-gradient-to-br from-slate-800 to-slate-700 flex items-center justify-center">
                <GraduationCap className="h-32 w-32 text-slate-600" />
              </div>
            )}

            {/* Floating Elements */}
            <div className="absolute -right-4 -top-4 w-24 h-24 bg-blue-500/20 rounded-full blur-2xl" />
            <div className="absolute -left-4 -bottom-4 w-32 h-32 bg-indigo-500/20 rounded-full blur-2xl" />
          </div>
        </div>

        {/* Instructor Row with Enroll Button - Full Width, Horizontally Aligned */}
        {course.user && (
          <div className="grid lg:grid-cols-2 gap-12 items-center mt-6 pt-4 mb-12 border-t border-slate-700">
            ...
            {onEnroll && (
              <div className="flex justify-end">
                <Button
                  ...
                  disabled={isEnrolled}
                  size="lg"
                  className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold px-8 py-6 text-lg shadow-lg shadow-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                >
