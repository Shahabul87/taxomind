...
export function LearningJourneyMap({ user }: LearningJourneyMapProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "locked": return "bg-gray-100 text-slate-500 dark:text-slate-400 dark:bg-gray-800/30 dark:text-slate-500";
      default: return "bg-gray-100 text-slate-500 dark:text-slate-400 dark:bg-gray-800/30 dark:text-slate-500";
    }
  };
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
          <CardContent className="flex items-center justify-center py-12">
            <div className="flex flex-col items-center gap-3">
              <Loader2 className="w-8 h-8 animate-spin text-purple-500" />
              <p className="text-sm text-slate-500 dark:text-slate-400">Loading your learning journey...</p>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
          <CardHeader>
            <CardTitle className="flex items-center gap-3">
              ...
              <span className="text-slate-900 dark:text-white">Your Learning Journey</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="flex flex-col items-center justify-center py-8 text-center">
            <Target className="w-12 h-12 text-slate-500 mb-4" />
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Start Your Journey</h3>
              ...
              <div className="flex items-center gap-4 p-3 rounded-lg bg-gradient-to-r from-orange-500/10 to-yellow-500/10 border border-orange-500/20">
                {summary.currentStreak > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-900 dark:text-white font-medium">{summary.currentStreak} day streak</span>
                  </div>
                )}
                {summary.totalXP > 0 && (
                  <div className="flex items-center gap-2">
                    <Trophy className="w-5 h-5 text-yellow-500" />
                    <span className="text-sm text-slate-900 dark:text-white font-medium">{summary.totalXP.toLocaleString()} XP</span>
                  </div>
                )}
                {summary.level > 1 && (
          ...
          <Card className="border-0 shadow-lg bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                <span className="text-slate-900 dark:text-white">Current Focus</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <Badge variant="outline" className="border-slate-500/50 text-slate-500 dark:text-slate-400">
                      {currentNode.type}
                    </Badge>
                  </div>
                </div>
        ...
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-3">
                ...
                <span className="text-slate-900 dark:text-white">Learning Path</span>
              </CardTitle>
          ...
          <CardContent>
            <div className="space-y-3">
              {nodes.slice(0, 8).map((node, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="relative flex items-center gap-4 p-4 rounded-lg bg-slate-50 hover:bg-slate-100 dark:bg-slate-700/30 dark:hover:bg-slate-700/50 transition-all duration-200"
                  >
                    {/* Node Icon */}
                    <div className={`p-3 rounded-full bg-gradient-to-r ${getNodeColor(node.status)} text-slate-900 dark:text-white flex-shrink-0`}>
              ...
              {nodes.length > 8 && (
                <Link href="/dashboard/user/goals">
                  <Button variant="ghost" className="w-full text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white">
