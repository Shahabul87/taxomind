...
export function AdminAnalyticsClient({ initialStats }: AdminAnalyticsClientProps) {
  ...
  const maxChartValue = Math.max(
    ...stats.dailyStats.map((d) => Math.max(d.users, d.enrollments)),
    1
  );
      ...
      <div className="container mx-auto px-2 sm:px-4 py-3 sm:py-6 md:py-8 space-y-4 sm:space-y-6 md:space-y-8">
        ...
        <div className="grid grid-cols-1 gap-2.5 sm:gap-3 md:gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            },
            {
              ...
              change: `${stats.completionsChange >= 0 ? "+" : ""}${stats.completionsChange}%`,
              trend: stats.completionsChange >= 0 ? "up" : "down",
              icon: Target,
            ...
            },
            {
              ...
              value: formatSessionTime(stats.avgSessionMinutes),
              change: `${stats.sessionTimeChange >= 0 ? "+" : ""}${stats.sessionTimeChange}%`,
              trend: stats.sessionTimeChange >= 0 ? "up" : "down",
              icon: Clock,
        ...
        >
          <Tabs defaultValue="users" className="space-y-3 sm:space-y-4">
              ...
              <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200/50 dark:border-slate-700/50 shadow-lg">
                <CardHeader className="pb-3 sm:pb-4 px-3 sm:px-6 pt-3 sm:pt-6">
                  <CardTitle className="flex items-center gap-2 sm:gap-3 text-slate-900 dark:text-white">
                    ...
                    <span className="text-base sm:text-lg font-semibold">New User Registrations</span>
                  </CardTitle>
                </CardHeader>
                    ...
                    {stats.dailyStats.map((data, idx) => (
                      ...
                      >
                        <div
                          ...
                          style={{ height: `${Math.max((data.users / maxChartValue) * 100, 5)}%` }}
                        >
                        ...
                        <span className="text-[10px] sm:text-xs font-medium text-slate-600 dark:text-slate-400">{data.date}</span>
                      </motion.div>
                    ))}
                  </div>
                </CardContent>
            ...
            <TabsContent value="enrollments" className="space-y-3 sm:space-y-4">
              <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200/50 dark:border-slate-700/50 shadow-lg">
                <CardHeader className="pb-3 sm:pb-4 px-3 sm:px-6 pt-3 sm:pt-6">
                  <CardTitle className="flex items-center gap-2 sm:gap-3 text-slate-900 dark:text-white">
                    ...
                    <span className="text-base sm:text-lg font-semibold">Course Enrollments</span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
                  <div className="h-[200px] sm:h-[250px] md:h-[300px] flex items-end gap-1 sm:gap-2 p-2 sm:p-4 overflow-x-auto">
                    {stats.dailyStats.map((data, idx) => (
                      ...
                      >
                        <div
                          ...
                          style={{ height: `${Math.max((data.enrollments / maxChartValue) * 100, 5)}%` }}
                        >
                          <div className="absolute -top-5 sm:-top-6 left-1/2 -translate-x-1/2 text-[10px] sm:text-xs font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">
                            {data.enrollments}
                          </div>
              ...
              <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
                  ...
                  <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
                    <div className="space-y-3 sm:space-y-4">
                      {stats.popularCourses.length > 0 ? (
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-4">No course data available</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200/50 dark:border-slate-700/50 shadow-lg">
                  ...
                  <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
                    <div className="space-y-3 sm:space-y-4">
                      {[
                        { metric: "Page Views", value: stats.userActivityMetrics.totalPageViews.toLocaleString(), change: `${stats.userActivityMetrics.pageViewsChange >= 0 ? "+" : ""}${stats.userActivityMetrics.pageViewsChange}%` },
                        { metric: "Video Watched", value: `${stats.userActivityMetrics.videoWatchedHours} hrs`, change: `${stats.userActivityMetrics.videoChange >= 0 ? "+" : ""}${stats.userActivityMetrics.videoChange}%` },
                        { metric: "Exams Completed", value: stats.userActivityMetrics.assignmentsSubmitted.toLocaleString(), change: `${stats.userActivityMetrics.assignmentsChange >= 0 ? "+" : ""}${stats.userActivityMetrics.assignmentsChange}%` },
                        { metric: "Forum Posts", value: stats.userActivityMetrics.forumPosts.toLocaleString(), change: `${stats.userActivityMetrics.forumPostsChange >= 0 ? "+" : ""}${stats.userActivityMetrics.forumPostsChange}%` },
                      ].map((item, idx) => (
                        <motion.div
                          key={idx}
              ...
              <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200/50 dark:border-slate-700/50 shadow-lg">
                <CardHeader className="pb-3 sm:pb-4 px-3 sm:px-6 pt-3 sm:pt-6">
                  <CardTitle className="flex items-center gap-2 sm:gap-3 text-slate-900 dark:text-white">
                    ...
                    <span className="text-base sm:text-lg font-semibold">Category Performance</span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
                  <div className="space-y-3 sm:space-y-4">
                    {stats.categoryPerformance.length > 0 ? (
                      stats.categoryPerformance.map((cat, idx) => (
                        ...
                        >
                          ...
                          <span className="text-xs sm:text-sm font-medium text-slate-900 dark:text-slate-100">{formatCurrency(cat.revenue)}</span>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-4">No category data available</p>
                    )}
                  </div>
                </CardContent>
              </Card>
