...
export function AdminReportsClient({ initialStats, initialReports }: AdminReportsClientProps) {
  ...
  const reportTypes = useMemo(() => {
    const types = new Set(initialReports.map((r) => r.type));
  ...
  }, [initialReports, searchQuery, filterType, filterStatus]);
      ...
      <div className="container mx-auto px-2 sm:px-4 py-3 sm:py-6 md:py-8 space-y-4 sm:space-y-6 md:space-y-8">
        ...
        >
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
              View audit logs and platform activity reports
            </p>
          </div>
        ...
        >
          {[
            {
              title: "Total Reports",
              value: initialStats.totalReports.toLocaleString(),
              change: `+${initialStats.reportsThisMonth} this month`,
              trend: "up",
              icon: FileText,
            ...
            {
              title: "Completed",
              value: initialStats.completedReports.toLocaleString(),
              change: `${initialStats.successRate}% success rate`,
              trend: "up",
              icon: CheckCircle,
            ...
            {
              title: "Processing",
              value: initialStats.processingReports.toLocaleString(),
              change: "In progress",
              trend: "neutral",
              icon: Clock,
            ...
            {
              title: "Failed",
              value: initialStats.failedReports.toLocaleString(),
              change: initialStats.failedReports > 0 ? "Requires attention" : "All systems normal",
              trend: initialStats.failedReports > 0 ? "down" : "up",
              icon: AlertTriangle,
            ...
            <CardHeader className="pb-3 sm:pb-4 px-3 sm:px-6 pt-3 sm:pt-6">
              <div className="flex flex-col gap-3 sm:gap-4 md:flex-row md:items-center md:justify-between">
                <CardTitle className="flex items-center gap-2 sm:gap-3 text-slate-900 dark:text-white">
                  ...
                  <span className="text-base sm:text-lg font-semibold">Recent Activity Logs</span>
                </CardTitle>
              ...
              {filteredReports.length === 0 ? (
                <div className="text-center py-8 sm:py-12">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">
                    {initialReports.length === 0 ? "Activity logs will appear here as users interact with the platform" : "Try adjusting your search or filters"}
                  </p>
                </div>
              ) : (
                <>
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                  <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center shadow-sm shrink-0">
                                    <ReportIcon className="h-5 w-5 text-slate-600 dark:text-slate-300" />
                                  </div>
                          ...
                          {filteredReports.slice(0, 20).map((report, idx) => {
                            ...
                            return (
                              ...
                              >
                                <TableCell className="text-left">
                                  <div className="flex items-center gap-2 sm:gap-3">
                                    <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-lg bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center shadow-sm shrink-0">
                                      <ReportIcon className="h-4 w-4 sm:h-5 sm:w-5 text-slate-600 dark:text-slate-300" />
                                    </div>
                                ...
                                <TableCell className="text-center text-slate-700 dark:text-slate-300 text-sm truncate max-w-[150px]">{report.generatedBy}</TableCell>
