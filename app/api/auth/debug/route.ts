...
export async function GET(request: Request) {
  try {
    ...
    if (config.urls.NEXT_PUBLIC_APP_URL === 'NOT SET') {
      config.recommendations.push(
        'WARNING: Set NEXT_PUBLIC_APP_URL for consistent URL handling across the app'
      );
    }
    ...
    if (authUrl && appUrl && !authUrl.includes(new URL(appUrl).host)) {
      config.recommendations.push(
        `WARNING: URL mismatch: AUTH_URL (${authUrl}) and NEXT_PUBLIC_APP_URL (${appUrl}) should use the same domain`
      );
    }
    ...
    console.error('[AUTH_DEBUG] GET Error:', error);
