...
export const POST = withAdminAuth(async (
  ...
  // Rate limit AI content analysis
  ...
  try {
    ...
    const validationResult = AnalyzeContentBodySchema.safeParse(body);
    if (!validationResult.success) {
      return createErrorResponse(new ApiError('Invalid request body', 400));
    }
    const { sectionId, chapterId, courseId, sectionData, context: requestContext } = validationResult.data;
