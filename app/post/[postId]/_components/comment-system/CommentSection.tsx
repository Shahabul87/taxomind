...
export const CommentSection = ({ postId, initialComments = [] }: CommentSectionProps) => {
  ...
  const fetchComments = useCallback(async (pageToFetch = 1, sortOption = 'newest') => {
    ...
    try {
      ...
      if (!apiResponse || !apiResponse.success || !apiResponse.data) {
        logger.error("Invalid API response structure", apiResponse);
        setComments([]);
        setPagination({
          page: 1,
          pageSize: 20,
      ...
      const { data: fetchedComments, pagination: paginationData } = apiResponse.data;

      // Safety check: ensure fetchedComments is an array
      if (!Array.isArray(fetchedComments)) {
        logger.error("Invalid API response: fetchedComments is not an array", { fetchedComments, apiResponse });
        setComments([]);
        setPagination({
          page: 1,
          pageSize: 20,
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 404) {
          ...
          toast.error("Server error loading comments");
        } else {
          toast.error(`Failed to load comments: ${error.response?.data?.error || error.message}`);
        }
      } else {
  ...
  // Fetch comments only on initial mount if no initial comments
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Fetch comments only if URL params changed and differ from current pagination
    if (shouldFetch || (urlPage !== pagination.page && page === urlPage)) {
      fetchComments(urlPage, urlSortBy || sortBy);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams.toString()]); // Only depend on URL changes
