...
const ReadingModes = ({ post }: ReadingModesProps) => {
  ...
  return (
    <div className="flex flex-col w-full overflow-visible">
      ...
      <div className="sticky top-0 w-full px-2 sm:px-4 py-1.5 sm:py-2 mb-3 sm:mb-6 z-[45] bg-gradient-to-b from-white/95 via-white/90 to-transparent dark:from-slate-900/95 dark:via-slate-900/90 dark:to-transparent backdrop-blur-md transition-all duration-300">
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3 }}
          className="flex flex-col space-y-2 sm:space-y-0 sm:flex-row items-start sm:items-center justify-between bg-white/80 dark:bg-slate-800/80 border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm shadow-sm rounded-lg sm:rounded-xl"
        >
          {/* Mode Selection */}
          <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-2 sm:gap-3 sm:items-center">
            <div className="flex items-center gap-2 w-full sm:w-auto px-2 sm:px-1 py-1.5 sm:py-0">
              <Book className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 dark:text-blue-400 flex-shrink-0" />
              <span className="text-xs sm:text-sm md:text-base font-semibold text-slate-900 dark:text-white">
                Reading Mode
              </span>
            </div>
                ...
                {readingModes.map((mode) => (
                  !mode.desktopOnly && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={cn(
                        "relative px-3 py-2 rounded-md flex items-center gap-1.5 transition-all duration-200 flex-shrink-0 min-w-[100px] justify-center touch-manipulation",
                        activeMode === mode.id
                          ? "bg-gradient-to-r from-blue-500 to-indigo-500 text-white shadow-md"
                          : "text-slate-700 dark:text-slate-300 bg-slate-100/90 dark:bg-slate-700/70 border border-slate-200/60 dark:border-slate-600/60"
                      )}
                    >
      ...
      {/* Content Area */}
      <div className={cn(
        "flex-1 w-full bg-white/80 dark:bg-slate-800/80 rounded-lg sm:rounded-xl md:rounded-2xl border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm shadow-md sm:shadow-lg",
