  ...
  try {
    ...
    if (isStreakAction(action)) {
      await updateSAMStreak(userId, {
        ...
        currentStreak: userStats.streaks[0]?.currentStreak || 0 + 1,
        longestStreak: Math.max(
          userStats.streaks[0]?.longestStreak || 0,
          (userStats.streaks[0]?.currentStreak || 0) + 1
        ),
        courseId: context?.courseId,
      });
    }
    ...
    // Check for level up
    const newTotalPoints = userStats.totalPoints + totalPointsAwarded;
...
async function completeChallengeForUser(
  ...
  try {
    ...
    await recordSAMInteraction({
      ...
      context: { challengeId: challenge.id, challenge },
...
export async function getAvailableChallengesForUser(userId: string): Promise<Challenge[]> {
  try {
    ...
    const userLevel = calculateLevel(userStats.totalPoints);
...
export async function getUserAchievementSummary(userId: string): Promise<{
  ...
  try {
    ...
    const pointsToNextLevel = nextLevelPoints - userStats.totalPoints;
    ...
    const recommendations = getAchievementRecommendations(userStats, achievementIds);
