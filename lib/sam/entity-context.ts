 ...
 */
export async function fetchChapterContext(chapterId: string): Promise<ChapterContext | null> {
  try {
 ...
 */
export async function fetchSectionContext(sectionId: string): Promise<SectionContext | null> {
  try {
    ...
    return {
      ...
      content: section.description ? truncateContent(section.description, 2000) : null,
...
export async function buildEntityContext(
  ...
  try {
    ...
    else if ((pageType === 'course-detail' || pageType === 'course-create') && entityId) {
...
function buildCourseSummary(course: CourseContext): string {
  ...
  if (course.whatYouWillLearn.length > 0) {
    parts.push(`Learning outcomes: ${course.whatYouWillLearn.slice(0, 5).join('; ')}`);
  }
  ...
  if (course.chapters.length > 0) {
    const chapterList = course.chapters.slice(0, 5).map(ch => ch.title).join(', ');
    parts.push(`Chapter titles: ${chapterList}${course.chapters.length > 5 ? '...' : ''}`);
  }
...
function buildChapterSummary(chapter: ChapterContext, course: CourseContext | null): string {
  const parts: string[] = [
  ...
  if (chapter.sections.length > 0) {
    const sectionList = chapter.sections.slice(0, 5).map(s => s.title).join(', ');
    parts.push(`Section titles: ${sectionList}${chapter.sections.length > 5 ? '...' : ''}`);
  }
 ...
 */
export function buildFormSummary(formData: Record<string, unknown> | undefined): string {
