...
const validStage3Description = [
  ...
  '<p>In production environments, API versioning powers the deployment pipelines at companies like Stripe and Twilio, where backward compatibility is a core product promise. Teams use version headers, URL path segments, or content negotiation to route clients to the correct contract. Monitoring dashboards track version adoption rates, enabling data-driven deprecation timelines that minimize disruption while keeping the API surface manageable for long-term maintenance.</p>',
].join('');
