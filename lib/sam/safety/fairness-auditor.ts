...
import { FairnessSafetyValidator, type FullFairnessValidatorConfig } from './fairness-validator';
 ...
 */
export const DEFAULT_AUDIT_CONFIG: Required<Omit<FairnessAuditConfig, 'logger'>> = {
  minSampleSize: 30,
...
export class FairnessAuditor {
  ...
  private groupByDemographics(
    ...
    const groups = new Map<string, Map<string, EvaluationWithDemographics[]>>();
  ...
  private analyzeScoreDistribution(
    ...
    const byGroup = new Map<string, { mean: number; median: number; stdDev: number }>();
  ...
  private calculateOverallStatistics(
    ...
    return {
      ...
      safetyPassRate:
        Array.from(validationResults.values()).filter((r) => r.issues.length === 0)
...
export function createStrictFairnessAuditor(
  config?: Omit<FullFairnessAuditorConfig, 'disparityThreshold' | 'significanceThreshold'>
): FairnessAuditor {
...
export function createLenientFairnessAuditor(
  config?: Omit<FullFairnessAuditorConfig, 'disparityThreshold' | 'minSampleSize'>
): FairnessAuditor {
  return new FairnessAuditor({
    ...config,
