...
async function calculateContentInsights(
  ...
  const contentLengths = contentInteractions
    .filter(i => i.result?.contentLength)
    .map(i => i.result.contentLength);
...
function analyzeBehaviorPatterns(interactions: any[]): BehaviorPatterns {
...
function generatePersonalizedInsights(
  ...
  if (userStats.streaks.length > 0 && userStats.streaks[0].currentStreak > 7) {
  ...
  const estimatedTimeToGoal = estimateTimeToNextLevel(userStats.totalPoints, metrics.learningVelocity);
...
// Helper functions
function calculateContentQuality(analyticsRecords: any[]): number {
...
function calculateEngagementScore(interactions: any[], analyticsRecords: any[]): number {
...
function calculateRecencyScore(interactions: any[]): number {
...
function calculateDiversityScore(interactions: any[]): number {
...
function extractMilestones(interactions: any[]): Array<{ date: Date; description: string }> {
...
function predictNextMilestone(userStats: any, metrics: LearningMetrics): string {
  ...
  if (userStats.badgeCount < 5) return 'Unlock 5 achievement badges';
