/**
 * OpenAI Adapter Test Script
 *
 * Tests the OpenAI AI adapter with various operations:
 * 1. Configuration check
 * 2. Simple chat completion
 * 3. Streaming chat completion
 *
 * Usage:
 *   npx ts-node scripts/test-openai-adapter.ts
 *
 * Requires:
 *   OPENAI_API_KEY environment variable
 */

import { createOpenAIAdapter } from '@sam-ai/core';
...
async function testOpenAIAdapter(): Promise<void> {
  log('\n========================================', 'cyan');
  log('  OpenAI Adapter Test Suite', 'cyan');
  log('========================================\n', 'cyan');
  ...
  if (!apiKey) {
    ...
    log('Please set your OpenAI API key in .env.development:', 'yellow');
    log('  OPENAI_API_KEY=sk-your-api-key-here', 'yellow');
    process.exit(1);
  }
  ...
  try {
    ...
    const response = await adapter.chat({
      messages: [
        {
          role: 'user',
          content: 'Say "OpenAI works!" and nothing else.',
        },
      ],
      maxTokens: 50,
      temperature: 0.1,
  ...
  log('========================================\n', 'cyan');
  log('OpenAI adapter is ready for use!', 'green');
  log('You can now use it in your application with:', 'reset');
  log('  import { createOpenAIAdapter } from "@sam-ai/core";', 'reset');
  log('\nOr use the provider factory:', 'reset');
  log('  import { createAIAdapter } from "@/lib/sam/providers";', 'reset');
  log('  const adapter = createAIAdapter("openai");', 'reset');
}

// Run tests
testOpenAIAdapter().catch((error) => {
