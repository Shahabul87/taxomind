...
describe('DataEncryption', () => {
  ...
  const testMasterKey = crypto.randomBytes(32).toString('hex'); // 64 hex chars = 32 bytes
  const originalEnvKey = process.env.ENCRYPTION_MASTER_KEY;

  beforeAll(() => {
    // Set test master key (must be 64 hex characters)
  ...
  beforeEach(() => {
    // Ensure env key is set before each test (some tests delete it)
    process.env.ENCRYPTION_MASTER_KEY = testMasterKey;
  ...
  describe('Initialization', () => {
    ...
    it('should accept a valid 64-char hex key as parameter', () => {
      delete process.env.ENCRYPTION_MASTER_KEY;

      const validKey = crypto.randomBytes(32).toString('hex');
      const keyEncryption = new DataEncryption(validKey);
      expect(keyEncryption).toBeDefined();

      // Restore for other tests
      process.env.ENCRYPTION_MASTER_KEY = testMasterKey;
    });

    it('should throw error for key that is not 64 hex characters', () => {
      delete process.env.ENCRYPTION_MASTER_KEY;

      expect(() => new DataEncryption('short-key')).toThrow('must be 64 hex characters');
  ...
  describe('Key Rotation', () => {
    it('should rotate encryption key successfully', async () => {
      ...
      // Verify original key cannot decrypt rotated data
...
describe('EncryptionUtils', () => {
  ...
  beforeEach(() => {
    // Ensure env key is set before each test
    process.env.ENCRYPTION_MASTER_KEY = testMasterKey;
  });
