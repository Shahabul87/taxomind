...
import { POST } from '@/app/api/create-nested-reply/route';
...
function req(body: Record<string, unknown>) {
  return new NextRequest('http://localhost:3000/api/create-nested-reply', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}

describe('/api/create-nested-reply route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('returns 429 when rate limited', async () => {
    mockIsRateLimited.mockResolvedValueOnce({ limited: true, reset: Date.now() + 2000 });
    const res = await POST(
      req({ postId: 'post-1', commentId: 'comment-1', content: 'hello' })
    );
  ...
  it('creates reply with derived depth/path when parentReplyId is provided', async () => {
    const res = await POST(
      req({
        postId: 'post-1',
        commentId: 'comment-1',
