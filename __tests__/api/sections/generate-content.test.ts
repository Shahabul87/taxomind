...
import { POST } from '@/app/api/sections/generate-content/route';
...
function req(body: Record<string, unknown>) {
  return new NextRequest('http://localhost:3000/api/sections/generate-content', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
describe('POST /api/sections/generate-content', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('returns generated content payload when section exists', async () => {
    ...
    expect(body.data.contentType).toBe('blog');
