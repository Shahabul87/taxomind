jest.mock('@/lib/api/cron-auth', () => ({
  ...
  verifyCronAuth: jest.fn(),
}));

import { NextRequest, NextResponse } from 'next/server';
import { GET } from '@/app/api/cron/sam-memory-lifecycle/route';
import { withCronAuth } from '@/lib/api/cron-auth';

const mockWithCronAuth = withCronAuth as jest.Mock;

describe('/api/cron/sam-memory-lifecycle route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('returns auth response when cron auth blocks', async () => {
    ...
    const req = new NextRequest('http://localhost:3000/api/cron/sam-memory-lifecycle');
