...
import { GET } from '@/app/api/cron/sam-analytics-rollups/route';
import { withCronAuth } from '@/lib/api/cron-auth';

const mockWithCronAuth = withCronAuth as jest.Mock;

describe('/api/cron/sam-analytics-rollups route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('returns auth response when cron auth blocks', async () => {
    ...
    const req = new NextRequest('http://localhost:3000/api/cron/sam-analytics-rollups');
