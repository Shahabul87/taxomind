/**
 * Tests for Nested Reply Reactions Route - app/api/posts/[postId]/comments/[commentId]/replies/[replyId]/reactions/route.ts
 */
...
import { POST } from '@/app/api/posts/[postId]/comments/[commentId]/replies/[replyId]/reactions/route';
...
function req(body: Record<string, unknown>) {
  return new Request('http://localhost:3000/api/posts/post-1/comments/comment-1/replies/reply-1/reactions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
describe('Nested reply reactions route', () => {
  const tx = {
    reaction: {
      findFirst: jest.fn(),
  ...
  beforeEach(() => {
    ...
    (db.comment.findFirst as jest.Mock).mockResolvedValue({ id: 'comment-1' });
  ...
  it('returns 404 when comment or reply is missing', async () => {
  ...
  it('creates reaction when no matching reaction exists', async () => {
    tx.reaction.findFirst.mockResolvedValue(null);

    const res = await POST(req({ type: 'love' }) as any, props() as any);
    expect(res.status).toBe(200);
    expect(tx.reaction.deleteMany).toHaveBeenCalled();
    expect(tx.reaction.create).toHaveBeenCalled();
  });

  it('deletes reaction when identical reaction exists', async () => {
