...
import { PATCH } from '@/app/api/courses/[courseId]/chapters/[chapterId]/sections/[sectionId]/unpublish/route';
...
describe('/api/courses/[courseId]/chapters/[chapterId]/sections/[sectionId]/unpublish route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockAuth.mockResolvedValue({ user: { id: 'teacher-1' } });
    (db.course.findUnique as jest.Mock).mockResolvedValue({ id: 'c1', userId: 'teacher-1' });
    (db.section.update as jest.Mock).mockResolvedValue({ id: 's1', isPublished: false });
  });
  ...
  it('unpublishes section and returns payload', async () => {
    const req = new NextRequest('http://localhost:3000/api/x', { method: 'PATCH' });
    const res = await PATCH(req, {
      params: Promise.resolve({ courseId: 'c1', chapterId: 'ch1', sectionId: 's1' }),
    });
