...
__export(index_exports, {
  ...
  DEFAULT_SOURCE_CREDIBILITY: () => DEFAULT_SOURCE_CREDIBILITY,
  ...
  ImpactLevelSchema: () => ImpactLevelSchema,
  ...
  NewsRankingEngine: () => NewsRankingEngine,
  NewsSourceTypeSchema: () => NewsSourceTypeSchema,
  PACKAGE_NAME: () => PACKAGE_NAME,
  PACKAGE_VERSION: () => PACKAGE_VERSION,
  ResearchPaperSchema: () => ResearchPaperSchema,
  TechnicalDepthSchema: () => TechnicalDepthSchema,
  createInMemoryCache: () => createInMemoryCache,
  createKnowledgeAggregator: () => createKnowledgeAggregator,
  createNewsRankingEngine: () => createNewsRankingEngine,
  getNewsRankingEngine: () => getNewsRankingEngine,
...
var DEFAULT_RANKING_WEIGHTS = {
  freshness: 0.25,
  // 25% - Prioritize recent news
  relevance: 0.2,
  // 20% - AI/ML specific relevance
  impact: 0.15,
  // 15% - Industry/research impact
  credibility: 0.15,
...
var NewsSourceTypeSchema = import_zod2.z.enum([
  "official",
  "research",
  "media",
  "blog",
...
var ImpactLevelSchema = import_zod2.z.enum(["low", "medium", "high", "critical"]);
var TechnicalDepthSchema = import_zod2.z.enum([
  "beginner",
  "intermediate",
  "advanced",
  "expert"
]);
var NewsCategorySchema = import_zod2.z.enum([
  "breakthrough",
  "research",
  "industry",
  "policy",
...
var DEFAULT_SOURCE_CREDIBILITY = {
  "OpenAI Blog": 95,
  "Google DeepMind": 98,
  "Microsoft News": 95,
...
// src/news/news-ranking-engine.ts
var INNOVATION_KEYWORDS = [
  "first",
  "novel",
  "new approach",
  "breakthrough",
...
var NewsRankingEngine = class {
  ...
  calculateFreshness(publishDate) {
    ...
    const ageInHours = (now.getTime() - new Date(publishDate).getTime()) / (1e3 * 60 * 60);
    if (ageInHours <= 1) return 100;
    if (ageInHours <= 3) return 95;
    if (ageInHours <= 6) return 90;
    if (ageInHours <= 12) return 80;
    if (ageInHours <= 24) return 70;
    if (ageInHours <= 48) return 50;
    if (ageInHours <= 72) return 30;
    if (ageInHours <= 168) return 20;
    return Math.max(0, 20 - ageInHours / 168 * 20);
  }
  /**
  ...
  determineTrendingStatus(article, rankingScore) {
    const ageInHours = (Date.now() - new Date(article.publishDate).getTime()) / (1e3 * 60 * 60);
  ...
  async getTrendingNews(articles, limit = 10) {
    const rankedArticles = await this.rankNews(articles);
    return rankedArticles.filter((article) => article.trendingStatus === "hot" || article.trendingStatus === "rising").slice(0, limit);
  }
  /**
...
function createNewsRankingEngine(config) {
  return new NewsRankingEngine(config);
}
var defaultEngine = null;
function getNewsRankingEngine() {
  if (!defaultEngine) {
