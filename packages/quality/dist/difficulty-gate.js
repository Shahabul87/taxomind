...
export class DifficultyMatchGate {
    ...
    calculateDifficultyMetrics(text) {
        ...
        const readabilityScore = this.calculateReadability(averageSentenceLength, avgSyllablesPerWord);
        // Determine levels
        ...
        const overallLevel = this.calculateOverallLevel(vocabularyLevel, sentenceLevel, conceptLevel);
        return {
            vocabularyLevel,
            conceptLevel,
            sentenceLevel,
    ...
    countTechnicalTerms(words) {
        ...
        return words.filter((w) => technicalPatterns.some((p) => p.test(w))).length;
    }
    /**
    ...
    getConceptLevel(text) {
        ...
        if (expertCount >= 2)
            return 'expert';
        if (advancedCount >= 2 || (expertCount >= 1 && advancedCount >= 1))
            return 'advanced';
        if (intermediateCount >= 2 || advancedCount >= 1)
            return 'intermediate';
        return 'beginner';
    }
    /**
    ...
    getLevelDifference(actual, target) {
        const diff = Math.abs(this.difficultyOrder[actual] - this.difficultyOrder[target]);
    ...
    checkBeginnerAccessibility(text, metrics) {
        ...
        const undefinedJargon = jargonMatches.filter((j) => !text.toLowerCase().includes(`${j.toLowerCase()} is`) &&
 ...
 */
export function createDifficultyMatchGate(config) {
    return new DifficultyMatchGate(config);
}
//# sourceMappingURL=difficulty-gate.js.map