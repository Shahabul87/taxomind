...
export class ContentQualityGatePipeline {
    ...
    async validateWithRetry(content, startTime) {
        ...
        const failedGates = gateResults
            .filter((r) => !r.passed)
            .map((r) => r.gateName);
        const criticalIssues = gateResults.flatMap((r) => r.issues.filter((i) => i.severity === 'critical'));
    ...
    async runGates(gates, content) {
        if (this.config.parallel) {
            ...
            const promises = gates.map((gate) => this.runGateWithTimeout(gate, content));
    ...
    async runGateWithTimeout(gate, content) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Gate timeout')), gateTimeout));
    ...
    async quickValidate(content) {
        ...
        const gates = Array.from(this.gates.values()).filter((g) => essentialGates.includes(g.name));
 ...
 */
export function createQualityGatePipeline(config) {
 ...
 */
export async function quickValidateContent(content) {
