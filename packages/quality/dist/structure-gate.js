...
export class StructureGate {
    ...
    async evaluate(content) {
        ...
        if (metrics.longestParagraphLength > this.config.maxParagraphLength) {
            const severity = metrics.longestParagraphLength > this.config.maxParagraphLength * 2
                ? 'high'
                : 'medium';
    ...
    detectMarkdownElements(text) {
        ...
        if (/^\s*[-*+]\s+/m.test(text))
            elements.push('bullet-list');
        if (/^\s*\d+\.\s+/m.test(text))
            elements.push('numbered-list');
        if (/```[\s\S]*?```/.test(text))
            elements.push('code-block');
        if (/`[^`]+`/.test(text))
            elements.push('inline-code');
        if (/\*\*[^*]+\*\*/.test(text))
            elements.push('bold');
        if (/\*[^*]+\*/.test(text) || /_[^_]+_/.test(text))
            elements.push('italic');
        if (/\[.+\]\(.+\)/.test(text))
            elements.push('link');
        if (/!\[.+\]\(.+\)/.test(text))
            elements.push('image');
 ...
 */
export function createStructureGate(config) {
    return new StructureGate(config);
}
//# sourceMappingURL=structure-gate.js.map