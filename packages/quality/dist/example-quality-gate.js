...
export class ExampleQualityGate {
    ...
    async evaluate(content) {
        ...
        const tooShortExamples = examples.filter((e) => e.wordCount < this.config.minExampleLength);
    ...
    classifyExampleType(content, _fullText) {
        ...
        if (/\b(company|business|customer|user|market|industry|organization|team)\b/i.test(content)) {
            return 'realWorld';
        }
        // Scenario detection
    ...
    assessExampleQuality(content) {
        ...
        const hasExplanation = /\b(because|therefore|this means|as a result|shows|demonstrates|illustrates)\b/i.test(content);
    ...
    isProgrammingContent(content) {
        ...
        return programmingKeywords.some((keyword) => text.includes(keyword) || topic.includes(keyword));
    }
    /**
    ...
    checkExampleVariety(examples) {
        ...
        if (types.size === 1 && examples.length >= 3) {
            const missingTypes = ['code', 'realWorld', 'scenario', 'conceptual'].filter((t) => !types.has(t));
            return {
                hasVariety: false,
    ...
    checkExamplePlacement(text, examples) {
        ...
        const examplesInLastQuarter = examples.filter((e) => e.startIndex > lastQuarterStart);
    ...
    deduplicateExamples(examples) {
        const unique = [];
        for (const example of examples) {
            const isDuplicate = unique.some((u) => Math.abs(u.startIndex - example.startIndex) < 50 ||
 ...
 */
export function createExampleQualityGate(config) {
    return new ExampleQualityGate(config);
}
//# sourceMappingURL=example-quality-gate.js.map