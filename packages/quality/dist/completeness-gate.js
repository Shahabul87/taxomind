...
export class CompletenessGate {
    ...
    defaultWeight = 1.5; // Higher weight - completeness is critical
    applicableTypes = [
        'lesson',
        'explanation',
        'tutorial',
    ...
    async evaluate(content) {
        ...
        if (content.context?.learningObjectives && content.context.learningObjectives.length > 0) {
            const coverageResult = this.checkObjectiveCoverage(text, content.context.learningObjectives);
            if (coverageResult.coverage < this.config.objectiveCoverageThreshold) {
                ...
                suggestions.push(`Add content covering: ${coverageResult.missing.slice(0, 3).join(', ')}`);
            }
        }
    ...
    findMissingSections(text, requiredSections) {
        ...
        for (const section of requiredSections) {
            ...
            const hasSection = textLower.includes(`# ${sectionLower}`) ||
 ...
 */
export function createCompletenessGate(config) {
    return new CompletenessGate(config);
}
//# sourceMappingURL=completeness-gate.js.map