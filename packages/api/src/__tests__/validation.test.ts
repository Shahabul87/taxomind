...
function createMockContext(): SAMHandlerContext {
  return {
...
describe('createValidationMiddleware', () => {
  ...
  it('should pass valid request body to handler', async () => {
    ...
    expect(handler).toHaveBeenCalledWith(
      expect.objectContaining({ body: { name: 'John', age: 25 } }),
      context
    );
  });
  ...
  it('should include validation error details', async () => {
    ...
    const body = response.body as { error: { details: { errors: Array<{ path: string }> } } };
...
describe('validateQuery', () => {
  ...
  it('should return 400 for invalid query parameters', async () => {
    ...
    expect(response.status).toBe(400);
    expect((response.body as { error: { message: string } }).error.message).toContain('query');
  });
...
describe('composeValidation', () => {
  it('should compose multiple validators', async () => {
    const validator1 = vi.fn((handler: SAMHandler) => handler);
    const validator2 = vi.fn((handler: SAMHandler) => handler);
  ...
  it('should execute validators in correct order (right to left)', async () => {
    ...
    const validator1 = (handler: SAMHandler): SAMHandler => {
      order.push(1);
      return handler;
    };

    const validator2 = (handler: SAMHandler): SAMHandler => {
      order.push(2);
      return handler;
    };
...
describe('analyzeRequestSchema', () => {
  ...
  it('should accept all analysis types', () => {
    const types = ['blooms', 'content', 'assessment', 'full'] as const;
...
describe('gamificationRequestSchema', () => {
  ...
  it('should accept all action types', () => {
    const actions = ['get', 'update', 'award-badge', 'update-streak'] as const;
...
describe('profileRequestSchema', () => {
  ...
  it('should accept all action types', () => {
    const actions = ['get', 'update', 'get-learning-style', 'get-progress'] as const;
