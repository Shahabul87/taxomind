...
import { createAuthMiddleware, createTokenAuthenticator, composeAuthMiddleware, requireRoles, } from '../middleware/auth';
// ============================================================================
// TEST FIXTURES
// ============================================================================
...
describe('requireRoles', () => {
    ...
    it('should include required roles in error message', async () => {
        ...
        const context = createMockContext({ user: { id: 'user-1', role: 'user' } });
        const response = await wrappedHandler(request, context);
        expect(response.body.error.message).toContain('admin or moderator');
    });
});
