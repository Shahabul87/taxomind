...
__export(index_exports, {
  VERSION: () => VERSION,
  analyzeBloomsLevel: () => analyzeBloomsLevel,
  analyzeRequestSchema: () => analyzeRequestSchema,
  chatRequestSchema: () => chatRequestSchema,
  composeAuthMiddleware: () => composeAuthMiddleware,
  composeValidation: () => composeValidation,
  createAnalyzeHandler: () => createAnalyzeHandler,
  createAuthMiddleware: () => createAuthMiddleware,
  createChatHandler: () => createChatHandler,
  createErrorResponse: () => createErrorResponse5,
  createGamificationHandler: () => createGamificationHandler,
  createProfileHandler: () => createProfileHandler,
  createRateLimiter: () => createRateLimiter,
  createRouteHandlerFactory: () => createRouteHandlerFactory,
  createStreamingChatHandler: () => createStreamingChatHandler,
  createSuccessResponse: () => createSuccessResponse5,
  createTokenAuthenticator: () => createTokenAuthenticator,
  createValidationMiddleware: () => createValidationMiddleware,
  gamificationRequestSchema: () => gamificationRequestSchema,
  generateRequestId: () => generateRequestId,
  profileRequestSchema: () => profileRequestSchema,
...
function createChatHandler(config) {
  ...
  return async (request, handlerContext) => {
    ...
    try {
      ...
      const baseContext = (0, import_core.createDefaultContext)(body.context);
      const samContext = {
        ...baseContext,
...
function createAnalyzeHandler(config) {
  ...
  return async (request, handlerContext) => {
    ...
    try {
      ...
      const samContext = (0, import_core2.createDefaultContext)({
        user: userContext,
        page: {
          type: "other",
          path: "/",
...
var chatRequestSchema = import_zod.z.object({
  message: import_zod.z.string().min(1, "Message is required").max(1e4, "Message too long"),
  ...
  history: import_zod.z.array(
    import_zod.z.object({
      ...
      timestamp: import_zod.z.string().or(import_zod.z.date()).optional()
    })
  ).optional(),
  stream: import_zod.z.boolean().optional()
});
var analyzeRequestSchema = import_zod.z.object({
  content: import_zod.z.string().optional(),
  context: import_zod.z.record(import_zod.z.unknown()).optional(),
  type: import_zod.z.enum(["blooms", "content", "assessment", "full"]).optional(),
  options: import_zod.z.object({
    includeRecommendations: import_zod.z.boolean().optional(),
    targetBloomsLevel: import_zod.z.string().optional()
  }).optional()
});
var gamificationRequestSchema = import_zod.z.object({
  userId: import_zod.z.string().min(1, "User ID is required"),
  action: import_zod.z.enum(["get", "update", "award-badge", "update-streak"]),
  payload: import_zod.z.object({
    badgeId: import_zod.z.string().optional(),
    points: import_zod.z.number().optional(),
    activity: import_zod.z.string().optional()
  }).optional()
});
var profileRequestSchema = import_zod.z.object({
  userId: import_zod.z.string().min(1, "User ID is required"),
  action: import_zod.z.enum(["get", "update", "get-learning-style", "get-progress"]),
  payload: import_zod.z.object({
    preferences: import_zod.z.record(import_zod.z.unknown()).optional(),
    learningStyle: import_zod.z.string().optional()
  }).optional()
});
