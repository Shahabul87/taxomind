 ...
 */
const BLOOMS_COGNITIVE_REQUIREMENTS = {
    REMEMBER: 20,
    UNDERSTAND: 35,
    APPLY: 50,
    ANALYZE: 65,
...
export class CognitiveLoadAnalyzer {
    ...
    analyze(content, targetBloomsLevel) {
        ...
        const recommendations = this.generateRecommendations(intrinsic, extraneous, germane, balance);
        // Assess Bloom's compatibility
        const bloomsCompatibility = this.assessBloomsCompatibility(totalLoad, targetBloomsLevel);
        return {
            totalLoad,
            loadCategory,
            measurements: {
    ...
    measureGermaneLoad(text) {
        ...
        const exampleCount = exampleIndicators.reduce((count, indicator) => count + (text.match(new RegExp(indicator, 'gi')) || []).length, 0);
    ...
    calculateTotalLoad(intrinsic, extraneous, germane) {
        ...
        const weightedLoad = (intrinsic.score * 0.35 +
    ...
    assessBalance(intrinsic, extraneous, germane) {
        ...
        if (extraneousMinimized && germaneMaximized && intrinsicAppropriate) {
            ...
            status = 'suboptimal';
            score = 60;
            if (extraneousMinimized)
                score += 10;
            if (germaneMaximized)
                score += 10;
            if (intrinsicAppropriate)
                score += 10;
        }
        return {
            status,
    ...
    assessBloomsCompatibility(totalLoad, targetLevel) {
        ...
        if (targetLevel && !supportsTargetLevel) {
            ...
            adjustments.push(`Reduce cognitive load by ${deficit}% to support ${targetLevel} level activities`);
            adjustments.push(`Consider simplifying content or reducing extraneous load`);
        }
        return {
 ...
 */
export function createCognitiveLoadAnalyzer() {
    return new CognitiveLoadAnalyzer();
}
//# sourceMappingURL=cognitive-load-analyzer.js.map