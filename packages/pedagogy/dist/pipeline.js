...
import { createBloomsAligner, } from './blooms-aligner';
import { createScaffoldingEvaluator, } from './scaffolding-evaluator';
import { createZPDEvaluator, } from './zpd-evaluator';
/**
 * Pedagogical Evaluation Pipeline
...
export class PedagogicalPipeline {
    ...
    constructor(config = {}) {
        this.config = {
            ...
            requireStudentProfile: config.requireStudentProfile ??
                DEFAULT_PEDAGOGICAL_PIPELINE_CONFIG.requireStudentProfile,
        };
        this.bloomsAligner = createBloomsAligner(config.bloomsConfig);
        this.scaffoldingEvaluator = createScaffoldingEvaluator(config.scaffoldingConfig);
    ...
    async evaluate(content, studentProfile) {
        ...
        if (this.config.requireStudentProfile && !studentProfile) {
            ...
            return this.createErrorResult('Student profile is required for ZPD evaluation', startTime);
        }
        // Run evaluators
        ...
        // Aggregate results
        return this.aggregateResults(evaluatorResults, evaluatorsRun, startTime, studentProfile !== undefined);
    }
    /**
    ...
    async runParallel(content, studentProfile, evaluatorResults, evaluatorsRun) {
        ...
        if (this.config.evaluators.includes('blooms')) {
            promises.push(this.runWithTimeout(async () => {
                evaluatorResults.blooms = await this.bloomsAligner.evaluate(content);
        ...
        if (this.config.evaluators.includes('zpd')) {
            promises.push(this.runWithTimeout(async () => {
                evaluatorResults.zpd = await this.zpdEvaluator.evaluate(content, studentProfile);
    ...
    aggregateResults(evaluatorResults, evaluatorsRun, startTime, studentProfileUsed) {
        ...
        if (evaluatorResults.blooms) {
            evaluatorResults.blooms.recommendations.forEach((r) => allRecommendations.add(r));
        }
        if (evaluatorResults.scaffolding) {
            evaluatorResults.scaffolding.recommendations.forEach((r) => allRecommendations.add(r));
        }
        if (evaluatorResults.zpd) {
            evaluatorResults.zpd.recommendations.forEach((r) => allRecommendations.add(r));
        }
        // Calculate overall score
        const scores = [];
        if (evaluatorResults.blooms)
            scores.push(evaluatorResults.blooms.score);
        if (evaluatorResults.scaffolding)
            scores.push(evaluatorResults.scaffolding.score);
        if (evaluatorResults.zpd)
            scores.push(evaluatorResults.zpd.score);
        const overallScore = scores.length > 0
            ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length)
            : 0;
        // Determine overall pass/fail
        const passed = overallScore >= this.config.threshold &&
 ...
 */
export function createPedagogicalPipeline(config) {
    return new PedagogicalPipeline(config);
}
/**
 * Create a Bloom's-only pipeline
 */
export function createBloomsPipeline(config) {
 ...
 */
export function createScaffoldingPipeline(config) {
    return new PedagogicalPipeline({
 ...
 */
export function createZPDPipeline(config) {
    return new PedagogicalPipeline({
        evaluators: ['zpd'],
        zpdConfig: config,
 ...
 */
export function createStrictPedagogicalPipeline() {
    return new PedagogicalPipeline({
        threshold: 80,
        requireStudentProfile: true,
        bloomsConfig: {
 ...
 */
export async function evaluatePedagogically(content, studentProfile, config) {
