...
import { createCognitiveLoadAnalyzer, } from './cognitive-load-analyzer';
// ============================================================================
// ZPD CONSTANTS
// ============================================================================
/**
...
export class ZPDEvaluator {
    ...
    async evaluate(content, studentProfile) {
        ...
        if (this.config.includeCognitiveLoad) {
            cognitiveLoadResult = this.cognitiveLoadAnalyzer.analyze(content.content, content.targetBloomsLevel);
        }
        ...
        const adjustedChallengeScore = cognitiveLoadResult
            ? this.adjustChallengeForCognitiveLoad(challengeLevel.score, cognitiveLoadResult)
        ...
        const engagementPrediction = this.predictEngagement(zpdZone, challengeLevel, supportAdequacy, cognitiveLoadResult);
        ...
        const score = this.calculateScore(challengeLevel, supportAdequacy, personalizationFit, zpdZone, cognitiveLoadResult);
        // Analyze issues and recommendations
        const { issues, recommendations } = this.analyzeIssuesAndRecommendations(zpdZone, challengeLevel, supportAdequacy, engagementPrediction, personalizationFit, studentProfile, cognitiveLoadResult);
    ...
    calculateScore(challengeLevel, supportAdequacy, personalizationFit, zpdZone, cognitiveLoad) {
        ...
        if (cognitiveLoad) {
            ...
            cognitiveLoadScore = Math.max(0, Math.min(100, 100 - extraneousPenalty + germaneBonus + balanceBonus));
        }
    ...
    analyzeIssuesAndRecommendations(zpdZone, challengeLevel, supportAdequacy, engagementPrediction, personalizationFit, studentProfile, cognitiveLoadResult) {
        ...
        if (cognitiveLoadResult) {
            ...
            if (cognitiveLoadResult.loadCategory === 'overload') {
                ...
                recommendations.push('Break content into smaller, focused sections', 'Remove decorative or non-essential elements', 'Use progressive disclosure for complex topics');
            }
            ...
            if (cognitiveLoadResult.measurements.extraneous.score > 60) {
                ...
                recommendations.push('Simplify visual presentation and reduce clutter', 'Use consistent formatting and clear structure', 'Eliminate redundant or confusing navigation');
            }
            ...
            if (cognitiveLoadResult.measurements.germane.score < 30) {
                ...
                recommendations.push('Add more practice exercises with feedback', 'Include comparisons to prior knowledge', 'Add self-explanation prompts');
            }
            // High intrinsic load without adequate support
            if (cognitiveLoadResult.measurements.intrinsic.score > 60 &&
                ...
                recommendations.push('Add step-by-step worked examples', 'Break complex procedures into sub-steps', 'Consider completion problems (partially solved examples)');
            }
