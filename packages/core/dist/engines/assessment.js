...
// ASSESSMENT ENGINE
// ============================================================================
export class AssessmentEngine extends BaseEngine {
    defaultConfig = {
        questionCount: 10,
        duration: 30,
        bloomsDistribution: {
...
`;
    ...
    parseQuestionsResponse(response, config) {
        try {
            ...
            if (jsonMatch) {
                ...
                return parsed.map((q, index) => this.normalizeQuestion(q, index));
            }
        }
        catch (error) {
    ...
    analyzeAssessment(questions, config) {
        ...
        // Difficulty analysis
        const avgDifficulty = (difficultyCount.easy * 1 + difficultyCount.medium * 2 + difficultyCount.hard * 3) /
            (total || 1);
        ...
        const isBalanced = Math.abs(targetEasy - actualEasy) < 0.15 &&
