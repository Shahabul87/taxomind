...
function computeContextDiff(previous, current) {
    ...
    if (previous.content.textSummary !== current.content.textSummary) {
        ...
        if (current.content.textSummary)
            newContent.push(current.content.textSummary);
        if (previous.content.textSummary)
            removedContent.push(previous.content.textSummary);
    }
    // Compare navigation
    ...
    // Compare interaction
    if (Math.abs(previous.interaction.scrollPosition - current.interaction.scrollPosition) > 10 ||
...
export class ContextMemoryHydrator {
    ...
    async hydrate(userId, snapshot, directives) {
        ...
        if (directives.shouldIngestContent && this.vectorStore && directives.contentForIngestion.length > 0) {
            try {
                vectorsQueued = await this.vectorStore.ingest(userId, directives.contentForIngestion, {
        ...
        if (directives.shouldUpdateKnowledgeGraph &&
            this.knowledgeGraph &&
            directives.entitiesForGraph.length > 0) {
            try {
                graphEntitiesAdded = await this.knowledgeGraph.addEntities(userId, directives.entitiesForGraph);
...
// ============================================================================
export class InMemoryContextMemoryAdapter {
    snapshots = new Map();
    idCounter = 0;
...
// FACTORY
// ============================================================================
export function createContextMemoryHydrator(options) {
    return new ContextMemoryHydrator(options);
}
export function createInMemoryContextMemoryAdapter() {
