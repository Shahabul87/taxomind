...
import type { ContextEngineOutput } from './context';
...
export interface CognitiveLoadProfile {
  ...
  capacity: number; // 0-100, how much more they can handle
  factors: {
...
export class PersonalizationEngine extends BaseEngine<PersonalizationEngineOutput> {
  ...
  protected async process(input: EngineInput): Promise<PersonalizationEngineOutput> {
    const { context, previousResults, options } = input;

    // Get context engine data
    const contextResult = previousResults?.['context'] as
      | { success: boolean; data: ContextEngineOutput }
      | undefined;
  ...
  private async analyzeLearningStyle(
    ...
    _contextData?: ContextEngineOutput
  ...
  private detectLearningStyleIndicators(
    messages: EngineInput['context']['conversation']['messages']
  ): LearningStyleProfile['indicators'] {
    const styles: SAMLearningStyle[] = [
      'visual',
      'auditory',
      'kinesthetic',
      'reading-writing',
    ...
    const indicators: LearningStyleProfile['indicators'] = [];
  ...
  private generateStyleRecommendations(
    ...
    const styleRecs: Record<SAMLearningStyle, string[]> = {
      visual: [
  ...
  private detectEmotionalState(
    messages: EngineInput['context']['conversation']['messages']
  ): { state: EmotionalState; confidence: number; triggers: string[] } {
    ...
    for (const msg of messages) {
      if (msg.metadata?.emotion) {
        state = msg.metadata.emotion as EmotionalState;
        confidence = 85;
  ...
  private getRecommendedTone(state: EmotionalState): SAMTone {
    const toneMap: Record<EmotionalState, SAMTone> = {
  ...
  private analyzeCognitiveLoad(context: EngineInput['context']): CognitiveLoadProfile {
    const messages = context.conversation.messages;
    ...
    let currentLoad: CognitiveLoad = 'optimal';
    let capacity = 60;
  ...
  private generateCognitiveAdaptations(
    load: CognitiveLoad,
    factors: CognitiveLoadProfile['factors']
  ): ContentAdaptation[] {
  ...
  private analyzeMotivation(
    ...
    const hasInternalIndicators =
      emotional.currentState === 'curious' || messages.some((m) => m.content.includes('want to learn'));

    let type: 'intrinsic' | 'extrinsic' | 'mixed' = 'mixed';
  ...
  private async generateLearningPath(
    ...
    const baseNodes: LearningPathNode[] = [
      {
        id: 'review-1',
        title: 'Quick Review',
        type: 'review',
    ...
    return {
      ...
      adaptationLevel:
        cognitiveLoad.currentLoad === 'high' ? 'significant' :
  ...
  private buildOverallProfile(
    ...
    cognitiveLoad: CognitiveLoadProfile,
    motivation: MotivationProfile
  ): PersonalizationEngineOutput['overallProfile'] {
...
export function createPersonalizationEngine(config: SAMConfig): PersonalizationEngine {
