...
const PAGE_TYPE_INTENTS: Record<string, string> = {
...
const PAGE_TYPE_ACTIONS: Record<string, string[]> = {
...
export class ContextGatheringEngine extends BaseEngine<ContextGatheringInput, ContextGatheringOutput> {
  ...
  protected async process(
    input: EngineInput & ContextGatheringInput,
  ...
  private enrichSnapshot(
    ...
    enrichmentData?: ContextGatheringInput['enrichmentData'],
  ): PageContextSnapshot {
  ...
  private buildFormSummary(forms: FormSnapshot[]): string {
    if (forms.length === 0) return 'No forms on this page.';

    const parts: string[] = [`${forms.length} form(s) on page:`];
  ...
  private buildContentSummary(content: ContentSnapshot): string {
  ...
  private buildNavigationSummary(navigation: NavigationSnapshot): string {
    const parts: string[] = [];
  ...
  private determineAvailableActions(snapshot: PageContextSnapshot): string[] {
  ...
  private produceMemoryDirectives(
    ...
    const contentForIngestion: string[] = [];
    const entitiesForGraph: MemoryDirectives['entitiesForGraph'] = [];
    const sessionContextUpdates: Record<string, unknown> = {};
    ...
    if (enrichmentData?.entityContext) {
      ...
      entitiesForGraph.push({
        ...
        relationships: enrichmentData.relatedEntities
          ? enrichmentData.relatedEntities.map(
...
export function createContextGatheringEngine(config: SAMConfig): ContextGatheringEngine {
