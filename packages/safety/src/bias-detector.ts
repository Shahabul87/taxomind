...
const GENDER_BIAS_PATTERNS: BiasPattern[] = [
  ...
  {
    pattern:
      /\b(girls|women|females)\s+(aren'?t|don'?t)\s+(as\s+)?(good|capable)\b/gi,
    category: 'gender',
    confidence: 0.9,
...
const DISABILITY_BIAS_PATTERNS: BiasPattern[] = [
  {
    pattern:
      /\b(despite|considering)\s+your\s+(condition|disability|challenges)\b/gi,
    category: 'disability',
    confidence: 0.8,
...
const SOCIOECONOMIC_BIAS_PATTERNS: BiasPattern[] = [
  {
    pattern:
      /\bfor\s+(someone\s+from|your)\s+(background|neighborhood|family)\b/gi,
...
const NEURODIVERSITY_BIAS_PATTERNS: BiasPattern[] = [
  ...
  {
    pattern:
      /\byou\s+(just\s+)?need\s+to\s+(focus|try\s+harder|pay\s+attention)\b/gi,
    category: 'neurodiversity',
    confidence: 0.6,
...
const EDUCATIONAL_BIAS_PATTERNS: BiasPattern[] = [
  {
    pattern:
      /\b(homeschooled|public\s+school|private\s+school)\s+(students|kids)\s+(always|never)\b/gi,
