...
const FIXED_MINDSET_PATTERNS = [
  {
    pattern:
      /\byou'?re\s+(not\s+)?(a\s+)?(math|science|writing|reading)\s+person\b/gi,
    suggestion: 'Focus on specific skills that can be developed',
  },
  {
    pattern:
      /\bsome\s+(people|students)\s+(just\s+)?(can'?t|aren'?t\s+able)\b/gi,
    suggestion:
      'All students can improve with appropriate support and practice',
  },
  {
...
export class ConstructiveFramingChecker {
  ...
  check(feedback: EvaluationFeedback): ConstructiveFramingResult {
    ...
    if (positiveElements.length < this.config.minPositiveElements) {
      issues.push({
        ...
        text: '',
        suggestion:
          'Start with something the student did well before addressing areas for improvement',
      });
    }
    ...
    if (this.config.requireActionableSuggestions) {
      ...
      if (
        ...
        issues.push({
          type: 'no_next_steps',
          description:
            'Improvements identified but no actionable suggestions provided',
          text: feedback.improvements.join('; '),
          suggestion:
            'Include specific, actionable steps the student can take to improve',
        });
      }
    }
    ...
    if (feedback.score / feedback.maxScore < 0.6) {
      ...
      if (!hasEncouragement) {
        issues.push({
          ...
          text: '',
          suggestion:
            'Add supportive language that acknowledges effort and potential for growth',
        });
      }
    }
    ...
    for (const patternDef of FIXED_MINDSET_PATTERNS) {
      ...
      while ((match = patternDef.pattern.exec(text)) !== null) {
        issues.push({
          ...
          description:
            'Uses fixed mindset language that may limit student belief in growth',
          text: match[0],
  ...
  private checkCriticismBalance(
    ...
    const strengthCount =
      (feedback.strengths?.length ?? 0) + positiveElements.length;
    ...
    if (improvementCount > 3 && strengthCount === 0) {
      return {
        ...
        description:
          'Feedback focuses heavily on criticisms without acknowledging strengths',
        text: `${improvementCount} improvements vs ${strengthCount} positives`,
        suggestion:
          'Balance criticism with recognition of what the student did well',
      };
    }
    ...
    if (improvementCount > strengthCount * 3 && strengthCount > 0) {
      return {
        ...
        text: `${improvementCount} improvements vs ${strengthCount} positives`,
        suggestion:
          'Consider the feedback sandwich approach: positive-constructive-positive',
      };
    }
...
export function createStrictConstructiveChecker(
  config?: Omit<
    ConstructiveFramingCheckerConfig,
    'minPositiveElements' | 'minConstructivenessScore'
  >
): ConstructiveFramingChecker {
...
export function createLenientConstructiveChecker(
  config?: Omit<
    ConstructiveFramingCheckerConfig,
    'minPositiveElements' | 'requireActionableSuggestions'
  >
): ConstructiveFramingChecker {
  return new ConstructiveFramingChecker({
    ...config,
