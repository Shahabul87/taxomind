 ...
 */
const ABSOLUTE_NEGATIVE_PATTERNS = [
    {
    ...
    deduplicateMatches(matches) {
        ...
        const sorted = [...matches].sort((a, b) => {
            ...
            return (b.position.end -
                b.position.start -
                (a.position.end - a.position.start));
        });
        const result = [];
        let lastEnd = -1;
 ...
 */
export function createDiscouragingLanguageDetector(config) {
    return new DiscouragingLanguageDetector(config);
}
/**
 * Create strict detector (reports all severities)
 */
export function createStrictDiscouragingDetector(config) {
 ...
 */
export function createLenientDiscouragingDetector(config) {
    return new DiscouragingLanguageDetector({
        ...config,
        minSeverity: 'high',
    });
}
//# sourceMappingURL=discouraging-language-detector.js.map