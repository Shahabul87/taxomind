...
export class AccessibilityChecker {
    ...
    check(text, targetAudience) {
        ...
        const statistics = this.calculateStatistics(words, sentences, totalSyllables, complexWordCount, text);
        // Calculate readability scores
        const gradeLevel = calculateFleschKincaidGradeLevel(words.length, sentences.length, totalSyllables);
        const readabilityScore = calculateFleschReadingEase(words.length, sentences.length, totalSyllables);
        // Identify issues
        const issues = this.identifyIssues(statistics, gradeLevel, targetGradeLevel, sentences, text);
        // Determine pass/fail
    ...
    calculateStatistics(words, sentences, syllableCount, complexWordCount, text) {
        ...
        return {
            ...
            averageSentenceLength: sentenceCount > 0
                ...
                : 0,
            averageWordSyllables: wordCount > 0
    ...
    identifyIssues(statistics, gradeLevel, targetGradeLevel, sentences, text) {
        ...
        const longSentences = sentences.filter((s) => extractWords(s).length > this.config.maxSentenceLength);
        ...
        const denseParagraphs = paragraphs.filter((p) => extractWords(p).length > 100);
    ...
    getSuggestions(result) {
        ...
        if (result.gradeLevel > this.config.targetGradeLevel) {
            ...
            suggestions.push(`Aim to reduce reading level by ${difference.toFixed(1)} grades for target audience.`);
        }
        return suggestions;
    }
}
 ...
 */
export function createAccessibilityChecker(config) {
    return new AccessibilityChecker(config);
}
/**
 * Create accessibility checker for elementary level
 */
export function createElementaryAccessibilityChecker(config) {
 ...
 */
export function createHighSchoolAccessibilityChecker(config) {
    return new AccessibilityChecker({
        ...config,
        targetGradeLevel: 10,
        maxGradeLevel: 14,
 ...
 */
export function createCollegeAccessibilityChecker(config) {
    return new AccessibilityChecker({
        ...config,
        targetGradeLevel: 12,
        maxGradeLevel: 16,
