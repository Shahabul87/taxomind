...
import { AccessibilityChecker, createAccessibilityChecker, createElementaryAccessibilityChecker, createHighSchoolAccessibilityChecker, createCollegeAccessibilityChecker, DEFAULT_ACCESSIBILITY_CONFIG, } from '../accessibility-checker';
// ============================================================================
// SAMPLE DATA
// ============================================================================
...
describe('AccessibilityChecker', () => {
    ...
    describe('check - complex text', () => {
        ...
        it('should detect jargon in complex text', () => {
            const result = checker.check(COMPLEX_TEXT);
            const jargonIssues = result.issues.filter(i => i.type === 'jargon_without_explanation');
            expect(jargonIssues.length).toBeGreaterThan(0);
        });
        it('should detect reading level too high', () => {
            const result = checker.check(COMPLEX_TEXT);
            const readingLevelIssues = result.issues.filter(i => i.type === 'reading_level_too_high');
    ...
    describe('check - long sentences', () => {
        it('should detect sentences that are too long', () => {
            ...
            const longSentenceIssues = result.issues.filter(i => i.type === 'sentence_too_long');
            expect(longSentenceIssues.length).toBeGreaterThan(0);
        });
        it('should suggest breaking up long sentences', () => {
            const result = checker.check(LONG_SENTENCE_TEXT);
            const longSentenceIssue = result.issues.find(i => i.type === 'sentence_too_long');
    ...
    describe('check - passive voice', () => {
        it('should detect passive voice overuse', () => {
            ...
            const passiveIssues = result.issues.filter(i => i.type === 'passive_voice_overuse');
    ...
    describe('check - jargon', () => {
        ...
        it('should list detected jargon terms', () => {
            const result = checker.check(JARGON_TEXT);
            const jargonIssue = result.issues.find(i => i.type === 'jargon_without_explanation');
    ...
    describe('check - dense paragraphs', () => {
        it('should detect dense paragraphs', () => {
            ...
            const denseParagraphIssues = result.issues.filter(i => i.type === 'dense_paragraphs');
    ...
    describe('issue severity', () => {
        ...
        it('should fail only for high/critical issues', () => {
            ...
            const result = checker.check(BALANCED_TEXT);
            if (!result.passed) {
                const hasHighCritical = result.issues.some(i => i.severity === 'high' || i.severity === 'critical');
...
//# sourceMappingURL=accessibility-checker.test.js.map