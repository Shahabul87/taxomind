...
import { DiscouragingLanguageDetector, } from './discouraging-language-detector';
import { BiasDetector } from './bias-detector';
import { AccessibilityChecker, } from './accessibility-checker';
import { ConstructiveFramingChecker, } from './constructive-framing-checker';
// ============================================================================
...
export class FairnessSafetyValidator {
    ...
    async validateFeedback(feedback) {
        ...
        if (this.config.checkAccessibility) {
            const accessibilityResult = this.accessibilityChecker.check(fullText, feedback.targetGradeLevel);
    ...
    suggestImprovements(feedback) {
        ...
        const discouragingResult = this.discouragingDetector.detect(fullText);
        if (discouragingResult.found) {
            const alternatives = this.discouragingDetector.suggestAlternatives(discouragingResult.matches);
            Array.from(alternatives.entries()).forEach(([phrase, alternative]) => {
                suggestions.push(`Replace "${phrase}" with: ${alternative}`);
            });
        }
        const biasResult = this.biasDetector.detect(fullText);
        const biasSuggestions = this.biasDetector.getSuggestions(biasResult.indicators);
        ...
        suggestions.push(...this.accessibilityChecker.getSuggestions(accessibilityResult));
        const constructiveResult = this.constructiveChecker.check(feedback);
        suggestions.push(...this.constructiveChecker.getSuggestions(constructiveResult));
    ...
    rewriteFeedback(feedback) {
        ...
        const rewrittenText = this.discouragingDetector.rewriteWithAlternatives(feedback.text, discouragingResult.matches);
        return {
            ...feedback,
            text: rewrittenText,
        };
    ...
    getDetailedAnalysis(feedback) {
        ...
        return {
            ...
            accessibility: this.accessibilityChecker.check(fullText, feedback.targetGradeLevel),
 ...
 */
export function createFairnessSafetyValidator(config) {
    return new FairnessSafetyValidator(config);
}
/**
 * Create strict validator (all checks enabled, low tolerance)
 */
export function createStrictFairnessValidator(config) {
 ...
 */
export function createLenientFairnessValidator(config) {
    return new FairnessSafetyValidator({
        ...config,
        minPassingScore: 50,
        checkDiscouragingLanguage: true,
 ...
 */
export function createQuickFairnessValidator(config) {
    return new FairnessSafetyValidator({
        ...config,
        minPassingScore: 60,
        checkDiscouragingLanguage: true,
 ...
 */
export function getDefaultFairnessValidator() {
    if (!defaultValidator) {
 ...
 */
export function resetDefaultFairnessValidator() {
    defaultValidator = undefined;
}
//# sourceMappingURL=fairness-validator.js.map