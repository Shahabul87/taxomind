...
const mockCheckIn = {
    ...
    questions: [
        { id: 'q1', question: 'Rate your progress', type: 'scale', required: true },
    ],
...
describe('useAgentic', () => {
    ...
    describe('Initialization', () => {
        ...
        it('should auto-fetch goals when autoFetchGoals is true', async () => {
            ...
            await waitFor(() => {
                expect(fetchMock).toHaveBeenCalledWith('/api/sam/agentic/goals', expect.any(Object));
            });
        });
        it('should auto-fetch recommendations when autoFetchRecommendations is true', async () => {
            ...
            await waitFor(() => {
                expect(fetchMock).toHaveBeenCalledWith('/api/sam/agentic/recommendations?time=45', expect.any(Object));
            });
        });
        it('should auto-fetch pending check-ins when autoFetchCheckIns is true', async () => {
            renderHook(() => useAgentic({ autoFetchCheckIns: true }));
            await waitFor(() => {
                expect(fetchMock).toHaveBeenCalledWith('/api/sam/agentic/checkins?status=pending', expect.any(Object));
            });
        });
    });
    // --------------------------------------------------------------------------
    ...
    describe('Goals', () => {
        ...
        it('should fetch goals with status filter', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith('/api/sam/agentic/goals?status=active', expect.any(Object));
        });
    ...
    describe('Plans', () => {
        ...
        it('should start, pause, and resume a plan', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith('/api/sam/agentic/plans/plan_1/start', expect.objectContaining({ method: 'POST' }));
            await act(async () => {
                await result.current.pausePlan('plan_1');
            });
            expect(fetchMock).toHaveBeenCalledWith('/api/sam/agentic/plans/plan_1/pause', expect.objectContaining({ method: 'POST' }));
            await act(async () => {
                await result.current.resumePlan('plan_1');
            });
            expect(fetchMock).toHaveBeenCalledWith('/api/sam/agentic/plans/plan_1/resume', expect.objectContaining({ method: 'POST' }));
        });
    });
    // --------------------------------------------------------------------------
