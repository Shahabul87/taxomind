...
const DEFAULT_ROUTE_PATTERNS: Array<{
  pattern: RegExp;
  type: SAMPageType;
  extract?: (match: RegExpMatchArray) => Partial<PageContextDetection>;
}> = [
  // Teacher routes
  {
...
const DEFAULT_CAPABILITIES: Partial<Record<SAMPageType, string[]>> = {
 ...
 */
export function createContextDetector(options?: ContextDetectorOptions) {
   ...
   */
  function detectFromPath(path: string): PageContextDetection {
   ...
   */
  function detectFromDOM(): Partial<PageContextDetection> {
    if (typeof document === 'undefined') return {};

    const detection: Partial<PageContextDetection> = {};
    ...
    if (pageTypeElement) {
      detection.type = pageTypeElement.getAttribute('data-page-type') as SAMPageType;
    }
    ...
    if (metaPageType) {
      detection.type = metaPageType.getAttribute('content') as SAMPageType;
    }
   ...
   */
  function detect(): PageContextDetection {
 ...
 */
export function getCapabilities(pageType: SAMPageType): string[] {
