...
function detectPageContext(): PageSnapshot {
...
function detectPageTypeFromPath(path: string): string {
...
function buildBreadcrumbFromPath(path: string): string[] {
  return path
    .split('/')
    .filter(Boolean)
...
function scanForms(maxForms: number): FormSnapshot[] {
...
function scanSingleForm(form: HTMLFormElement, index: number): FormSnapshot | null {
  ...
  elements.forEach((el, order) => {
    const field = scanFormField(el as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement, order);
    if (field) {
      fields.push(field);

      // Collect validation rules
      const rules = extractValidationRules(el as HTMLInputElement);
      if (rules.length > 0) {
...
function scanFormField(
  ...
  let options: FormFieldSnapshot['options'];
  ...
  return {
    ...
    value,
    label,
    placeholder: 'placeholder' in el ? (el as HTMLInputElement | HTMLTextAreaElement).placeholder || undefined : undefined,
    helpText,
...
function getFieldValue(el: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement): unknown {
  if (el instanceof HTMLInputElement) {
...
function detectValidationState(el: HTMLElement): ValidationState {
...
function extractFieldErrors(el: HTMLElement): string[] {
  const errors: string[] = [];
...
function extractValidationRules(el: HTMLInputElement): ValidationRule[] {
...
function inferFormPurpose(form: HTMLFormElement, fields: FormFieldSnapshot[]): FormPurpose {
...
function extractNavigation(maxLinks: number): NavigationSnapshot {
  ...
  let pagination: NavigationSnapshot['pagination'];
  if (paginationEl) {
...
function categorizeLink(anchor: HTMLAnchorElement, href: string): LinkCategory {
  // Breadcrumb
...
function countParentListDepth(el: HTMLElement): number {
  let depth = 0;
...
function captureInteraction(pageLoadTime: number): InteractionSnapshot {
  ...
  const focusedElement = focusedEl && focusedEl !== document.body
    ? describeElement(focusedEl as HTMLElement)
    : undefined;
...
export function useContextGathering(
  options: UseContextGatheringOptions = {},
): UseContextGatheringReturn {
  const {
    enabled = true,
    debounceMs = 500,
  ...
  const [snapshot, setSnapshot] = useState<PageContextSnapshot | null>(null);
  ...
  const providersRef = useRef<ContextProvider[]>(initialProviders);
  ...
  const gather = useCallback(async () => {
    ...
    try {
      ...
      const hashInput = `${page.path}:${page.type}:${page.title}:${forms.length}:${content.wordCount}:${JSON.stringify(forms.map((f) => f.fields.map((fi: FormFieldSnapshot) => `${fi.name}=${String(fi.value ?? '')}`)))}`;
      ...
      const newSnapshot: PageContextSnapshot = {
  ...
  const registerProvider = useCallback((provider: ContextProvider) => {
