...
export class InMemoryMemoryStore {
    ...
    async getByTopic(studentId, topicId, limit) {
        const entries = Array.from(this.entries.values())
            .filter((e) => e.studentId === studentId && e.relatedTopics.includes(topicId))
            .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
        return limit ? entries.slice(0, limit) : entries;
    }
    /**
...
export class EvaluationMemoryIntegrationImpl {
    ...
    constructor(implConfig) {
        ...
        this.spacedRepetitionScheduler = new SpacedRepetitionScheduler(implConfig.reviewStore, this.config.spacedRepetitionConfig);
    }
    /**
    ...
    async recordEvaluationOutcome(outcome) {
        ...
        try {
            ...
            if (this.config.adjustPathwayOnEvaluation && outcome.courseId) {
                const pathways = await this.profileStore.getActivePathways(outcome.studentId);
                const coursePathway = pathways.find((p) => p.courseId === outcome.courseId);
                if (coursePathway) {
                    const adjustmentResult = await this.pathwayCalculator.calculateAdjustment(outcome.studentId, coursePathway.id, outcome);
    ...
    async getRelevantMemories(studentId, topicId) {
        ...
        const topicMemories = await this.memoryStore.getByTopic(studentId, topicId, 10);
        // Get recent important memories
        const importantMemories = await this.memoryStore.getImportant(studentId, 'high');
    ...
    async recalculatePathway(studentId, pathwayId) {
        const recalculated = await this.pathwayCalculator.recalculatePathway(studentId, pathwayId);
        // Update in store
 ...
 */
export function createEvaluationMemoryIntegration(config) {
 ...
 */
export function resetDefaultMemoryStore() {
    defaultMemoryStore = null;
}
//# sourceMappingURL=evaluation-memory-integration.js.map