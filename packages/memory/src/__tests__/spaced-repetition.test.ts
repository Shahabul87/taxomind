...
import {
  createSampleReviewScheduleEntry,
...
describe('InMemoryReviewScheduleStore', () => {
  let store: InMemoryReviewScheduleStore;
  ...
  describe('updateReview', () => {
    ...
    it('should throw error for non-existent entry', async () => {
      await expect(
        store.updateReview('non-existent', { priority: 'high' })
      ).rejects.toThrow('Review entry not found');
    });
  });
  ...
  describe('completeReview', () => {
    ...
    it('should throw error for non-existent entry', async () => {
      await expect(
        store.completeReview('non-existent', 80)
      ).rejects.toThrow('Review entry not found');
    });
  });
