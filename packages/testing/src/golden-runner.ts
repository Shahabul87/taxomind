...
const defaultLogger: TestingLogger = {
  debug: () => {},
  info: () => {},
  warn: () => {},
  error: () => {},
...
export class GoldenTestRunner {
  private testStore: GoldenTestStore;
  private resultStore: TestResultStore;
  private runStore: TestRunStore;
  private validators: Map<string, ValidationRule>;
  private logger: TestingLogger;
  private parallelism: number;
  private defaultTimeout: number;
  private stopOnFailure: boolean;
  private executors = new Map<TestCaseCategory, TestExecutor>();
   ...
   */
  async getTestCase(id: string): Promise<GoldenTestCase | null> {
  ...
  async runTest(testCase: GoldenTestCase): Promise<TestResult> {
    ...
    try {
      ...
      const status: TestCaseStatus = passed ? 'passed' : 'failed';
  ...
  async runTests(options?: {
    ...
    if (options?.testIds) {
      ...
      )).filter((t): t is GoldenTestCase => t !== null);
    } else {
   ...
   */
  async getTestHistory(testCaseId: string, limit?: number): Promise<TestResult[]> {
...
export function createGoldenTestRunner(config?: GoldenTestRunnerConfig): GoldenTestRunner {
