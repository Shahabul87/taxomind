...
import { TrendDirection, MasteryLevel, TimePeriod, LearningSessionInputSchema, } from './types';
// ============================================================================
// IN-MEMORY STORES
// ============================================================================
/**
 ...
 */
export class InMemoryLearningSessionStore {
    sessions = new Map();
 ...
 */
export class InMemoryTopicProgressStore {
    progress = new Map();
    ...
    async getByMasteryLevel(userId, level) {
        return Array.from(this.progress.values()).filter((p) => p.userId === userId && p.masteryLevel === level);
    }
}
/**
 * In-memory implementation of LearningGapStore
 */
export class InMemoryLearningGapStore {
    gaps = new Map();
    async create(gap) {
        const newGap = {
            ...gap,
    ...
    async getByUser(userId, includeResolved = false) {
        return Array.from(this.gaps.values()).filter((g) => g.userId === userId && (includeResolved || !g.isResolved));
    }
    async resolve(id) {
    ...
    async getBySeverity(userId, severity) {
        return Array.from(this.gaps.values()).filter((g) => g.userId === userId && g.severity === severity && !g.isResolved);
    }
}
// ============================================================================
// DEFAULT LOGGER
...
export class ProgressAnalyzer {
    ...
    async detectGaps(userId) {
        ...
        for (const progress of allProgress) {
            ...
            for (const concept of progress.conceptsInProgress) {
                if (existingGapConcepts.has(concept))
                    continue;
     ...
     */
    async analyzeTrends(userId, period = TimePeriod.WEEKLY) {
        ...
        const trends = [];
        // Mastery trend
        const masteryTrend = this.calculateTrend(sessions, 'mastery', (s) => (s.correctAnswers / Math.max(1, s.questionsAnswered)) * 100, period);
        trends.push(masteryTrend);
        // Time spent trend
        const timeTrend = this.calculateTrend(sessions, 'time_spent', (s) => s.duration, period);
        trends.push(timeTrend);
        // Engagement trend
        const engagementTrend = this.calculateTrend(sessions, 'engagement', (s) => s.focusScore ?? 0.5, period);
     ...
     */
    async generateReport(userId, period = TimePeriod.WEEKLY) {
     ...
     */
    async getSnapshot(userId, period = TimePeriod.WEEKLY) {
        const periodDays = this.getPeriodDays(period);
        const now = new Date();
        const start = new Date(now.getTime() - periodDays * 24 * 60 * 60 * 1000);
        const sessions = await this.sessionStore.getByPeriod(userId, start, now);
    ...
    calculateMasteryScore(accuracy, totalTime, sessionCount) {
        ...
        return Math.round(accuracy * accuracyWeight +
    ...
    determineTrend(recent, older) {
        ...
        const recentVariance = this.calculateVariance(recent.map((s) => s.questionsAnswered > 0 ? (s.correctAnswers / s.questionsAnswered) * 100 : 0));
        if (recentVariance > 400)
            return TrendDirection.FLUCTUATING; // SD > 20%
        return TrendDirection.STABLE;
    }
    ...
    calculateSummary(sessions, progress) {
        ...
        const inProgressTopics = progress.filter((p) => p.completionPercentage > 0 && p.completionPercentage < 100).length;
    ...
    calculateLongestStreak(sessions) {
        if (sessions.length === 0)
            return 0;
        const dates = Array.from(new Set(sessions.map((s) => s.startTime.toISOString().split('T')[0]))).sort();
        let longestStreak = 1;
        let currentStreak = 1;
    ...
    generateRecommendations(summary, gaps, trends) {
        ...
        if (criticalGaps.length > 0) {
            recommendations.push(`Focus on resolving ${criticalGaps.length} critical knowledge gap(s)`);
        }
 ...
 */
export function createProgressAnalyzer(config) {
    return new ProgressAnalyzer(config);
}
//# sourceMappingURL=progress-analyzer.js.map