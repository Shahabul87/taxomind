...
import { ContentType, RecommendationPriority, RecommendationReason, MasteryLevel, RecommendationFeedbackSchema, } from './types';
// ============================================================================
// IN-MEMORY STORES
// ============================================================================
/**
 ...
 */
export class InMemoryRecommendationStore {
...
export class InMemoryContentStore {
    ...
    async search(query, filters) {
        ...
        return Array.from(this.content.values()).filter((c) => {
            // Text search
            const matchesQuery = c.title.toLowerCase().includes(lowerQuery) ||
            ...
            // Apply filters
            if (filters) {
                if (filters.types && !filters.types.includes(c.type))
                    return false;
                if (filters.difficulty && !filters.difficulty.includes(c.difficulty))
                    return false;
                if (filters.minDuration && c.duration < filters.minDuration)
                    return false;
                if (filters.maxDuration && c.duration > filters.maxDuration)
                    return false;
                if (filters.topicIds && !filters.topicIds.includes(c.topicId))
                    return false;
                if (filters.skillIds && !filters.skillIds.some((s) => c.skillIds.includes(s)))
                    return false;
            }
            return true;
        });
...
export class RecommendationEngine {
    ...
    async generateRecommendations(input) {
        ...
        if (input.learningGaps && input.learningGaps.length > 0) {
            const gapRecs = await this.generateGapRecommendations(input.userId, input.learningGaps, expiresAt);
            recommendations.push(...gapRecs);
        }
        // 2. Generate skill decay recommendations
        if (input.skillDecay && input.skillDecay.length > 0) {
            const decayRecs = await this.generateDecayRecommendations(input.userId, input.skillDecay, expiresAt);
        ...
        if (input.skillAssessments && input.skillAssessments.length > 0) {
            const skillRecs = await this.generateSkillRecommendations(input.userId, input.skillAssessments, expiresAt);
            recommendations.push(...skillRecs);
        }
        // 4. Generate exploration recommendations
        const explorationRecs = await this.generateExplorationRecommendations(input.userId, context, expiresAt);
    ...
    async recordFeedback(feedback) {
        ...
        if (validated.completed) {
            await this.recommendationStore.markCompleted(validated.recommendationId, validated.rating);
        }
    }
    /**
    ...
    async generateLearningPath(userId, targetSkillIds, currentAssessments) {
        ...
        for (const skillId of targetSkillIds) {
            ...
            for (const item of orderedContent) {
                ...
                if (this.shouldSkipContent(item, currentLevel))
                    continue;
                steps.push({
    ...
    async generateSkillRecommendations(userId, assessments, expiresAt) {
        ...
        const improvableSkills = assessments.filter((a) => a.level === MasteryLevel.BEGINNER ||
        ...
        for (const assessment of improvableSkills.slice(0, 2)) {
            ...
            const challengeContent = content.find((c) => c.difficulty === 'medium' || c.difficulty === 'hard');
            if (challengeContent) {
                recommendations.push({
                    id: '',
                    userId,
    ...
    async generateExplorationRecommendations(userId, context, expiresAt) {
        ...
        const explorationContent = allContent.filter((c) => !context.recentTopics.includes(c.topicId));
