...
import { CheckInStatus, CheckInType, TriggerType, NotificationChannel, CheckInResponseSchema, QuestionType, ActionType, } from './types';
// ============================================================================
// IN-MEMORY STORE
// ============================================================================
/**
 ...
 */
export class InMemoryCheckInStore {
    checkIns = new Map();
    responses = new Map();
    async get(id) {
    ...
    async getByUser(userId, status) {
        return Array.from(this.checkIns.values()).filter((checkIn) => checkIn.userId === userId && (status === undefined || checkIn.status === status));
    }
    ...
    async getAllScheduled(from, to) {
        return Array.from(this.checkIns.values()).filter((checkIn) => checkIn.scheduledTime >= from &&
            checkIn.scheduledTime <= to);
    }
...
export class TriggerEvaluator {
    ...
    calculateBehindSchedule(context) {
        if (!context.goalProgress || !context.goalDeadline)
            return 0;
        const now = new Date();
        const totalDays = (context.goalDeadline.getTime() - now.getTime()) / (24 * 60 * 60 * 1000);
    ...
    calculateWeeklyReviewDue(context) {
        if (!context.lastSessionAt)
            return 7;
        const now = new Date();
        const daysSinceSession = Math.floor((now.getTime() - context.lastSessionAt.getTime()) / (24 * 60 * 60 * 1000));
...
export class CheckInScheduler {
    ...
    async evaluateTriggers(userId, context) {
        ...
        for (const checkIn of scheduledCheckIns) {
            const evaluatedConditions = this.triggerEvaluator.evaluateAllConditions(checkIn.triggerConditions, context);
            const shouldTrigger = this.triggerEvaluator.shouldTrigger(checkIn.triggerConditions, context);
            if (shouldTrigger) {
    ...
    calculateUrgency(checkIn, conditions) {
        ...
        const hasCriticalCondition = conditions.some((c) => c.met && criticalTypes.includes(c.type));
 ...
 */
export function createCheckInScheduler(config) {
    return new CheckInScheduler(config);
}
//# sourceMappingURL=check-in-scheduler.js.map