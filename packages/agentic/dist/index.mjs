...
var LearningSessionInputSchema = z14.object({
  userId: z14.string().min(1),
  topicId: z14.string().min(1),
  startTime: z14.date().optional(),
  duration: z14.number().min(0).optional(),
  activitiesCompleted: z14.number().min(0).optional(),
  questionsAnswered: z14.number().min(0).optional(),
  correctAnswers: z14.number().min(0).optional(),
  conceptsCovered: z14.array(z14.string()).optional(),
  focusScore: z14.number().min(0).max(1).optional()
});
var SkillAssessmentInputSchema = z14.object({
  userId: z14.string().min(1),
  skillId: z14.string().min(1),
  skillName: z14.string().min(1).optional(),
  score: z14.number().min(0).max(100),
  maxScore: z14.number().min(1).optional().default(100),
  source: z14.nativeEnum(AssessmentSource),
  duration: z14.number().min(0).optional(),
  questionsAnswered: z14.number().min(0).optional(),
  correctAnswers: z14.number().min(0).optional(),
  evidence: z14.array(
...
var RecommendationFeedbackSchema = z14.object({
  recommendationId: z14.string().min(1),
  userId: z14.string().min(1),
  isHelpful: z14.boolean(),
  rating: z14.number().min(1).max(5).optional(),
  comment: z14.string().optional(),
  timeSpent: z14.number().min(0).optional(),
...
var SAMWebSocketEventSchema = z15.object({
  type: z15.string(),
  payload: z15.unknown(),
  timestamp: z15.date(),
  eventId: z15.string().min(1),
  userId: z15.string().optional(),
  sessionId: z15.string().optional()
});
var ConnectionConfigSchema = z15.object({
  url: z15.string().url(),
  maxReconnectAttempts: z15.number().min(0).max(20),
  reconnectDelay: z15.number().min(100).max(6e4),
  heartbeatInterval: z15.number().min(5e3).max(3e5),
  idleTimeout: z15.number().min(1e4).max(6e5),
  awayTimeout: z15.number().min(6e4).max(36e5),
  autoReconnect: z15.boolean(),
  authToken: z15.string().optional()
});
var PushDeliveryRequestSchema = z15.object({
  id: z15.string().min(1),
  userId: z15.string().min(1),
  event: SAMWebSocketEventSchema,
  priority: z15.enum(["critical", "high", "normal", "low"]),
  channels: z15.array(z15.enum(["websocket", "sse", "push_notification", "email", "in_app"])),
  fallbackChannels: z15.array(z15.enum(["websocket", "sse", "push_notification", "email", "in_app"])).optional(),
