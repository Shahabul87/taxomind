...
var GoalPrioritySchema = z.enum(["low", "medium", "high", "critical"]);
var GoalStatusSchema = z.enum(["draft", "active", "paused", "completed", "abandoned"]);
var SubGoalTypeSchema = z.enum(["learn", "practice", "assess", "review", "reflect", "create"]);
var PlanStatusSchema = z.enum(["draft", "active", "paused", "completed", "failed", "cancelled"]);
var StepStatusSchema = z.enum(["pending", "in_progress", "completed", "failed", "skipped", "blocked"]);
var MasteryLevelSchema = z.enum(["novice", "beginner", "intermediate", "advanced", "expert"]);
var GoalContextSchema = z.object({
...
var CreateGoalInputSchema = z.object({
...
var UpdateGoalInputSchema = z.object({
...
var DecompositionOptionsSchema = z.object({
...
var SubGoalAISchema = z2.object({
  title: z2.string(),
  description: z2.string().optional(),
  type: z2.enum(["learn", "practice", "assess", "review", "reflect", "create"]),
  estimatedMinutes: z2.number().int().min(5).max(240),
  difficulty: z2.enum(["easy", "medium", "hard"]),
  prerequisites: z2.array(z2.number().int()).default([]),
  // indices of prerequisite sub-goals
  successCriteria: z2.array(z2.string()).default([])
});
var DecompositionAIResponseSchema = z2.object({
  subGoals: z2.array(SubGoalAISchema).min(1).max(20),
  overallDifficulty: z2.enum(["easy", "medium", "hard"]),
...
var ToolCategorySchema = z3.enum([
  "content",
  "assessment",
  "communication",
  "analytics",
...
var PermissionLevelSchema = z3.enum(["read", "write", "execute", "admin"]);
var ConfirmationTypeSchema = z3.enum(["none", "implicit", "explicit", "critical"]);
var ToolExecutionStatusSchema = z3.enum([
  "pending",
...
var RateLimitSchema = z3.object({
  maxCalls: z3.number().int().min(1),
  windowMs: z3.number().int().min(1e3),
  scope: z3.enum(["global", "user", "session"])
});
var ToolExampleSchema = z3.object({
  name: z3.string().min(1),
  description: z3.string(),
  input: z3.unknown(),
  expectedOutput: z3.unknown().optional()
});
var RegisterToolInputSchema = z3.object({
  name: z3.string().min(1).max(100),
  description: z3.string().min(1).max(1e3),
  category: ToolCategorySchema,
  version: z3.string().regex(/^\d+\.\d+\.\d+$/),
  requiredPermissions: z3.array(PermissionLevelSchema).min(1),
  confirmationType: ConfirmationTypeSchema,
  timeoutMs: z3.number().int().min(1e3).max(3e5).optional(),
  maxRetries: z3.number().int().min(0).max(5).optional(),
  rateLimit: RateLimitSchema.optional(),
  tags: z3.array(z3.string()).optional(),
  examples: z3.array(ToolExampleSchema).optional(),
  enabled: z3.boolean().optional().default(true)
});
var InvokeToolInputSchema = z3.object({
  toolId: z3.string().min(1),
  input: z3.unknown(),
  sessionId: z3.string().min(1),
  skipConfirmation: z3.boolean().optional(),
...
var ContentGenerationRequestSchema = z5.object({
  type: z5.enum(["explanation", "example", "quiz", "summary", "hint", "feedback"]),
  topic: z5.string().min(1),
  context: z5.object({
    courseId: z5.string().optional(),
    chapterId: z5.string().optional(),
    sectionId: z5.string().optional(),
    learningObjective: z5.string().optional()
  }).optional(),
  difficulty: z5.enum(["beginner", "intermediate", "advanced"]).optional(),
  format: z5.enum(["markdown", "html", "plain"]).optional(),
  maxLength: z5.number().int().min(50).max(1e4).optional(),
  style: z5.enum(["formal", "casual", "technical"]).optional(),
  includeExamples: z5.boolean().optional(),
  targetAudience: z5.string().optional()
});
var ContentRecommendationRequestSchema = z5.object({
  userId: z5.string().min(1),
  currentContext: z5.object({
    courseId: z5.string().optional(),
    chapterId: z5.string().optional(),
    sectionId: z5.string().optional(),
    currentTopic: z5.string().optional()
  }),
  learningGoals: z5.array(z5.string()).optional(),
  maxRecommendations: z5.number().int().min(1).max(20).optional(),
  includeExternal: z5.boolean().optional()
});
var StudySessionRequestSchema = z5.object({
  userId: z5.string().min(1),
  goalId: z5.string().optional(),
  duration: z5.number().int().min(15).max(480),
  topics: z5.array(z5.string()).optional(),
  preferredTime: z5.object({
    start: z5.string().regex(/^\d{2}:\d{2}$/),
    end: z5.string().regex(/^\d{2}:\d{2}$/)
  }).optional(),
  breakInterval: z5.number().int().min(15).max(120).optional(),
  breakDuration: z5.number().int().min(5).max(30).optional()
});
var ReminderRequestSchema = z5.object({
  userId: z5.string().min(1),
  type: z5.enum(["study", "assessment", "deadline", "check_in", "custom"]),
  message: z5.string().min(1).max(500),
  scheduledFor: z5.coerce.date(),
  recurring: z5.object({
    frequency: z5.enum(["daily", "weekly", "monthly"]),
    until: z5.coerce.date().optional()
  }).optional(),
  channels: z5.array(z5.enum(["email", "push", "in_app"])).optional()
});
var NotificationRequestSchema = z5.object({
  userId: z5.string().min(1),
  type: z5.enum([
    "achievement",
    "reminder",
  ...
  title: z5.string().min(1).max(100),
  body: z5.string().min(1).max(500),
  priority: z5.enum(["low", "normal", "high", "urgent"]),
  channels: z5.array(z5.enum(["email", "push", "in_app", "sms"])).optional(),
  data: z5.record(z5.unknown()).optional(),
  expiresAt: z5.coerce.date().optional(),
  actionUrl: z5.string().url().optional(),
  actionLabel: z5.string().max(50).optional()
});
var ProgressReportRequestSchema = z5.object({
  userId: z5.string().min(1),
  period: z5.enum(["daily", "weekly", "monthly"]),
  includeComparison: z5.boolean().optional(),
  includeGoals: z5.boolean().optional(),
  includeRecommendations: z5.boolean().optional()
});
  ...
  return [
    ...
    },
    {
      ...
      inputSchema: z6.object({
        ...
        weekStart: z6.coerce.date(),
        goals: z6.array(
          z6.object({
            id: z6.string(),
            title: z6.string(),
            estimatedMinutes: z6.number().int().min(15),
            deadline: z6.coerce.date().optional(),
            priority: z6.number().int().min(1).max(5)
          })
        ),
        preferences: z6.object({
          dailyStudyLimit: z6.number().int().min(30).max(480),
          preferredDays: z6.array(z6.number().int().min(0).max(6)),
          preferredHours: z6.object({
            start: z6.number().int().min(0).max(23),
            end: z6.number().int().min(0).max(23)
          }),
          breakFrequency: z6.number().int().min(15).max(120)
        })
      }),
...
function createNotificationTools(deps) {
  return [
    ...
    },
    {
      ...
      inputSchema: z7.object({
        userId: z7.string().min(1),
        achievement: z7.object({
          id: z7.string(),
          name: z7.string(),
          description: z7.string(),
          icon: z7.string().optional(),
          rarity: z7.enum(["common", "uncommon", "rare", "epic", "legendary"]).optional()
        }),
...
var VectorSearchOptionsSchema = z8.object({
  topK: z8.number().min(1).max(100),
  minScore: z8.number().min(0).max(1).optional(),
  maxDistance: z8.number().min(0).optional(),
  filter: z8.object({
    sourceTypes: z8.array(z8.string()).optional(),
    userIds: z8.array(z8.string()).optional(),
    courseIds: z8.array(z8.string()).optional(),
    tags: z8.array(z8.string()).optional(),
    dateRange: z8.object({
      start: z8.date().optional(),
      end: z8.date().optional()
    }).optional()
  }).optional(),
  includeMetadata: z8.boolean().optional()
});
var GraphQueryOptionsSchema = z8.object({
  maxDepth: z8.number().min(1).max(10).optional(),
  relationshipTypes: z8.array(z8.string()).optional(),
  entityTypes: z8.array(z8.string()).optional(),
  minWeight: z8.number().min(0).max(1).optional(),
  limit: z8.number().min(1).max(1e3).optional(),
  direction: z8.enum(["outgoing", "incoming", "both"]).optional()
});
var RetrievalQuerySchema = z8.object({
  query: z8.string().min(1),
  userId: z8.string().optional(),
  courseId: z8.string().optional(),
  memoryTypes: z8.array(z8.string()).optional(),
  sourceTypes: z8.array(z8.string()).optional(),
  timeRange: z8.object({
    start: z8.date().optional(),
    end: z8.date().optional()
  ...
  minRelevance: z8.number().min(0).max(1).optional(),
  includeRelated: z8.boolean().optional(),
  hybridSearch: z8.boolean().optional()
});
...
var ContentChangeEventSchema = z9.object({
  id: z9.string().min(1),
  entityType: z9.enum([
    "course",
    "chapter",
  ...
  entityId: z9.string().min(1),
  changeType: z9.enum(["create", "update", "delete", "bulk_update"]),
  timestamp: z9.date(),
  metadata: z9.object({
    courseId: z9.string().optional(),
    chapterId: z9.string().optional(),
    sectionId: z9.string().optional(),
    userId: z9.string().optional(),
    previousHash: z9.string().optional(),
    newHash: z9.string().optional(),
    fieldsChanged: z9.array(z9.string()).optional(),
    batchId: z9.string().optional()
  })
});
var MemoryLifecycleConfigSchema = z9.object({
  autoReindexEnabled: z9.boolean().default(true),
  debounceMs: z9.number().min(0).default(5e3),
  maxBatchSize: z9.number().min(1).max(1e3).default(100),
  maxConcurrentJobs: z9.number().min(1).max(50).default(5),
  retry: z9.object({
    maxAttempts: z9.number().min(1).max(10).default(3),
    backoffMs: z9.number().min(100).max(6e4).default(1e3),
    backoffMultiplier: z9.number().min(1).max(10).default(2)
  }),
  priorityRules: z9.array(z9.any()).default([]),
  entityConfigs: z9.record(z9.any()).default({})
});
...
var NormalizedMemoryContextSchema = z10.object({
  id: z10.string().min(1),
  userId: z10.string().min(1),
  courseId: z10.string().optional(),
  generatedAt: z10.date(),
  generationTimeMs: z10.number().min(0),
  segments: z10.array(
    z10.object({
      type: z10.string(),
      title: z10.string(),
      items: z10.array(z10.any()),
      relevanceScore: z10.number().min(0).max(1),
      priority: z10.number()
    })
  ),
  relevanceScore: z10.number().min(0).max(1),
  ...
  strategies: z10.array(
    z10.object({
      type: z10.string(),
      durationMs: z10.number(),
      resultsCount: z10.number(),
      avgRelevance: z10.number()
    })
  ),
  metadata: z10.object({
    query: z10.string().optional(),
    totalItemsFound: z10.number(),
    filteredItems: z10.number(),
    estimatedTokens: z10.number(),
    truncated: z10.boolean(),
    custom: z10.record(z10.unknown()).optional()
  })
});
var MemoryNormalizerConfigSchema = z10.object({
  maxItems: z10.number().min(1).max(1e3),
  maxItemsPerSegment: z10.number().min(1).max(100),
  maxContentLength: z10.number().min(100).max(1e4),
  minRelevanceScore: z10.number().min(0).max(1),
  includeSummaries: z10.boolean(),
  maxSummaryLength: z10.number().min(50).max(1e3),
  segmentPriority: z10.array(z10.string()),
  tokenBudget: z10.number().min(100).max(1e5),
  charsPerToken: z10.number().min(1).max(10)
});
...
var BaseJobSchema = z11.object({
  ...
  priority: z11.number().min(0).max(100),
  data: z11.unknown(),
  result: z11.unknown().optional(),
  error: z11.string().optional(),
  attempts: z11.number().min(0),
  maxAttempts: z11.number().min(1),
  createdAt: z11.date(),
  updatedAt: z11.date(),
  scheduledFor: z11.date(),
  startedAt: z11.date().optional(),
  completedAt: z11.date().optional(),
  progress: z11.number().min(0).max(100).optional()
});
var JobQueueConfigSchema = z11.object({
  name: z11.string().min(1),
  concurrency: z11.number().min(1).max(100),
  defaultPriority: z11.number().min(0).max(100),
  defaultMaxAttempts: z11.number().min(1).max(10),
  retryDelayMs: z11.number().min(100).max(36e5),
  retryBackoffMultiplier: z11.number().min(1).max(10),
  jobTimeoutMs: z11.number().min(1e3).max(36e5),
  cleanupAfterMs: z11.number().min(6e4),
  persistJobs: z11.boolean()
});
var WorkerConfigSchema = z11.object({
  id: z11.string().min(1),
  queues: z11.array(z11.string().min(1)),
  concurrency: z11.number().min(1).max(50),
  pollIntervalMs: z11.number().min(100).max(6e4),
  maxJobsPerCycle: z11.number().min(1).max(1e3),
  gracefulShutdown: z11.boolean(),
  shutdownTimeoutMs: z11.number().min(1e3).max(3e5)
});
...
var LearningPlanInputSchema = z12.object({
  userId: z12.string().min(1),
  goalTitle: z12.string().min(1).max(200),
  goalDescription: z12.string().min(1).max(1e3),
  targetDate: z12.date().optional(),
  courseId: z12.string().optional(),
  chapterId: z12.string().optional(),
  currentLevel: z12.enum(["beginner", "intermediate", "advanced"]),
  targetLevel: z12.enum(["beginner", "intermediate", "advanced", "mastery"]),
  preferredDailyMinutes: z12.number().min(5).max(480),
  preferredDaysPerWeek: z12.number().min(1).max(7),
  constraints: z12.array(
    z12.object({
      type: z12.enum(["time", "content", "pace", "style"]),
      description: z12.string(),
      value: z12.unknown()
    })
  ).optional()
});
var ProgressUpdateSchema = z12.object({
  planId: z12.string().min(1),
  date: z12.date(),
  completedActivities: z12.array(z12.string()),
  actualMinutes: z12.number().min(0),
  notes: z12.string().max(1e3).optional(),
  emotionalState: z12.string().optional(),
  difficultyFeedback: z12.enum(["too_easy", "just_right", "too_hard"]).optional()
});
var CheckInResponseSchema = z12.object({
  checkInId: z12.string().min(1),
  respondedAt: z12.date(),
  answers: z12.array(
    z12.object({
      questionId: z12.string().min(1),
      answer: z12.union([z12.string(), z12.array(z12.string()), z12.number(), z12.boolean()])
    })
  ),
  selectedActions: z12.array(z12.string()),
  feedback: z12.string().max(1e3).optional(),
  emotionalState: z12.string().optional()
});
var BehaviorEventSchema = z12.object({
  userId: z12.string().min(1),
  sessionId: z12.string().min(1),
  timestamp: z12.date(),
  type: z12.string(),
  data: z12.record(z12.unknown()),
  pageContext: z12.object({
    url: z12.string(),
    courseId: z12.string().optional(),
    chapterId: z12.string().optional(),
    sectionId: z12.string().optional(),
    contentType: z12.string().optional(),
    timeOnPage: z12.number().optional(),
    scrollDepth: z12.number().optional()
  }),
  emotionalSignals: z12.array(
    z12.object({
      type: z12.string(),
      intensity: z12.number().min(0).max(1),
      source: z12.enum(["text", "behavior", "timing", "pattern"]),
      timestamp: z12.date()
    })
  ).optional()
});
...
var ConfidenceInputSchema = z13.object({
  responseId: z13.string().min(1),
  userId: z13.string().min(1),
  sessionId: z13.string().min(1),
  responseText: z13.string().min(1),
  responseType: z13.enum([
    "explanation",
    "answer",
    "hint",
    "feedback",
  ...
  topic: z13.string().optional(),
  context: z13.object({
    courseId: z13.string().optional(),
    chapterId: z13.string().optional(),
    sectionId: z13.string().optional(),
    questionText: z13.string().optional(),
    studentLevel: z13.string().optional(),
    previousAttempts: z13.number().optional(),
    relatedConcepts: z13.array(z13.string()).optional()
  }).optional(),
  sources: z13.array(
    z13.object({
      id: z13.string(),
      ...
      reliability: z13.number().min(0).max(1),
      lastVerified: z13.date().optional()
    })
  ).optional()
});
var VerificationInputSchema = z13.object({
  responseId: z13.string().min(1),
  userId: z13.string().min(1),
  responseText: z13.string().min(1),
  claims: z13.array(z13.string()).optional(),
  sources: z13.array(
    z13.object({
      id: z13.string(),
      type: z13.string(),
  ...
  strictMode: z13.boolean().optional()
});
var StudentFeedbackSchema = z13.object({
  responseId: z13.string().min(1),
  userId: z13.string().min(1),
  helpful: z13.boolean(),
  rating: z13.number().min(1).max(5).optional(),
  clarity: z13.number().min(1).max(5).optional(),
  comment: z13.string().max(1e3).optional(),
  didUnderstand: z13.boolean(),
  needMoreHelp: z13.boolean(),
  askedFollowUp: z13.boolean().optional(),
  triedAgain: z13.boolean().optional(),
  succeededAfter: z13.boolean().optional()
});
...
var SelfCritiqueInputSchema = z14.object({
  responseId: z14.string().min(1),
  userId: z14.string().min(1),
  sessionId: z14.string().min(1),
  responseText: z14.string().min(10),
  responseType: z14.enum([
    "explanation",
    "answer",
    "hint",
    "feedback",
  ...
  context: z14.object({
    ...
    chapterId: z14.string().optional(),
    sectionId: z14.string().optional(),
    questionText: z14.string().optional(),
    studentLevel: z14.string().optional(),
    previousAttempts: z14.number().optional(),
    relatedConcepts: z14.array(z14.string()).optional()
  }).optional(),
  targetAudience: z14.enum(["beginner", "intermediate", "advanced", "expert"]).optional(),
  learningObjectives: z14.array(z14.string()).optional(),
  enabledDimensions: z14.array(
    z14.enum([
      "accuracy",
      "clarity",
      "completeness",
  ...
  passThreshold: z14.number().min(0).max(100).optional(),
  iteration: z14.number().int().positive().optional(),
  previousScore: z14.number().min(0).max(100).optional()
});
var SelfCritiqueLoopInputSchema = SelfCritiqueInputSchema.extend({
  maxIterations: z14.number().int().min(1).max(10).optional(),
  minImprovement: z14.number().min(0).max(100).optional()
});
...
var LearningSessionInputSchema = z15.object({
  userId: z15.string().min(1),
  topicId: z15.string().min(1),
  startTime: z15.date().optional(),
  duration: z15.number().min(0).optional(),
  activitiesCompleted: z15.number().min(0).optional(),
  questionsAnswered: z15.number().min(0).optional(),
  correctAnswers: z15.number().min(0).optional(),
  conceptsCovered: z15.array(z15.string()).optional(),
  focusScore: z15.number().min(0).max(1).optional()
});
var SkillAssessmentInputSchema = z15.object({
  userId: z15.string().min(1),
  skillId: z15.string().min(1),
  skillName: z15.string().min(1).optional(),
  score: z15.number().min(0).max(100),
  maxScore: z15.number().min(1).optional().default(100),
  source: z15.nativeEnum(AssessmentSource),
  duration: z15.number().min(0).optional(),
  questionsAnswered: z15.number().min(0).optional(),
  correctAnswers: z15.number().min(0).optional(),
  evidence: z15.array(
    z15.object({
      ...
      score: z15.number().optional(),
      timestamp: z15.date(),
      weight: z15.number().min(0).max(1)
    })
  ).optional()
});
var RecommendationFeedbackSchema = z15.object({
  recommendationId: z15.string().min(1),
  userId: z15.string().min(1),
  isHelpful: z15.boolean(),
  rating: z15.number().min(1).max(5).optional(),
  comment: z15.string().optional(),
  timeSpent: z15.number().min(0).optional(),
  completed: z15.boolean().optional()
});
...
var SAMWebSocketEventSchema = z16.object({
  type: z16.string(),
  payload: z16.unknown(),
  timestamp: z16.date(),
  eventId: z16.string().min(1),
  userId: z16.string().optional(),
  sessionId: z16.string().optional()
});
var ConnectionConfigSchema = z16.object({
  url: z16.string().url(),
  maxReconnectAttempts: z16.number().min(0).max(20),
  reconnectDelay: z16.number().min(100).max(6e4),
  heartbeatInterval: z16.number().min(5e3).max(3e5),
  idleTimeout: z16.number().min(1e4).max(6e5),
  awayTimeout: z16.number().min(6e4).max(36e5),
  autoReconnect: z16.boolean(),
  authToken: z16.string().optional()
});
var PushDeliveryRequestSchema = z16.object({
  id: z16.string().min(1),
  userId: z16.string().min(1),
  event: SAMWebSocketEventSchema,
  priority: z16.enum(["critical", "high", "normal", "low"]),
  channels: z16.array(z16.enum(["websocket", "sse", "push_notification", "email", "in_app"])),
  fallbackChannels: z16.array(z16.enum(["websocket", "sse", "push_notification", "email", "in_app"])).optional(),
  expiresAt: z16.date().optional(),
...
var LearningEventSchema = z17.object({
  ...
  outcome: z17.enum(["success", "partial", "failure"]).optional(),
  confidence: z17.number().min(0).max(1).optional(),
  strategyId: z17.string().optional(),
  strategyApplied: z17.string().optional(),
  responseQuality: z17.number().min(0).max(100).optional(),
  studentSatisfaction: z17.number().min(1).max(5).optional(),
  metadata: z17.record(z17.unknown()).optional().default({})
});
var GetInsightsSchema = z17.object({
  ...
  priority: z17.enum(["critical", "high", "medium", "low", "info"]).optional(),
  limit: z17.number().int().min(1).max(100).optional().default(20),
  activeOnly: z17.boolean().optional().default(true)
});
var GetAnalyticsSchema = z17.object({
  userId: z17.string().optional(),
  period: z17.enum(["hour", "day", "week", "month", "quarter", "all_time"]).optional().default("week"),
  includePatterns: z17.boolean().optional().default(true),
  includeStrategies: z17.boolean().optional().default(true),
  includeTrends: z17.boolean().optional().default(true)
});
...
export {
  ...
  ResponseType4 as TelemetryResponseType,
  ToolExecutionStatus2 as TelemetryToolExecutionStatus,
  TimePeriod,
  ToolCategory,
  ToolCategorySchema,
  ToolExampleSchema,
