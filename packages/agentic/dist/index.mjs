...
var LearningPlanInputSchema = z9.object({
  userId: z9.string().min(1),
  goalTitle: z9.string().min(1).max(200),
  goalDescription: z9.string().min(1).max(1e3),
  targetDate: z9.date().optional(),
  courseId: z9.string().optional(),
  chapterId: z9.string().optional(),
  currentLevel: z9.enum(["beginner", "intermediate", "advanced"]),
  targetLevel: z9.enum(["beginner", "intermediate", "advanced", "mastery"]),
  preferredDailyMinutes: z9.number().min(5).max(480),
  preferredDaysPerWeek: z9.number().min(1).max(7),
  constraints: z9.array(
    z9.object({
      type: z9.enum(["time", "content", "pace", "style"]),
      description: z9.string(),
      value: z9.unknown()
    })
  ).optional()
});
var ProgressUpdateSchema = z9.object({
  planId: z9.string().min(1),
  date: z9.date(),
  completedActivities: z9.array(z9.string()),
  actualMinutes: z9.number().min(0),
  notes: z9.string().max(1e3).optional(),
  emotionalState: z9.string().optional(),
  difficultyFeedback: z9.enum(["too_easy", "just_right", "too_hard"]).optional()
});
var CheckInResponseSchema = z9.object({
  checkInId: z9.string().min(1),
  respondedAt: z9.date(),
  answers: z9.array(
    z9.object({
      questionId: z9.string().min(1),
      answer: z9.union([z9.string(), z9.array(z9.string()), z9.number(), z9.boolean()])
    })
  ),
  selectedActions: z9.array(z9.string()),
  feedback: z9.string().max(1e3).optional(),
  emotionalState: z9.string().optional()
});
var BehaviorEventSchema = z9.object({
  userId: z9.string().min(1),
  sessionId: z9.string().min(1),
  timestamp: z9.date(),
  type: z9.string(),
  data: z9.record(z9.unknown()),
  pageContext: z9.object({
    url: z9.string(),
    courseId: z9.string().optional(),
    chapterId: z9.string().optional(),
    sectionId: z9.string().optional(),
    contentType: z9.string().optional(),
    timeOnPage: z9.number().optional(),
    scrollDepth: z9.number().optional()
  }),
  emotionalSignals: z9.array(
    z9.object({
      type: z9.string(),
      intensity: z9.number().min(0).max(1),
      source: z9.enum(["text", "behavior", "timing", "pattern"]),
      timestamp: z9.date()
    })
  ).optional()
});
...
var ConfidenceInputSchema = z10.object({
  responseId: z10.string().min(1),
  userId: z10.string().min(1),
  sessionId: z10.string().min(1),
  responseText: z10.string().min(1),
  responseType: z10.enum([
    "explanation",
    "answer",
    "hint",
    "feedback",
  ...
  context: z10.object({
    ...
    chapterId: z10.string().optional(),
    sectionId: z10.string().optional(),
    questionText: z10.string().optional(),
    studentLevel: z10.string().optional(),
    previousAttempts: z10.number().optional(),
    relatedConcepts: z10.array(z10.string()).optional()
  }).optional(),
  sources: z10.array(
    z10.object({
      id: z10.string(),
      ...
      reliability: z10.number().min(0).max(1),
      lastVerified: z10.date().optional()
    })
  ).optional()
});
var VerificationInputSchema = z10.object({
  responseId: z10.string().min(1),
  userId: z10.string().min(1),
  responseText: z10.string().min(1),
  claims: z10.array(z10.string()).optional(),
  sources: z10.array(
    z10.object({
      id: z10.string(),
      type: z10.string(),
  ...
  strictMode: z10.boolean().optional()
});
var StudentFeedbackSchema = z10.object({
  responseId: z10.string().min(1),
  userId: z10.string().min(1),
  helpful: z10.boolean(),
  rating: z10.number().min(1).max(5).optional(),
  clarity: z10.number().min(1).max(5).optional(),
  comment: z10.string().max(1e3).optional(),
  didUnderstand: z10.boolean(),
  needMoreHelp: z10.boolean(),
  askedFollowUp: z10.boolean().optional(),
  triedAgain: z10.boolean().optional(),
  succeededAfter: z10.boolean().optional()
});
...
var LearningSessionInputSchema = z11.object({
  userId: z11.string().min(1),
  topicId: z11.string().min(1),
  startTime: z11.date().optional(),
  duration: z11.number().min(0).optional(),
  activitiesCompleted: z11.number().min(0).optional(),
  questionsAnswered: z11.number().min(0).optional(),
  correctAnswers: z11.number().min(0).optional(),
  conceptsCovered: z11.array(z11.string()).optional(),
  focusScore: z11.number().min(0).max(1).optional()
});
var SkillAssessmentInputSchema = z11.object({
  userId: z11.string().min(1),
  skillId: z11.string().min(1),
  skillName: z11.string().min(1).optional(),
  score: z11.number().min(0).max(100),
  maxScore: z11.number().min(1).optional().default(100),
  source: z11.nativeEnum(AssessmentSource),
  duration: z11.number().min(0).optional(),
  questionsAnswered: z11.number().min(0).optional(),
  correctAnswers: z11.number().min(0).optional(),
  evidence: z11.array(
...
var RecommendationFeedbackSchema = z11.object({
  recommendationId: z11.string().min(1),
  userId: z11.string().min(1),
  isHelpful: z11.boolean(),
  rating: z11.number().min(1).max(5).optional(),
  comment: z11.string().optional(),
  timeSpent: z11.number().min(0).optional(),
