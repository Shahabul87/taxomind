...
// ============================================================================
export class InMemoryConfidencePredictionStore {
    predictions = new Map();
    maxPredictions;
    constructor(maxPredictions = 10000) {
    ...
    async getCalibrationMetrics(periodStart, periodEnd) {
        const predictions = Array.from(this.predictions.values()).filter((p) => p.predictedAt >= periodStart && p.predictedAt <= periodEnd);
        const withOutcomes = predictions.filter((p) => p.actualOutcome !== undefined);
        // Calculate overall metrics
        const avgPredictedConfidence = predictions.length > 0
            ? predictions.reduce((sum, p) => sum + p.predictedConfidence, 0) / predictions.length
            : 0;
        const avgActualAccuracy = withOutcomes.length > 0
            ? withOutcomes.reduce((sum, p) => sum + (p.actualOutcome.accurate ? 1 : 0), 0) / withOutcomes.length
            : 0;
    ...
    calculateBuckets(withOutcomes) {
        ...
        for (const [rangeStart, rangeEnd] of bucketRanges) {
            const inBucket = withOutcomes.filter((p) => p.predictedConfidence >= rangeStart && p.predictedConfidence < rangeEnd);
            ...
            const actualAccuracy = inBucket.reduce((sum, p) => sum + (p.actualOutcome.accurate ? 1 : 0), 0) /
                inBucket.length;
            buckets.push({
                rangeStart,
                rangeEnd,
    ...
    calculateByResponseType(predictions, withOutcomes) {
        ...
        for (const type of types) {
            ...
            const avgPredictedConfidence = typePredictions.length > 0
                ? typePredictions.reduce((sum, p) => sum + p.predictedConfidence, 0) /
                    typePredictions.length
                : 0;
            const avgActualAccuracy = typeWithOutcomes.length > 0
                ? typeWithOutcomes.reduce((sum, p) => sum + (p.actualOutcome.accurate ? 1 : 0), 0) / typeWithOutcomes.length
                : 0;
            result[type] = {
...
export class ConfidenceCalibrationTracker {
    ...
    async getCalibrationSummary() {
        ...
        const calibrationQuality = metrics.calibrationError < 0.05
            ? 'excellent'
        ...
        if (worstBucket && worstBucket.error > 0.2) {
            recommendations.push(`Focus on improving calibration in ${(worstBucket.rangeStart * 100).toFixed(0)}-${(worstBucket.rangeEnd * 100).toFixed(0)}% confidence range`);
        }
        return {
            calibrationQuality,
...
export function createInMemoryConfidencePredictionStore(maxPredictions) {
    return new InMemoryConfidencePredictionStore(maxPredictions);
}
//# sourceMappingURL=confidence-calibration.js.map