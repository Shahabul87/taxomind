...
export class InMemoryToolExecutionStore {
    ...
    async getMetrics(periodStart, periodEnd) {
        const events = Array.from(this.events.values()).filter((e) => e.startedAt >= periodStart && e.startedAt <= periodEnd);
        const completedEvents = events.filter((e) => e.status === ToolExecutionStatus.SUCCESS || e.status === ToolExecutionStatus.FAILED);
        ...
        const confirmationGivenEvents = events.filter((e) => e.confirmationRequired && e.confirmationGiven);
        ...
        return {
            ...
            confirmationAcceptRate: confirmationRequiredEvents.length > 0
...
export const DEFAULT_TOOL_TELEMETRY_CONFIG = {
    enabled: true,
    sampleRate: 1.0,
    maxEvents: 10000,
    sanitize: true,
...
export function createInMemoryToolExecutionStore(maxEvents) {
    return new InMemoryToolExecutionStore(maxEvents);
}
//# sourceMappingURL=tool-telemetry.js.map