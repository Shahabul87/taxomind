...
import { ContentGenerationRequestSchema, ContentRecommendationRequestSchema, } from './types';
// ============================================================================
// TOOL HANDLERS
// ============================================================================
/**
    ...
    return async (input, _context) => {
        ...
        try {
            ...
            if (deps.contentRepository) {
                recommendations = await deps.contentRepository.getRelatedContent(request.currentContext, maxRecs);
            }
            // If we need more or no repository, use AI to generate recommendations
            if (recommendations.length < maxRecs) {
                const aiRecommendations = await generateAIRecommendations(deps.aiAdapter, request, maxRecs - recommendations.length);
 ...
 */
export function createContentTools(deps) {
    return [
        {
            id: 'content-generate',
