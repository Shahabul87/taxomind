...
import { StudySessionRequestSchema, ReminderRequestSchema, } from './types';
import { z } from 'zod';
// ============================================================================
// TOOL HANDLERS
// ============================================================================
    ...
    return async (input, _context) => {
        ...
        try {
            ...
            if (deps.notificationService) {
                await deps.notificationService.schedule(request.userId, request.message, request.scheduledFor, reminder.channels);
            }
...
function generateOptimizedSchedule(request, _existingSessions) {
    ...
    for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
        ...
        for (const goal of sortedGoals) {
            ...
            const sessionDuration = Math.min(goal.estimatedMinutes, request.preferences.dailyStudyLimit - dailyMinutes, 90 // Max 90 minutes per session
            );
    ...
    if (schedule.coveragePercentage < 100) {
        schedule.recommendations.push(`Current schedule covers ${schedule.coveragePercentage.toFixed(0)}% of your goals. Consider extending study time or adding more days.`);
    }
    if (sortedGoals.some((g) => g.deadline)) {
        schedule.recommendations.push('You have goals with deadlines. Prioritize these in your daily schedule.');
    }
    return schedule;
}
// ============================================================================
 ...
 */
export function createSchedulingTools(deps) {
    return [
        {
            id: 'schedule-session',
...
//# sourceMappingURL=scheduling-tools.js.map