 ...
 */
import { z } from 'zod';
import { SubGoalType, StepStatus, DecompositionOptionsSchema, } from './types';
...
export class GoalDecomposer {
    ...
    async decompose(goal, options) {
        ...
        try {
            ...
            this.logger.debug?.(`[GoalDecomposer] Decomposed into ${subGoals.length} sub-goals in ${Date.now() - startTime}ms`);
...
- Keep descriptions concise to fit within token limits`,
    ...
    buildDecompositionPrompt(goal, options) {
        ...
        if (options.targetCompletionDate) {
            const daysUntil = Math.ceil((options.targetCompletionDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
...
// FACTORY FUNCTION
// ============================================================================
export function createGoalDecomposer(config) {
