...
export class AgentStateMachine {
    ...
    async executeNextStep() {
        if (!this.currentPlan || !this.planState)
            return;
        if (this.currentState !== 'running')
            return;
        // Find next pending step
        const nextStep = this.currentPlan.steps.find((s) => s.status === StepStatus.PENDING);
        if (!nextStep) {
            // All steps completed
    ...
    async handleStepFailed(stepId, error) {
        ...
        this.logger.warn?.(`[StateMachine] Step failed: ${stepId}, attempt ${step.retryCount}/${maxRetries}`);
...
// FACTORY FUNCTION
// ============================================================================
export function createAgentStateMachine(config) {
