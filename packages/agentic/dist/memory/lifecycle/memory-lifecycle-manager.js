...
// IN-MEMORY JOB STORE
// ============================================================================
export class InMemoryReindexJobStore {
    jobs = new Map();
    async create(job) {
        const now = new Date();
        const newJob = {
    ...
    async findPending(limit) {
        const pending = Array.from(this.jobs.values())
            .filter((j) => j.status === 'pending' || j.status === 'queued' || j.status === 'retrying')
            .filter((j) => j.scheduledFor <= new Date())
            .sort((a, b) => b.priority - a.priority || a.createdAt.getTime() - b.createdAt.getTime());
        return pending.slice(0, limit);
    }
    async findByEntity(entityType, entityId) {
        return Array.from(this.jobs.values()).filter((j) => j.entityType === entityType && j.entityId === entityId);
    }
...
export class MemoryLifecycleManager {
    ...
    constructor(options) {
        ...
        this.debouncer = new ChangeDebouncer(this.config.debounceMs, (events) => this.processDebouncedEvents(events));
    }
    // -------------------------------------------------------------------------
    // Event Handling
    // -------------------------------------------------------------------------
    ...
    async queueReindexJob(job) {
        ...
        const pendingJob = existingJobs.find((j) => j.status === 'pending' || j.status === 'queued');
        if (pendingJob) {
        ...
        for (const job of jobs) {
            try {
                ...
                if (job.attempts + 1 < job.maxAttempts) {
                    const backoffMs = this.config.retry.backoffMs *
    ...
    async start() {
        ...
        this.processingInterval = setInterval(() => this.processJobs().catch((e) => this.logger.error('Job processing error', { error: e })), 5000 // Process every 5 seconds
        );
    }
    async stop() {
        if (!this.isRunning)
...
//# sourceMappingURL=memory-lifecycle-manager.js.map