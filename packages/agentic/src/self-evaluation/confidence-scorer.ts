 ...
 */
export class InMemoryConfidenceScoreStore implements ConfidenceScoreStore {
  private scores: Map<string, ConfidenceScore> = new Map();
  private responseIndex: Map<string, string> = new Map(); // responseId -> scoreId
  ...
  async getAverageByTopic(topic: string, since?: Date): Promise<number> {
    const topicScores = Array.from(this.scores.values()).filter(
      (score) =>
        score.topic === topic && (!since || score.scoredAt >= since)
    );
 ...
 */
const DEFAULT_FACTOR_WEIGHTS: Record<ConfidenceFactorType, number> = {
...
export class ConfidenceScorer {
  private store: ConfidenceScoreStore;
  ...
  private factorWeights: Record<ConfidenceFactorType, number>;
   ...
   */
  async scoreResponse(input: ConfidenceInput): Promise<ConfidenceScore> {
    ...
    const typedInput: ConfidenceInput = {
      ...validated,
      responseType: validated.responseType as ResponseType,
   ...
   */
  async getScore(responseId: string): Promise<ConfidenceScore | null> {
   ...
   */
  async getUserHistory(userId: string, limit?: number): Promise<ConfidenceScore[]> {
   ...
   */
  async getTopicAverage(topic: string, since?: Date): Promise<number> {
  ...
  ): Promise<{ score: number; level: ConfidenceLevel; shouldVerify: boolean }> {
    const tempInput: ConfidenceInput = {
      responseId: 'temp',
      userId: 'temp',
      sessionId: 'temp',
      responseText,
  ...
  private async calculateFactors(input: ConfidenceInput): Promise<ConfidenceFactor[]> {
  ...
  private calculateKnowledgeCoverage(input: ConfidenceInput): ConfidenceFactor {
    ...
    if (input.sources && input.sources.length > 0) {
      const avgReliability =
        input.sources.reduce((sum, s) => sum + s.reliability, 0) / input.sources.length;
  ...
  private calculateSourceReliability(sources?: SourceReference[]): ConfidenceFactor {
    ...
    for (const source of sources) {
      const typeWeight = typeWeights[source.type as SourceType] ?? 0.5;
  ...
  private calculateComplexityMatch(input: ConfidenceInput): ConfidenceFactor {
    ...
    const levelMap: Record<string, ComplexityLevel> = {
  ...
  private calculateContextRelevance(input: ConfidenceInput): ConfidenceFactor {
  ...
  private async calculateHistoricalAccuracy(input: ConfidenceInput): Promise<ConfidenceFactor> {
  ...
  private calculateConceptClarity(responseText: string): ConfidenceFactor {
    let score = 0.7;
    const reasoning: string[] = [];
    ...
    const hasExplanations = explanationMarkers.some((marker) =>
  ...
  private calculatePrerequisiteKnowledge(input: ConfidenceInput): ConfidenceFactor {
    let score = 0.7;
    const reasoning: string[] = [];
  ...
  private calculateAmbiguityLevel(responseText: string): ConfidenceFactor {
    ...
    const ambiguityCount = ambiguousPatterns.filter((pattern) =>
    ...
    const contradictionCount = contradictionMarkers.filter((marker) =>
      responseText.toLowerCase().includes(marker)
    ).length;
  ...
  private assessComplexity(
    responseText: string,
    context?: ConfidenceInput['context']
  ): ComplexityLevel {
    // Simple heuristics for complexity assessment
    const wordCount = responseText.split(/\s+/).length;
    const avgWordLength =
      responseText.replace(/\s+/g, '').length / Math.max(1, wordCount);
