 ...
 */
export class InMemoryQualityRecordStore implements QualityRecordStore {
  private records: Map<string, QualityRecord> = new Map();
  private responseIndex: Map<string, string> = new Map();
  private feedbackStore: Map<string, StudentFeedback> = new Map();
  private outcomeStore: Map<string, LearningOutcome> = new Map();
  ...
  async update(id: string, updates: Partial<QualityRecord>): Promise<QualityRecord> {
    ...
    const updatedRecord: QualityRecord = {
      ...record,
      ...updates,
      id: record.id,
      updatedAt: new Date(),
  ...
  async getSummary(
    ...
    return {
      ...
      averageConfidence,
      calibrationScore,
      byResponseType: {} as Record<ResponseType, QualityAggregate>,
      byTopic: {},
      byComplexity: {} as Record<ComplexityLevel, QualityAggregate>,
      qualityTrend,
      confidenceTrend,
  ...
  private calculateCalibrationScore(records: QualityRecord[]): number {
    const withBoth = records.filter(
      (r) => r.confidenceScore !== undefined && r.overallQuality !== undefined
    );
  ...
  private identifyImprovementAreas(records: QualityRecord[]): string[] {
    const areas: string[] = [];
  ...
  private identifyStrengths(records: QualityRecord[]): string[] {
...
export class QualityTracker {
   ...
   */
  async recordFeedback(feedback: StudentFeedback): Promise<void> {
    ...
    const feedbackWithId: StudentFeedback = {
      ...validated,
      id: uuidv4(),
   ...
   */
  async recordExpertReview(responseId: string, review: ExpertReview): Promise<void> {
   ...
   */
  async calculateCalibration(userId?: string, topic?: string): Promise<CalibrationData | null> {
    this.logger.info('Calculating calibration', { userId, topic });

    const now = new Date();
    const windowStart = new Date(
      now.getTime() - this.calibrationWindow * 24 * 60 * 60 * 1000
    );
    ...
    const expectedAccuracy =
      records.reduce((sum, r) => sum + (r.confidenceScore ?? 0), 0) / records.length;
    const actualAccuracy =
      records.reduce((sum, r) => sum + r.overallQuality, 0) / records.length;
    ...
    const adjustmentFactor =
      actualAccuracy > 0 ? actualAccuracy / Math.max(0.1, expectedAccuracy) : 1;
    const adjustmentDirection: 'increase' | 'decrease' | 'none' =
      adjustmentFactor > 1.1 ? 'increase' : adjustmentFactor < 0.9 ? 'decrease' : 'none';
   ...
   */
  async getCalibrationHistory(userId?: string, limit?: number): Promise<CalibrationData[]> {
   ...
   */
  async getLatestCalibration(userId?: string, topic?: string): Promise<CalibrationData | null> {
   ...
   */
  async getQualityRecord(responseId: string): Promise<QualityRecord | null> {
   ...
   */
  async getUserHistory(userId: string, limit?: number): Promise<QualityRecord[]> {
  ...
  private calculateOverallQuality(metrics: QualityMetric[]): number {
    ...
    const sourceWeights: Record<MetricSource, number> = {
  ...
  private deriveFeedbackMetrics(feedback: StudentFeedback): QualityMetric[] {
    const metrics: QualityMetric[] = [];
  ...
  private deriveExpertMetrics(review: ExpertReview): QualityMetric[] {
    const metrics: QualityMetric[] = [];
  ...
  private deriveOutcomeMetrics(outcome: LearningOutcome): QualityMetric[] {
    const metrics: QualityMetric[] = [];

    // Calculate success rate
    const successRate =
      outcome.subsequentAttempts > 0
  ...
  private calculateCalibrationBuckets(records: QualityRecord[]): CalibrationBucket[] {
    const buckets: CalibrationBucket[] = [];

    const levelRanges: Array<{ level: ConfidenceLevel; min: number; max: number }> = [
    ...
    for (const range of levelRanges) {
      const levelRecords = records.filter(
        (r) =>
          (r.confidenceScore ?? 0) >= range.min && (r.confidenceScore ?? 0) < range.max
      );
      ...
      const expectedAccuracy =
        levelRecords.reduce((sum, r) => sum + (r.confidenceScore ?? 0), 0) /
        levelRecords.length;
      const actualAccuracy =
        levelRecords.reduce((sum, r) => sum + r.overallQuality, 0) / levelRecords.length;
 ...
 */
export function createQualityTracker(config?: QualityTrackerConfig): QualityTracker {
