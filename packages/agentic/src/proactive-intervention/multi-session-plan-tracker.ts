 ...
 */
export class InMemoryLearningPlanStore implements LearningPlanStore {
  private plans: Map<string, LearningPlan> = new Map();
...
export class MultiSessionPlanTracker {
  private store: LearningPlanStore;
  private logger: ProactiveLogger;
  private defaultDailyMinutes: number;
  private streakGracePeriodDays: number;
   ...
   */
  async createLearningPlan(input: LearningPlanInput): Promise<LearningPlan> {
   ...
   */
  async generateWeeklyBreakdown(plan: LearningPlan): Promise<WeeklyBreakdown> {
   ...
   */
  async getDailyPractice(userId: string, date: Date): Promise<DailyPractice> {
   ...
   */
  async trackProgress(planId: string, progress: ProgressUpdate): Promise<void> {
   ...
   */
  async adjustPlan(planId: string, feedback: PlanFeedback): Promise<LearningPlan> {
    const plan = await this.store.get(planId);
    if (!plan) {
      throw new Error(`Plan not found: ${planId}`);
    }
   ...
   * Get a plan by ID
   */
  async getPlan(planId: string): Promise<LearningPlan | null> {
   ...
   */
  async getActivePlan(userId: string): Promise<LearningPlan | null> {
   ...
   * Pause a plan
   */
  async pausePlan(planId: string): Promise<LearningPlan> {
    const plan = await this.store.get(planId);
    if (!plan) {
      throw new Error(`Plan not found: ${planId}`);
    }
   ...
   */
  async resumePlan(planId: string): Promise<LearningPlan> {
    const plan = await this.store.get(planId);
    if (!plan) {
      throw new Error(`Plan not found: ${planId}`);
    }
  ...
  private calculateDefaultTargetDate(input: LearningPlanInput): Date {
  ...
  private estimateWeeksNeeded(input: LearningPlanInput): number {
  ...
  private getWeekTitle(week: number, totalWeeks: number): string {
  ...
  private getWeekObjectives(week: number, totalWeeks: number): string[] {
    const phase = week / totalWeeks;
    if (phase <= 0.25) {
      return [
  ...
  private isActiveDay(dayOfWeek: number, preferredDaysPerWeek: number): boolean {
  ...
  private convertToActivities(planned: PlannedActivity[]): DailyActivity[] {
  ...
  private async getReviewItems(_userId: string): Promise<ReviewItem[]> {
    // In a real implementation, this would fetch from spaced repetition system
    return [];
  }

  private async calculateStreakInfo(userId: string): Promise<StreakInfo> {
    ...
    for (const target of allTargets) {
      ...
      if (lastDate === null) {
        ...
        const dayDiff = Math.floor(
          (lastDate.getTime() - targetDate.getTime()) / (24 * 60 * 60 * 1000)
        );
    ...
    const daysSinceActivity = Math.floor(
      (today.getTime() - new Date(lastActivityDate).getTime()) / (24 * 60 * 60 * 1000)
    );
  ...
  private extractDailyGoals(target: DailyTarget): string[] {
    return target.activities.map((a) => a.title);
  }

  private generateMotivationalMessage(streak: StreakInfo, progress: number): string {
  ...
  private async updateOverallProgress(planId: string): Promise<void> {
    const plan = await this.store.get(planId);
    if (!plan) return;
  ...
  private expectedProgress(plan: LearningPlan): number {
    const now = new Date();
  ...
  private analyzeDayPatterns(targets: DailyTarget[]): { strongDays: number[]; weakDays: number[] } {
  ...
  private generateRecommendations(
    plan: LearningPlan,
    dayStats: { strongDays: number[]; weakDays: number[] }
  ): PlanRecommendation[] {
    ...
    if (plan.overallProgress < expectedProg - 10) {
      recommendations.push({
        ...
        suggestedAction:
          'Consider increasing daily practice time or extending your target date',
      });
    }
    ...
    if (plan.difficultyAdjustments.length === 0 && plan.overallProgress > 25) {
      recommendations.push({
        ...
        suggestedAction:
          'Provide feedback on difficulty to optimize your learning experience',
      });
    }
  ...
  private adjustPace(plan: LearningPlan, feedback: PlanFeedback): LearningPlan {
    const currentPace =
      plan.dailyTargets.reduce((sum, t) => sum + t.estimatedMinutes, 0) / plan.durationWeeks;
  ...
  private adjustDifficulty(plan: LearningPlan, feedback: PlanFeedback): LearningPlan {
  ...
  private adjustContent(plan: LearningPlan, _feedback: PlanFeedback): LearningPlan {
  ...
  private adjustSchedule(plan: LearningPlan, _feedback: PlanFeedback): LearningPlan {
 ...
 */
export function createMultiSessionPlanTracker(
  config?: MultiSessionPlanTrackerConfig
