 ...
 */
export class InMemorySkillAssessmentStore implements SkillAssessmentStore {
  private assessments: Map<string, SkillAssessment> = new Map();
  private userSkillIndex: Map<string, string> = new Map(); // `userId:skillId` -> assessmentId

  private getKey(userId: string, skillId: string): string {
  ...
  async getByUser(userId: string): Promise<SkillAssessment[]> {
    const userAssessments: SkillAssessment[] = [];
...
export class SkillAssessor {
   ...
   * Assess a skill
   */
  async assessSkill(input: SkillAssessmentInput): Promise<SkillAssessment> {
   ...
   */
  async getAssessment(userId: string, skillId: string): Promise<SkillAssessment | null> {
   ...
   * Predict skill decay
   */
  async predictDecay(userId: string): Promise<SkillDecay[]> {
    this.logger.info('Predicting skill decay', { userId });

    const assessments = await this.store.getByUser(userId);
    const decayPredictions: SkillDecay[] = [];
    const now = new Date();

    for (const assessment of assessments) {
      const daysSinceAssessment = Math.floor(
        (now.getTime() - assessment.assessedAt.getTime()) / (24 * 60 * 60 * 1000)
      );
      ...
      const suggestedReviewDate = new Date(
        assessment.assessedAt.getTime() + daysUntilSignificantDecay * 24 * 60 * 60 * 1000
      );
   ...
   */
  async compareSkills(userId: string, benchmarkData?: Map<string, number>): Promise<SkillComparison[]> {
  ...
  async getPrerequisiteStatus(
    userId: string,
    skillId: string
  ): Promise<{ met: string[]; unmet: string[]; partiallyMet: string[] }> {
   ...
   */
  async getImprovementRate(userId: string, skillId: string): Promise<number> {
   ...
   */
  async getSkillsByLevel(userId: string, level: MasteryLevel): Promise<SkillAssessment[]> {
    const assessments = await this.store.getByUser(userId);
  ...
  private calculateConfidence(source: AssessmentSource, evidence?: AssessmentEvidence[]): number {
    // Base confidence from source
    const sourceConfidence: Record<AssessmentSource, number> = {
  ...
  private calculateDecayRate(assessment: SkillAssessment): number {
    // Decay rate varies by mastery level - higher levels decay slower
    const levelMultiplier: Record<MasteryLevel, number> = {
  ...
  private suggestFocusAreas(nodes: SkillNode[], _assessments: SkillAssessment[]): string[] {
    const suggestions: Array<{ skillId: string; priority: number }> = [];
    ...
    for (const node of nodes) {
      ...
      if (node.lastAssessed) {
        const daysSince = Math.floor(
          (Date.now() - node.lastAssessed.getTime()) / (24 * 60 * 60 * 1000)
        );
 ...
 */
export function createSkillAssessor(config?: SkillAssessorConfig): SkillAssessor {
