...
export class MemoryRetriever {
  ...
  private readonly sessionContext?: CrossSessionContext;
  private readonly logger: MemoryLogger;
  private readonly defaultLimit: number;
  private readonly minRelevanceScore: number;
  private readonly recencyBoostFactor: number;
  private readonly userContextBoostFactor: number;
  private readonly hybridSearchWeight: number;
  ...
  async retrieve(query: RetrievalQuery): Promise<RetrievalResult> {
    ...
    if (query.sourceTypes?.length) {
      memories = memories.filter((m) =>
        query.sourceTypes!.includes(m.source.type as EmbeddingSourceType)
      );
    }
  ...
  async retrieveRecent(
    ...
    results.sort(
      (a, b) =>
        b.embedding.createdAt.getTime() - a.embedding.createdAt.getTime()
    );
  ...
  async retrieveRelatedConcepts(
    ...
    return related.map((entity) => ({
      ...
      relevanceScore: 0.8,
      source: {
        type: 'course_content' as EmbeddingSourceType,
        id: entity.id,
        title: entity.name,
      },
      context: {
  ...
  private async vectorSearch(query: RetrievalQuery): Promise<MemoryItem[]> {
  ...
  private async graphSearch(query: RetrievalQuery): Promise<MemoryItem[]> {
    if (!this.knowledgeGraph) {
      return [];
    }
  ...
  private async applyUserContextBoost(
    userId: string,
    memories: MemoryItem[],
    courseId?: string
  ): Promise<MemoryItem[]> {
  ...
  private applyRecencyBoost(memories: MemoryItem[]): MemoryItem[] {
    const now = Date.now();
   ...
   */
  async retrieveLearningPathContext(
    fromTopicId: string,
    toTopicId: string
  ): Promise<MemoryItem[]> {
    if (!this.knowledgeGraph) {
  ...
  private convertToMemoryItem(result: SimilarityResult): MemoryItem {
  ...
  private inferMemoryType(sourceType: EmbeddingSourceType): MemoryType {
    switch (sourceType) {
      case 'course_content':
  ...
  private deduplicateAndSort(memories: MemoryItem[]): MemoryItem[] {
   ...
   */
  async getStats(): Promise<MemoryRetrieverStats> {
