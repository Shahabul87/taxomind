...
// ============================================================================
// IN-MEMORY PUSH QUEUE STORE
// ============================================================================

interface QueuedRequest extends PushDeliveryRequest {
  queuedAt: Date;
  attempts: number;
  lastAttemptAt?: Date;
  processing: boolean;
...
export class InMemoryPushQueueStore implements PushQueueStore {
  private queue: Map<string, QueuedRequest> = new Map();
  private completed: Map<string, PushDeliveryResult> = new Map();
  private failed: Map<string, PushDeliveryResult> = new Map();
  ...
  async dequeue(count: number): Promise<PushDeliveryRequest[]> {
    ...
    const priorityOrder: DeliveryPriority[] = ['critical', 'high', 'normal', 'low'];
  ...
  async requeue(request: PushDeliveryRequest): Promise<void> {
    const existing = this.queue.get(request.id) as QueuedRequest | undefined;
    if (existing) {
...
export class ProactivePushDispatcher implements PushDispatcherInterface {
  private readonly store: PushQueueStore;
  private readonly config: PushDispatcherConfig;
  private readonly logger: RealtimeLogger;
  private readonly handlers: Map<DeliveryChannel, DeliveryHandler> = new Map();
  private readonly presenceTracker?: PresenceTrackerInterface;
  ...
  async dispatch(request: PushDeliveryRequest): Promise<void> {
  ...
  async processQueue(): Promise<PushDeliveryResult[]> {
    if (!this.isRunning) return [];
  ...
  private async processRequest(request: PushDeliveryRequest): Promise<PushDeliveryResult> {
    const now = new Date();
  ...
  private shouldRetry(request: PushDeliveryRequest): boolean {
    const queuedRequest = request as QueuedRequest;
  ...
  async getStats(): Promise<DispatcherStats> {
...
export function createInMemoryPushQueueStore(): InMemoryPushQueueStore {
