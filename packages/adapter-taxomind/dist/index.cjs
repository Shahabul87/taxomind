...
__export(src_exports, {
  ...
  TAXOMIND_PROFILE_ID: () => TAXOMIND_PROFILE_ID,
  TAXOMIND_PROFILE_VERSION: () => TAXOMIND_PROFILE_VERSION,
  ...
  createTaxomindIntegrationProfile: () => createTaxomindIntegrationProfile,
  createTaxomindSAMVectorService: () => createTaxomindSAMVectorService,
  createTaxomindVectorService: () => createTaxomindVectorService,
  getTaxomindIntegration: () => getTaxomindIntegration,
  initializeTaxomindIntegration: () => initializeTaxomindIntegration,
  setTaxomindIntegration: () => setTaxomindIntegration,
  taxomindDataSources: () => taxomindDataSources,
  taxomindEntityMappings: () => taxomindEntityMappings,
  taxomindToolConfigurations: () => taxomindToolConfigurations
});
module.exports = __toCommonJS(src_exports);
var import_integration3 = require("@sam-ai/integration");
...
var taxomindToolConfigurations = {
  content: [
    {
      id: "course_lookup",
      enabled: true,
      permissionLevel: import_integration.ToolPermissionLevel.READ_ONLY,
  ...
  assessment: [
    ...
    },
    {
      ...
      enabled: true,
      permissionLevel: import_integration.ToolPermissionLevel.READ_WRITE,
...
var taxomindDataSources = [
  {
    type: import_integration.DataSourceType.CURRICULUM,
    enabled: true,
    cacheEnabled: true,
    cacheTTL: 3600,
    // 1 hour
  ...
  {
    type: import_integration.DataSourceType.USER_HISTORY,
    enabled: true,
    cacheEnabled: true,
    cacheTTL: 300,
    // 5 minutes
  ...
  {
    type: import_integration.DataSourceType.EXTERNAL_KNOWLEDGE,
    enabled: true,
    cacheEnabled: true,
    cacheTTL: 86400,
    // 24 hours
  ...
  {
    type: import_integration.DataSourceType.REAL_TIME,
    enabled: false,
...
function createTaxomindIntegrationProfile(options) {
  ...
  return {
    ...
    environment: {
      runtime: import_integration.RuntimeEnvironment.NODE,
      framework: import_integration.HostFrameworkType.NEXTJS,
    ...
    capabilities: {
      database: {
        available: true,
        type: import_integration.DatabaseType.PRISMA,
        supportsTransactions: true,
        supportsVectors: true,
        vectorAdapter: import_integration.VectorAdapterType.PGVECTOR,
        connectionPooling: true,
        maxConnections: 10
      },
      auth: {
        available: true,
        provider: import_integration.AuthProviderType.NEXTAUTH,
      ...
      ai: {
        available: true,
        chatProvider: import_integration.AIProviderType.ANTHROPIC,
        embeddingProvider: import_integration.EmbeddingProviderType.OPENAI,
      ...
      realtime: {
        available: true,
        type: import_integration.RealtimeType.WEBSOCKET,
...
var import_integration2 = require("@sam-ai/integration");
var NextAuthAdapter = class {
  ...
  async hasPermission(userId, permission) {
    ...
    for (const userRole of user.roles) {
      const rolePerms = import_integration2.DefaultRolePermissions[userRole] ?? [];
  ...
  async hasRole(userId, role) {
    ...
    const roleHierarchy = {
      [import_integration2.SAMRoles.ADMIN]: 100,
      [import_integration2.SAMRoles.TEACHER]: 50,
      [import_integration2.SAMRoles.STUDENT]: 25,
      [import_integration2.SAMRoles.GUEST]: 0
    };
  ...
  mapPrismaUserToSAMUser(user) {
    const userRole = user.role ?? import_integration2.SAMRoles.STUDENT;
    return {
      id: user.id,
      email: user.email ?? void 0,
      name: user.name ?? void 0,
      roles: [userRole],
      permissions: import_integration2.DefaultRolePermissions[userRole] ?? [],
      metadata: {
...
function initializeTaxomindIntegration(options) {
  ...
  const registry = new import_integration3.CapabilityRegistry(profile);
  const factory = new import_integration3.AdapterFactory(profile);
