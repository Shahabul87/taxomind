...
export type SocraticQuestionType =
  ...
  | 'probing_assumptions' // What are you assuming...?
  | 'probing_reasons' // Why do you think that...?
  | 'questioning_viewpoints' // What's an alternative view...?
  | 'probing_implications' // What are the consequences...?
  | 'questioning_the_question'; // Why is this question important...?
...
export interface SocraticDatabaseAdapter {
  ...
  getUserDialogues(userId: string, options?: { limit?: number; topic?: string }): Promise<SocraticDialogue[]>;
...
export interface SocraticTeachingEngine {
  ...
  endDialogue(dialogueId: string): Promise<{ synthesis: string; performance: DialoguePerformance }>;
