...
export type CognitiveDimensionName =
  | 'memory'
  | 'attention'
  | 'reasoning'
  | 'creativity'
...
export type BuddyPersonalityType =
  | 'motivator'
  | 'challenger'
  | 'supporter'
  | 'analyst'
...
// Innovation Engine Database Adapter
export interface InnovationDatabaseAdapter {
  ...
  storeQuantumObservation(
    pathId: string,
    observation: PathObservation
  ): Promise<void>;
  getQuantumObservations(pathId: string): Promise<PathObservation[]>;
  findLearningPeers(userId: string): Promise<{ pathId: string; userId: string }[]>;
}

export interface InnovationLearningData {
  userId: string;
  progress: Array<{ quizScore?: number; progressPercentage?: number; timeSpent?: number; courseId?: string }>;
  activities: Array<{ timestamp: Date; contentId?: string; contentType?: string; metadata?: Record<string, unknown> }>;
  achievements: Array<{ name: string; achievedAt: Date }>;
