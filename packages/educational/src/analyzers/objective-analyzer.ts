...
import {
  ObjectiveAnalysis,
  SMARTCriteriaAnalysis,
  CriterionScore,
  MeasurabilityAnalysis,
  ObjectiveCluster,
...
export class ObjectiveAnalyzer {
  private readonly STRONG_VERBS: Record<BloomsLevel, string[]> = {
   ...
   */
  private analyzeActionVerb(objective: string): ActionVerbAnalysis {
    const words = objective.toLowerCase().split(/\s+/);
    let foundVerb = '';
    let foundLevel: BloomsLevel = 'UNDERSTAND';
   ...
   */
  private determineDOKLevel(objective: string, bloomsLevel: BloomsLevel): WebbDOKLevel {
   ...
   */
  private analyzeSMARTCriteria(objective: string): SMARTCriteriaAnalysis {
  ...
  private analyzeSpecific(objective: string): CriterionScore {
  ...
  private analyzeMeasurable(objective: string): CriterionScore {
  ...
  private analyzeAchievable(objective: string): CriterionScore {
  ...
  private analyzeRelevant(objective: string): CriterionScore {
    let score = 50; // Base score
    const suggestions: string[] = [];
  ...
  private analyzeTimeBound(objective: string): CriterionScore {
   ...
   */
  private analyzeMeasurability(objective: string): MeasurabilityAnalysis {
    const lower = objective.toLowerCase();
    let score = 50;
  ...
  private calculateClarityScore(
    ...
    const startsWithVerb = /^[A-Z]?[a-z]+\s/.test(objective) &&
      BLOOMS_KEYWORD_MAP.some(m => m.keywords.some(k =>
        objective.toLowerCase().startsWith(k)
      ));
    if (startsWithVerb) {
      score += 10;
    }
  ...
  private generateSuggestions(
    objective: string,
    verbAnalysis: ActionVerbAnalysis,
    smartCriteria: SMARTCriteriaAnalysis,
    ...
    for (const [criterion, analysis] of Object.entries(smartCriteria)) {
      if (criterion !== 'overallScore' && typeof analysis === 'object' && 'suggestions' in analysis) {
        const criterionAnalysis = analysis as CriterionScore;
   ...
   */
  private clusterSimilarObjectives(objectives: string[]): ObjectiveCluster[] {
    const clusters: ObjectiveCluster[] = [];
    const processed: Set<number> = new Set();

    for (let i = 0; i < objectives.length; i++) {
      if (processed.has(i)) continue;

      const cluster: ObjectiveCluster = {
        clusterId: `cluster-${i}`,
   ...
   */
  private calculateSimilarity(obj1: string, obj2: string): number {
   ...
   */
  private generateMergedObjective(objectives: string[]): string {
    // Find the longest/most detailed one as base
    const base = objectives.reduce((longest, current) =>
   ...
   */
  private generateDeduplicationRecommendations(clusters: ObjectiveCluster[]): string[] {
   ...
   */
  private generateOptimizedObjectives(clusters: ObjectiveCluster[]): string[] {
