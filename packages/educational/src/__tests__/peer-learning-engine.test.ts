...
} from '../engines/peer-learning-engine';
...
function createSampleProfileInput(
  overrides: Partial<CreatePeerProfileInput> = {}
): CreatePeerProfileInput {
  return {
    userId: 'user-1',
    ...
    expertise: [
      {
        subject: 'Mathematics',
        topic: 'Algebra',
        proficiencyLevel: 'INTERMEDIATE' as PeerProficiencyLevel,
      },
      {
        subject: 'Programming',
        topic: 'JavaScript',
        proficiencyLevel: 'ADVANCED' as PeerProficiencyLevel,
      },
    ],
    learningGoals: [
      {
        subject: 'Physics',
        topic: 'Mechanics',
        targetLevel: 'INTERMEDIATE' as PeerProficiencyLevel,
        priority: 'HIGH' as PeerGoalPriority,
      },
    ],
    languages: ['en'],
...
function createSampleStudyGroupInput(
  overrides: Partial<CreateStudyGroupInput> = {}
): CreateStudyGroupInput {
  return {
...
function createSampleDiscussionInput(
  overrides: Partial<CreateDiscussionInput> = {}
): CreateDiscussionInput {
  return {
    groupId: 'group-1',
...
function createSampleMentorshipInput(
  overrides: Partial<RequestMentorshipInput> = {}
): RequestMentorshipInput {
  return {
    mentorId: 'mentor-1',
    menteeId: 'mentee-1',
...
function createSampleProjectInput(
...
function createEngineConfig(
  overrides: Partial<PeerLearningEngineConfig> = {}
): PeerLearningEngineConfig {
  return {
...
function createPeerLearningAIMock() {
  return createMockAIAdapter((params: AIChatParams) => {
...
describe('PeerLearningEngine', () => {
  ...
  describe('updatePeerProfile', () => {
    it('should update an existing profile', () => {
      ...
      const updateInput: UpdatePeerProfileInput = {
        userId: 'user-1',
  ...
  describe('findPeerMatches', () => {
    ...
    it('should find peer matches', () => {
      const criteria: PeerMatchCriteria = {
        matchType: 'STUDY_PARTNER',
      };
      const input: FindPeerMatchesInput = {
        userId: 'user-1',
        criteria,
      };
  ...
  describe('edge cases', () => {
    it('should handle profile with minimal input', () => {
      const input: CreatePeerProfileInput = {
        userId: 'minimal-user',
        displayName: 'Minimal',
        languages: ['en'],
      };
  ...
  describe('configuration', () => {
    it('should use different matching algorithms', () => {
      const simpleEngine = new PeerLearningEngine(
        samConfig,
        createEngineConfig({ matchingAlgorithm: 'SIMPLE' })
      );
    ...
    it('should respect maxGroupSize config', () => {
      const smallGroupEngine = new PeerLearningEngine(
        samConfig,
        createEngineConfig({ maxGroupSize: 3 })
      );

      smallGroupEngine.createPeerProfile(createSampleProfileInput({ userId: 'user-1' }));

      const group = smallGroupEngine.createStudyGroup(
        createSampleStudyGroupInput({ maxMembers: 3 })
      );
    ...
    it('should disable features via config', () => {
      const limitedEngine = new PeerLearningEngine(
        samConfig,
        createEngineConfig({
