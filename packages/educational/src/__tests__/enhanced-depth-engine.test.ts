...
} from '../engines/enhanced-depth-engine';

// ============================================================================
// SAMPLE DATA FACTORIES
// ============================================================================

function createSampleSection(overrides: Partial<SectionData> = {}): SectionData {
  return {
    id: 'section-1',
    title: 'Introduction',
...
function createSampleChapter(overrides: Partial<ChapterData> = {}): ChapterData {
  return {
    id: 'chapter-1',
...
function createSampleCourseData(overrides: Partial<CourseData> = {}): CourseData {
  return {
    id: 'course-1',
...
function createMockStorage(): CourseDepthAnalysisStore {
  return {
...
describe('EnhancedDepthAnalysisEngine', () => {
  let engine: EnhancedDepthAnalysisEngine;
  let mockStorage: CourseDepthAnalysisStore;
  ...
  describe('analyze', () => {
    ...
    it('should use cached analysis when available', async () => {
      ...
      (mockStorage.getCachedAnalysis as ReturnType<typeof vi.fn>).mockResolvedValue(cachedResult);
  ...
  describe('getHistoricalTrends', () => {
    ...
    it('should return snapshots from storage', async () => {
      ...
      (mockStorage.listHistoricalSnapshots as ReturnType<typeof vi.fn>).mockResolvedValue(mockSnapshots);
  ...
  describe('section analysis', () => {
    it('should detect video activities', async () => {
      ...
      const sectionsWithVideo = result.chapterAnalysis.flatMap((ch) =>
        ch.sections.filter((s) => s.activities.some((a) => a.type === 'Video Lesson'))
      );
    ...
    it('should detect assessment activities', async () => {
      ...
      const sectionsWithExams = result.chapterAnalysis.flatMap((ch) =>
        ch.sections.filter((s) => s.activities.some((a) => a.type === 'Assessment'))
      );
