...
import type { TrendsEngineConfig, TrendAnalysis, TrendCategory } from '../types';
...
function createTrendsConfig(overrides: Partial<TrendsEngineConfig> = {}): TrendsEngineConfig {
  return {
...
function createSampleTrend(overrides: Partial<TrendAnalysis> = {}): TrendAnalysis {
  return {
...
function createSampleCategory(overrides: Partial<TrendCategory> = {}): TrendCategory {
  return {
    id: 'cat-1',
...
describe('TrendsEngine', () => {
  let engine: TrendsEngine;
  let config: TrendsEngineConfig;
  ...
  describe('compareTrends', () => {
    it('should compare two trends', async () => {
      const trends = await engine.analyzeTrends();
      if (trends.length >= 2) {
        const comparison = await engine.compareTrends(
          trends[0].trendId,
          trends[1].trendId
        );
    ...
    it('should throw error for invalid trend IDs', async () => {
      await expect(
        engine.compareTrends('invalid-1', 'invalid-2')
      ).rejects.toThrow();
    });
  });
  ...
  describe('predictTrendTrajectory', () => {
    ...
    it('should support different prediction horizons', async () => {
      const trends = await engine.analyzeTrends();
      if (trends.length > 0) {
        const prediction6m = await engine.predictTrendTrajectory(
          trends[0].trendId,
          '6months'
        );
        const prediction2y = await engine.predictTrendTrajectory(
          trends[0].trendId,
          '2years'
        );
    ...
    it('should throw error for non-existent trend', async () => {
      await expect(
        engine.predictTrendTrajectory('non-existent-id', '1year')
      ).rejects.toThrow();
    });
  });
  ...
  describe('getEducationalTrends', () => {
    it('should return education-related trends', async () => {
      ...
      expect(Array.isArray(trends)).toBe(true);
      trends.forEach((trend) => {
        const hasEducationArea = trend.applicationAreas.some(
          (a) =>
  ...
  describe('recordInteraction', () => {
    it('should not throw without database adapter', async () => {
      await expect(
        engine.recordInteraction('user-1', 'trend-1', 'view')
      ).resolves.not.toThrow();
    });

    it('should handle different interaction types', async () => {
      await expect(
        engine.recordInteraction('user-1', 'trend-1', 'share')
      ).resolves.not.toThrow();
      await expect(
        engine.recordInteraction('user-1', 'trend-1', 'save')
      ).resolves.not.toThrow();
      await expect(
        engine.recordInteraction('user-1', 'trend-1', 'analyze')
      ).resolves.not.toThrow();
    });
  });
