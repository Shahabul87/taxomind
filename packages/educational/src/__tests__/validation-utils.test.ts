...
describe('createRetryPrompt', () => {
  ...
  it('should format zod errors when present', () => {
    const error = {
      message: 'Validation failed',
      zodErrors: [
        { path: ['name'], message: 'Required', code: 'invalid_type' as const, expected: 'string' as const, received: 'undefined' as const },
        { path: ['value'], message: 'Expected number', code: 'invalid_type' as const, expected: 'number' as const, received: 'string' as const },
      ],
      timestamp: new Date(),
    };
