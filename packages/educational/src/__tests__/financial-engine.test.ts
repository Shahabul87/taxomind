...
const createMockEngineConfig = (
  overrides: Partial<FinancialEngineConfig> = {}
): FinancialEngineConfig => ({
  samConfig: createMockSAMConfig(),
  ...overrides,
});

const createDateRange = (daysBack: number = 30): DateRange => {
  const end = new Date();
...
describe('FinancialEngine', () => {
    ...
    it('should create engine with database adapter', () => {
      const config = createMockEngineConfig({
        database: {
          query: vi.fn(),
          execute: vi.fn(),
        } as unknown as FinancialEngineConfig['database'],
      });
      const engine = new FinancialEngine(config);
  ...
  describe('Revenue Analysis', () => {
    ...
    it('should break down recurring vs one-time revenue', async () => {
      ...
      expect(
        analytics.revenue.recurringRevenue + analytics.revenue.oneTimeRevenue
      ).toBeLessThanOrEqual(analytics.revenue.totalRevenue * 1.01); // Allow small rounding
    });
  ...
  describe('Pricing Analysis', () => {
    ...
    it('should include discount strategies', async () => {
      ...
      expect(Array.isArray(analytics.pricing.currentPricing.discountStrategy)).toBe(
        true
      );
    });
  ...
  describe('Subscription Metrics', () => {
    ...
    it('should track active subscribers', async () => {
      ...
      expect(analytics.subscriptions.activeSubscribers).toBeLessThanOrEqual(
        analytics.subscriptions.totalSubscribers
      );
    });
  ...
  describe('Data Consistency', () => {
    ...
    it('should have consistent revenue breakdown', async () => {
      ...
      const sourceSum = revenue.revenueBySource.reduce(
    ...
    it('should have consistent cost breakdown', async () => {
      ...
      expect(costs.fixedCosts + costs.variableCosts).toBeLessThanOrEqual(
        costs.totalCosts * 1.01
      );
    });
    ...
    it('should have consistent subscription metrics', async () => {
      ...
      expect(subscriptions.activeSubscribers).toBeLessThanOrEqual(
        subscriptions.totalSubscribers
      );

      // Churn + retention should relate
      expect(subscriptions.churnRate + subscriptions.retentionRate).toBeGreaterThanOrEqual(
        0
      );
    });
