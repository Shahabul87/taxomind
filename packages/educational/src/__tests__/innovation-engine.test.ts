...
import type { InnovationEngineConfig } from '../types';

// ============================================================================
// SAMPLE DATA FACTORIES
// ============================================================================

function createInnovationEngineConfig(
  overrides: Partial<InnovationEngineConfig> = {}
...
describe('InnovationEngine', () => {
  let engine: InnovationEngine;
  let config: InnovationEngineConfig;
  ...
  describe('assessCognitiveFitness', () => {
    it('should throw error without database adapter', async () => {
      await expect(engine.assessCognitiveFitness('user-1')).rejects.toThrow(
        'Database adapter required for cognitive fitness assessment'
      );
    });
  });
  ...
  describe('generateLearningDNA', () => {
    it('should throw error without database adapter', async () => {
      await expect(engine.generateLearningDNA('user-1')).rejects.toThrow(
        'Database adapter required for Learning DNA generation'
      );
    });
  });
  ...
  describe('createStudyBuddy', () => {
    it('should throw error without database adapter', async () => {
      await expect(engine.createStudyBuddy('user-1')).rejects.toThrow(
        'Database adapter required for Study Buddy creation'
      );
    });

    it('should throw error with preferences but without database adapter', async () => {
      await expect(
        engine.createStudyBuddy('user-1', { personalityType: 'motivator' })
      ).rejects.toThrow('Database adapter required for Study Buddy creation');
    });
  });
  ...
  describe('interactWithBuddy', () => {
    it('should throw error without database adapter', async () => {
      await expect(
        engine.interactWithBuddy('buddy-1', 'user-1', 'conversation', {})
      ).rejects.toThrow('Database adapter required for buddy interaction');
    });
  });
  ...
  describe('createQuantumPath', () => {
    it('should throw error without database adapter', async () => {
      await expect(
        engine.createQuantumPath('user-1', 'Learn JavaScript')
      ).rejects.toThrow('Database adapter required for Quantum Path creation');
    });
  });
  ...
  describe('observeQuantumPath', () => {
    it('should throw error without database adapter', async () => {
      await expect(
        engine.observeQuantumPath('path-1', 'progress_check', { userId: 'user-1' })
      ).rejects.toThrow('Database adapter required for path observation');
    });
  });
