...
} from '../engines/knowledge-graph-engine';
import type {
...
function createEngineConfig(
  overrides: Partial<KnowledgeGraphEngineConfig> = {}
): KnowledgeGraphEngineConfig {
  return {
...
function createSampleConcept(overrides: Partial<Concept> = {}): Concept {
...
function createConceptHierarchy(): { concepts: Concept[]; relations: ConceptRelation[] } {
...
describe('KnowledgeGraphEngine', () => {
  ...
  describe('constructor', () => {
    ...
    it('should accept custom confidence threshold', () => {
      const customEngine = createKnowledgeGraphEngine(
        createEngineConfig({ confidenceThreshold: 0.5 })
      );
    ...
    it('should accept custom max prerequisite depth', () => {
      const customEngine = createKnowledgeGraphEngine(
        createEngineConfig({ maxPrerequisiteDepth: 5 })
      );
      expect(customEngine).toBeInstanceOf(KnowledgeGraphEngine);
    });
  });
  ...
  describe('extractConcepts', () => {
    it('should extract concepts from educational content', async () => {
      const result = await engine.extractConcepts({
        content:
          'Variables are fundamental containers for storing data. Data types define what kind of data can be stored. Functions are reusable blocks of code.',
    ...
    it('should extract relations between concepts', async () => {
      const result = await engine.extractConcepts({
        content:
          'Understanding variables is essential before learning about data types. Functions build upon control flow concepts.',
        contentType: 'SECTION',
      });
    ...
    it('should detect concept types from content', async () => {
      const result = await engine.extractConcepts({
        content:
          'This basic fundamental introduction covers the definition of variables. Learn how to implement functions step by step.',
        contentType: 'LEARNING_OBJECTIVE',
      });

      if (result.concepts.length > 0) {
        expect(['FOUNDATIONAL', 'PROCEDURAL', 'CONCEPTUAL', 'METACOGNITIVE']).toContain(
          result.concepts[0].type
        );
      }
    });
    ...
    it('should detect Blooms levels from content', async () => {
      const result = await engine.extractConcepts({
        content:
          'Define the key terms. Explain how algorithms work. Apply these concepts to solve problems.',
        contentType: 'QUIZ',
      });
    ...
    it('should deduplicate extracted concepts', async () => {
      const result = await engine.extractConcepts({
        content:
          'Variables are important. Variables store data. Variables can change.',
        contentType: 'COURSE_DESCRIPTION',
      });

      const variablesConcepts = result.concepts.filter((c) =>
        c.name.toLowerCase().includes('variables')
      );
  ...
  describe('analyzePrerequisites', () => {
    ...
    it('should identify bottleneck concepts', async () => {
      ...
      const controlFlowNode = result.prerequisiteChain.find(
        (n) => n.concept.id === 'concept-control-flow'
      );
      if (controlFlowNode) {
  ...
  describe('generateLearningPath', () => {
    ...
    it('should use FASTEST strategy', async () => {
      ...
      expect(fastPath.totalEstimatedTime).toBeLessThanOrEqual(
        balancedPath.totalEstimatedTime
      );
    });
    ...
    it('should use THOROUGH strategy', async () => {
      ...
      expect(thoroughPath.totalEstimatedTime).toBeGreaterThanOrEqual(
        balancedPath.totalEstimatedTime
      );
    });
    ...
    it('should skip mastered concepts when requested', async () => {
      ...
      const hasVariables = path.sequence.some(
        (n) => n.concept.id === 'concept-variables'
      );
  ...
  describe('updateConceptMastery', () => {
    ...
    it('should update mastery after practice', async () => {
      const mastery = await engine.updateConceptMastery(
        'user-1',
        'concept-variables',
        80,
        'PRACTICE'
    ...
    it('should handle different evidence types', async () => {
      ...
      await engine.updateConceptMastery(
        'user-1',
        'concept-variables',
        85,
        'ASSIGNMENT'
  ...
  describe('edge cases', () => {
    ...
    it('should handle multiple graphs for different courses', () => {
      ...
      expect(engine.getGraph('course-programming')?.concepts.length).not.toBe(
        engine.getGraph('course-art')?.concepts.length
      );
    });
