...
} from '../engines/multimodal-input-engine';
import {
  createMockSAMConfig,
  createMockAIAdapter,
...
function createSampleImageFile(overrides: Partial<MultimodalFile> = {}): MultimodalFile {
  return {
...
function createSampleAudioFile(overrides: Partial<MultimodalFile> = {}): MultimodalFile {
  return {
...
function createSampleHandwritingFile(
  overrides: Partial<MultimodalFile> = {}
): MultimodalFile {
  return {
...
function createSampleProcessInput(
  overrides: Partial<ProcessMultimodalInput> = {}
): ProcessMultimodalInput {
  return {
...
function createMultimodalConfig(
  overrides: Partial<MultimodalConfig> = {}
): Partial<MultimodalConfig> {
  return {
...
describe('MultimodalInputEngine', () => {
  ...
  describe('assessQuality', () => {
    ...
    it('should return quality level based on score', async () => {
      ...
      expect(['EXCELLENT', 'GOOD', 'ACCEPTABLE', 'POOR', 'UNREADABLE']).toContain(
        result.level
      );
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle processing with minimal options', async () => {
      const input: ProcessMultimodalInput = {
        file: createSampleImageFile(),
        options: {},
        userId: 'user-1',
      };
  ...
  describe('configuration', () => {
    it('should respect disabled OCR config', async () => {
      const noOCREngine = new MultimodalInputEngine(
        samConfig,
        createMultimodalConfig({ enableOCR: false })
      );
    ...
    it('should respect disabled AI analysis config', async () => {
      const noAIEngine = new MultimodalInputEngine(
        samConfig,
        createMultimodalConfig({ enableAIAnalysis: false })
      );
    ...
    it('should respect quality threshold config', async () => {
      const strictEngine = new MultimodalInputEngine(
        samConfig,
        createMultimodalConfig({ qualityThreshold: 90 })
      );
