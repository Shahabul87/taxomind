...
const createMockDatabaseAdapter = (
  overrides: Partial<AchievementDatabaseAdapter> = {}
): AchievementDatabaseAdapter => ({
...
const createMockEngineConfig = (
  overrides: Partial<AchievementEngineConfig> = {}
): AchievementEngineConfig => ({
...
const createCustomAchievement = (overrides: Partial<Achievement> = {}): Achievement => ({
...
const createCustomChallenge = (overrides: Partial<Challenge> = {}): Challenge => ({
  id: 'custom_challenge',
...
describe('AchievementEngine', () => {
  ...
  describe('Track Progress', () => {
    let engine: AchievementEngine;
    let mockDatabase: AchievementDatabaseAdapter;

    beforeEach(() => {
      mockDatabase = createMockDatabaseAdapter();
      engine = new AchievementEngine(
        createMockEngineConfig({ database: mockDatabase })
      );
    });
    ...
    it('should record interaction when tracking progress', async () => {
      ...
      expect(mockDatabase.recordInteraction).toHaveBeenCalledWith(
    ...
    it('should update streak for streak actions', async () => {
      ...
      expect(mockDatabase.updateStreak).toHaveBeenCalledWith(
        'user-123',
        expect.objectContaining({
    ...
    it('should award level up bonus on level up', async () => {
      ...
      const calls = (mockDatabase.awardPoints as ReturnType<typeof vi.fn>).mock.calls;
      const levelUpCall = calls.find(
        (call: unknown[]) => (call[1] as { source: string }).source === 'level_up'
      );
  ...
  describe('Challenges', () => {
    ...
    describe('startChallenge', () => {
      it('should start a challenge successfully', async () => {
        ...
        expect(mockDatabase.updateUserChallenges).toHaveBeenCalledWith(
          'user-123',
          expect.objectContaining({
            activeChallenges: ['daily_learning'],
          })
  ...
  describe('Integration Scenarios', () => {
    ...
    it('should accumulate points across multiple actions', async () => {
      ...
      mockDatabase.getUserBadges = vi.fn().mockResolvedValue(
        result1.achievementsUnlocked.map(a => ({ description: a.id }))
      );
