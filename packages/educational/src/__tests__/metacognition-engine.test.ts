...
} from '../engines/metacognition-engine';
...
} as unknown as SAMConfig;

const createDefaultConfig = (overrides?: Partial<MetacognitionEngineConfig>): MetacognitionEngineConfig => ({
...
describe('MetacognitionEngine', () => {
  ...
  describe('Constructor and Initialization', () => {
    ...
    it('should accept custom reflection depth', () => {
      const customEngine = createMetacognitionEngine(
        createDefaultConfig({ defaultReflectionDepth: 'DEEP' })
      );
      expect(customEngine).toBeInstanceOf(MetacognitionEngine);
    });

    it('should accept AI reflection enabled config', () => {
      const aiEnabledEngine = createMetacognitionEngine(
        createDefaultConfig({ enableAIReflection: true })
      );
  ...
  describe('generateReflection', () => {
    ...
    it('should assign correct target skills for each reflection type', async () => {
      const skillMap: Record<ReflectionType, string> = {
  ...
  describe('analyzeReflection', () => {
    const createMockPrompt = (): ReflectionPrompt => ({
      id: 'prompt-1',
      type: 'POST_LEARNING',
      depth: 'MODERATE',
    ...
    const createMockResponse = (text: string): ReflectionResponse => ({
      promptId: 'prompt-1',
      userId: 'user-1',
      response: text,
    ...
    it('should assess MODERATE depth for detailed responses', async () => {
      ...
      const response = createMockResponse(
        'I realized that algebra is really about patterns. For example, when solving equations, ' +
    ...
    it('should assess DEEP depth for comprehensive responses', async () => {
      const prompt = createMockPrompt();
      const response = createMockResponse(
        'I realized that understanding algebra requires seeing the connections between different concepts. ' +
    ...
    it('should extract key insights from response', async () => {
      const prompt = createMockPrompt();
      const response = createMockResponse(
        'I realized that practice is important. I discovered that examples help me learn better. ' +
    ...
    it('should identify growth areas for struggling learners', async () => {
      const prompt = createMockPrompt();
      const response = createMockResponse(
        'I struggle with time management and often feel confused about the concepts. ' +
    ...
    it('should calculate quality score based on depth and time', async () => {
      const prompt = createMockPrompt();
      const response = createMockResponse(
        'I realized that this topic is important. For example, when I was studying, ' +
  ...
  describe('recordStudySession', () => {
    ...
    it('should record session with strategies', () => {
      ...
      const strategies: LearningStrategy[] = ['SPACED_PRACTICE', 'RETRIEVAL_PRACTICE'];
      const session = engine.recordStudySession({
        userId: 'user-1',
        courseId: 'course-1',
        startedAt,
    ...
    it('should record session with outcome data', () => {
      ...
      const outcome = {
        goalsAchieved: true,
        comprehensionLevel: 4 as ConfidenceLevel,
        satisfactionLevel: 5 as ConfidenceLevel,
    ...
    beforeEach(() => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        engine.recordStudySession({
          ...
          outcome: { goalsAchieved: true, comprehensionLevel: (3 + (i % 2)) as ConfidenceLevel, satisfactionLevel: 4 as ConfidenceLevel },
        });
      }
    });
  ...
  describe('recommendStrategies', () => {
    ...
    it('should recommend based on Blooms level', () => {
      ...
      const hasBloomsRelated = result.recommendations.some(rec =>
        rec.reason.includes('APPLY') || rec.reason.includes('underutilized')
      );
  ...
  describe('assessCognitiveLoad', () => {
    ...
    it('should accept self-reported load level', () => {
      const levels: CognitiveLoadLevel[] = ['LOW', 'OPTIMAL', 'HIGH', 'OVERLOAD'];
    ...
    it('should estimate load based on session history', () => {
      ...
      for (let i = 0; i < 5; i++) {
        engine.recordStudySession({
          ...
          topicsCovered: [`topic-${i}`],
          outcome: { goalsAchieved: false, comprehensionLevel: 2 as ConfidenceLevel, satisfactionLevel: 2 as ConfidenceLevel }, // Low comprehension
        });
      }
  ...
  describe('getMetacognitiveAssessment', () => {
    ...
    it('should calculate overall score as average of skills', () => {
      ...
      const expectedAverage = Math.round(
        assessment.skills.reduce((sum, s) => sum + s.score, 0) / assessment.skills.length
      );
    ...
    it('should improve scores based on user activity', () => {
      ...
      for (let i = 0; i < 5; i++) {
        engine.recordStudySession({
          ...
          outcome: { goalsAchieved: true, comprehensionLevel: 4 as ConfidenceLevel, satisfactionLevel: 4 as ConfidenceLevel },
  ...
  describe('recordIntervention', () => {
    it('should record an emotional intervention', () => {
      const intervention = engine.recordIntervention(
        'user-1',
        'EMOTIONAL',
        'Feeling frustrated with difficult problem',
  ...
  describe('AI Reflection Generation', () => {
    it('should handle AI reflection when AI is not configured', async () => {
      const aiEngine = createMetacognitionEngine(
        createDefaultConfig({ enableAIReflection: true })
      );
    ...
    it('should extract themes from previous reflections', async () => {
      ...
      const previousReflections: ReflectionResponse[] = [
        {
          promptId: 'p1',
          userId: 'user-1',
