...
function createSocialEngineConfig(
  overrides: Partial<SocialEngineConfig> = {}
): SocialEngineConfig {
  return {
    ...overrides,
  };
...
function createSampleGroup(): SocialLearningGroup {
  return {
    id: 'group-1',
...
function createSampleCommunity(): SocialCommunity {
  return {
    id: 'community-1',
    name: 'Test Community',
    memberCount: 100,
...
function createSampleInteractions(): SocialInteraction[] {
  return [
    {
      id: 'int-1',
      userId: 'user-1',
...
describe('SocialEngine', () => {
  ...
  describe('matchMentorMentee', () => {
    it('should throw error without database adapter', async () => {
      ...
      await expect(engine.matchMentorMentee(users)).rejects.toThrow(
