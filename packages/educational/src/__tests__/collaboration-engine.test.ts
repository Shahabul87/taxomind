...
} from '../engines/collaboration-engine';
import type { CollaborationEngineConfig, CollaborationActivityType } from '../types';
...
function createCollaborationConfig(
  overrides: Partial<CollaborationEngineConfig> = {}
...
describe('CollaborationEngine', () => {
  ...
  let config: CollaborationEngineConfig;
  ...
  describe('startCollaborationSession', () => {
    it('should throw error without database adapter', async () => {
      await expect(
        engine.startCollaborationSession('course-1', 'chapter-1', 'user-1', 'discussion')
  ...
  describe('joinCollaborationSession', () => {
    it('should throw error for non-existent session', async () => {
      await expect(
        engine.joinCollaborationSession('non-existent-session', 'user-1')
  ...
  describe('recordContribution', () => {
    it('should throw error for non-existent session', async () => {
      await expect(
        engine.recordContribution('non-existent-session', 'user-1', {
          type: 'message',
          content: { text: 'Hello!' },
          impact: 0.5,
        })
  ...
  describe('analyzeCollaboration', () => {
    it('should throw error for non-existent session without database', async () => {
      await expect(engine.analyzeCollaboration('non-existent-session')).rejects.toThrow(
        'Session not found'
      );
    });
  });
  ...
  describe('endCollaborationSession', () => {
    it('should throw error for non-existent session', async () => {
      await expect(
        engine.endCollaborationSession('non-existent-session')
