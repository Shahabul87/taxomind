...
} from '../engines/competency-engine';
import type {
...
function createEngineConfig(
  overrides: Partial<CompetencyEngineConfig> = {}
): CompetencyEngineConfig {
  return {
...
function createSampleSkillInput(overrides: Record<string, unknown> = {}) {
  return {
    name: 'Test Skill',
...
function createSampleSkillTreeInput(
  overrides: Partial<CreateSkillTreeInput> = {}
): CreateSkillTreeInput {
  return {
...
function createSamplePortfolioInput(
  overrides: Partial<AddPortfolioItemInput> = {}
): AddPortfolioItemInput {
  return {
    userId: 'user-1',
    type: 'PROJECT',
...
function createSampleAssessmentItems(): AssessmentItem[] {
  return [
    {
      id: 'item-1',
...
describe('CompetencyEngine', () => {
  ...
  describe('createSkill', () => {
    ...
    it('should include tags on created skill', () => {
      const skill = engine.createSkill(
        createSampleSkillInput({ tags: ['custom', 'tags'] })
      );
  ...
  describe('addPortfolioItem', () => {
    ...
    it('should update user proficiency based on portfolio evidence', () => {
      engine.addPortfolioItem(
        createSamplePortfolioInput({
          demonstratedSkills: [
            {
              skillId: 'skill-react',
              proficiency: 'PROFICIENT',
  ...
  describe('extractSkills', () => {
    it('should extract skills from job posting content', async () => {
      const result = await engine.extractSkills({
        content:
          'Looking for a JavaScript developer with React experience and knowledge of TypeScript',
    ...
    it('should match to existing skills when possible', async () => {
      ...
      const jsSkill = result.skills.find((s) =>
  ...
  describe('getHoursToReachProficiency', () => {
    ...
    it('should adjust based on skill learning hours', () => {
      const skill = engine.createSkill(
        createSampleSkillInput({ typicalLearningHours: 80 })
      );

      const hoursWithSkill = engine.getHoursToReachProficiency(
        'BEGINNER',
        'COMPETENT',
        skill
      );
      const hoursWithoutSkill = engine.getHoursToReachProficiency(
        'BEGINNER',
        'COMPETENT'
      );
