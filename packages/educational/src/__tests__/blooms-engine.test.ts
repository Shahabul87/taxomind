...
import type { BloomsAnalysisConfig, CourseAnalysisInput } from '../types';
...
function createSampleCourseInput(
  overrides: Partial<CourseAnalysisInput> = {}
): CourseAnalysisInput {
  return {
    id: 'course-1',
...
function createSampleCourseWithQuestions(): CourseAnalysisInput {
  return {
...
function createSampleCourseWithExams(): CourseAnalysisInput {
  return {
...
describe('BloomsAnalysisEngine', () => {
  ...
  let config: BloomsAnalysisConfig;
  ...
  describe('analyzeContent', () => {
    ...
    it('should use AI for comprehensive depth analysis', async () => {
      const aiConfig: BloomsAnalysisConfig = {
        samConfig: createMockSAMConfig(),
  ...
  describe('analyzeCourse', () => {
    ...
    it('should handle empty course', async () => {
      const courseData: CourseAnalysisInput = {
        id: 'empty-course',
        title: 'Empty Course',
        chapters: [],
      };
  ...
  describe('updateCognitiveProgress', () => {
    it('should handle missing database gracefully', async () => {
      ...
      await expect(
        engine.updateCognitiveProgress('user-1', 'section-1', 'APPLY', 85)
  ...
  describe('logLearningActivity', () => {
    it('should handle missing database gracefully', async () => {
      // Should not throw without database
      await expect(
        engine.logLearningActivity('user-1', 'quiz_completed', { score: 90 })
      ).resolves.toBeUndefined();
    });
  });
  ...
  describe('createProgressIntervention', () => {
    it('should handle missing database gracefully', async () => {
      // Should not throw without database
      await expect(
        engine.createProgressIntervention(
          'user-1',
          'struggling',
          'Needs Help',
          'Student appears to be struggling',
          { severity: 'high' }
        )
      ).resolves.toBeUndefined();
    });
  ...
  describe('factory function', () => {
    it('should create engine with basic config', () => {
      const basicConfig: BloomsAnalysisConfig = {
        samConfig: createMockSAMConfig(),
      };
    ...
    it('should create engine with quick analysis depth', () => {
      const quickConfig: BloomsAnalysisConfig = {
        samConfig: createMockSAMConfig(),
        analysisDepth: 'quick',
      };
    ...
    it('should create engine with comprehensive analysis depth', () => {
      const comprehensiveConfig: BloomsAnalysisConfig = {
        samConfig: createMockSAMConfig(),
        analysisDepth: 'comprehensive',
      };
    ...
    it('should create engine with custom AI adapter', () => {
      const customAdapter = createMockAIAdapter(() =>
        createMockAIResponse(JSON.stringify({
      ...
      const customConfig: BloomsAnalysisConfig = {
        samConfig: {
          ...createMockSAMConfig(),
          ai: customAdapter,
        },
  ...
  describe('AI integration', () => {
    it('should use AI for comprehensive course analysis', async () => {
      ...
      const trackingConfig: BloomsAnalysisConfig = {
        samConfig: {
          ...createMockSAMConfig(),
          ai: trackingAdapter,
        },
    ...
    it('should fall back to keywords when AI fails', async () => {
      ...
      const errorConfig: BloomsAnalysisConfig = {
        samConfig: {
          ...createMockSAMConfig(),
          ai: errorAdapter,
        },
