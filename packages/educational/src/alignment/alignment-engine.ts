...
const DOK_KEYWORDS: Record<WebbDOKLevel, string[]> = {
...
const DEFAULT_OPTIONS: Required<Omit<AlignmentEngineOptions, 'logger'>> = {
...
export class AlignmentEngine {
  private options: Required<Omit<AlignmentEngineOptions, 'logger'>>;
  private logger: AlignmentLogger | undefined;
   ...
   */
  async analyzeAlignment(course: CourseForAlignment): Promise<AlignmentAnalysisResult> {
    ...
    const sectionAlignments = this.mapSectionsToAssessments(
      course.sections,
      course.assessments,
      objectiveAlignments
    );
  ...
  private mapSectionsToAssessments(
    ...
    return sections.map((section) => {
      ...
      const bloomsDistribution = section.bloomsDistribution || this.analyzeBloomsDistribution(
  ...
  private mapAssessmentsToObjectives(
    ...
    return assessments.map((assessment) => {
      const questionAlignments: QuestionAlignment[] = assessment.questions.map((question) => {
   ...
   */
  private calculateCoverageScore(alignments: ObjectiveAlignment[]): number {
    if (alignments.length === 0) return 0;

    const coveredCount = alignments.filter(
      (a) => a.hasSectionCoverage && a.hasAssessmentCoverage
    ).length;

    const partialCount = alignments.filter(
      (a) => a.hasSectionCoverage || a.hasAssessmentCoverage
    ).length;
   ...
   */
  private calculateRedundancyScore(alignments: ObjectiveAlignment[]): number {
    if (alignments.length === 0) return 0;
    ...
    for (const alignment of alignments) {
      ...
      const directSections = alignment.linkedSections.filter(
        (s) => s.alignmentType === 'direct'
      ).length;
   ...
   * Calculate overall alignment score
   */
  private calculateOverallAlignmentScore(coverage: number, redundancy: number): number {
  ...
  private identifyGaps(
    ...
    for (const alignment of objectiveAlignments) {
      ...
      for (const section of alignment.linkedSections) {
        ...
        if (sectionAlignment && !this.bloomsLevelsCompatible(
          alignment.bloomsLevel,
          sectionAlignment.primaryBloomsLevel
        )) {
          gaps.push({
    ...
    const objectiveCoverageCount = new Map<string, number>();
    for (const alignment of objectiveAlignments) {
      const directSectionCount = alignment.linkedSections.filter(
        (s) => s.alignmentType === 'direct'
      ).length;
    ...
    // Sort by severity
    const severityOrder: Record<GapSeverity, number> = { critical: 0, warning: 1, info: 2 };
  ...
  private updateObjectiveAssessmentLinks(
    ...
    for (const objective of objectiveAlignments) {
      const linkedAssessments: LinkedAssessment[] = [];
      ...
      for (const assessment of assessmentAlignments) {
        if (assessment.objectivesCovered.includes(objective.objectiveId)) {
          ...
          const coverageScore =
            linkedQuestionIds.length / Math.max(1, assessment.questionAlignments.length);

          // Check Bloom's match
          const questionBlooms = assessment.questionAlignments
            .filter((qa) => qa.linkedObjectives.includes(objective.objectiveId))
            .map((qa) => qa.bloomsLevel);
          const bloomsMatch = questionBlooms.some(
            (bl) => this.bloomsLevelsCompatible(objective.bloomsLevel, bl)
          );
          ...
          const questionDoks = assessment.questionAlignments
            .filter((qa) => qa.linkedObjectives.includes(objective.objectiveId))
            .map((qa) => qa.dokLevel);
          const dokMatch = questionDoks.some(
            (dok) => Math.abs(dok - objective.dokLevel) <= 1
          );
   ...
   */
  private buildSummary(alignments: ObjectiveAlignment[]): AlignmentSummary {
    const totalObjectives = alignments.length;
    const fullyCoveredObjectives = alignments.filter((a) => a.isFullyCovered).length;
    const partialObjectives = alignments.filter(
      (a) => !a.isFullyCovered && (a.hasSectionCoverage || a.hasAssessmentCoverage)
    ).length;
    const uncoveredObjectives = alignments.filter(
      (a) => !a.hasSectionCoverage && !a.hasAssessmentCoverage
    ).length;
   ...
   */
  private matchText(extraction: KeywordExtraction, targetText: string): TextMatchResult {
    const normalizedTarget = targetText.toLowerCase();

    const matchedKeywords = extraction.keywords.filter((kw) =>
      normalizedTarget.includes(kw)
    );

    const matchedPhrases = extraction.phrases.filter((phrase) =>
   ...
   */
  private getSectionText(section: CourseSection): string {
   ...
   */
  private inferDOKLevel(text: string): WebbDOKLevel {
    const normalizedText = text.toLowerCase();
    const scores: Record<WebbDOKLevel, number> = { 1: 0, 2: 0, 3: 0, 4: 0 };

    for (const [level, keywords] of Object.entries(DOK_KEYWORDS)) {
      for (const keyword of keywords) {
        if (normalizedText.includes(keyword)) {
          scores[parseInt(level) as WebbDOKLevel]++;
        }
      }
    }

    // Find level with highest score
    let maxLevel: WebbDOKLevel = 2;
    let maxScore = 0;

    for (const [level, score] of Object.entries(scores)) {
      if (score > maxScore) {
        maxScore = score;
        maxLevel = parseInt(level) as WebbDOKLevel;
      }
    }
   ...
   */
  private analyzeBloomsDistribution(text: string): BloomsDistribution {
    const normalizedText = text.toLowerCase();
    ...
    for (const [level, keywords] of Object.entries(BLOOMS_KEYWORDS)) {
      for (const keyword of keywords) {
        ...
        if (matches) {
          counts[level as BloomsLevel] += matches.length;
        }
      }
    }
    ...
    if (total > 0) {
      for (const level of Object.keys(counts)) {
        counts[level as BloomsLevel] = Math.round((counts[level as BloomsLevel] / total) * 100);
      }
    }
   ...
   */
  private getAssessmentBloomsLevels(assessments: CourseAssessment[]): BloomsLevel[] {
   ...
   */
  private checkBloomsMatch(sectionLevel: BloomsLevel, assessmentLevels: BloomsLevel[]): boolean {
   ...
   */
  private bloomsLevelsCompatible(level1: BloomsLevel, level2: BloomsLevel): boolean {
    const levelOrder: BloomsLevel[] = ['REMEMBER', 'UNDERSTAND', 'APPLY', 'ANALYZE', 'EVALUATE', 'CREATE'];
   ...
   */
  private calculateBloomsFromQuestions(questions: QuestionAlignment[]): BloomsDistribution {
    const counts: BloomsDistribution = {
      REMEMBER: 0,
      UNDERSTAND: 0,
      APPLY: 0,
 ...
 */
export function createAlignmentEngine(options?: AlignmentEngineOptions): AlignmentEngine {
