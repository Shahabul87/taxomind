...
export interface SemanticClassificationResult {
  level: BloomsLevel;
  confidence: number;
  similarityScores: Record<BloomsLevel, number>;
 ...
 */
export const AMBIGUOUS_VERBS: Record<string, BloomsLevel[]> = {
 ...
 */
export const REFERENCE_PHRASES: Record<BloomsLevel, string[]> = {
  REMEMBER: [
...
export class SemanticBloomsClassifier {
  private readonly config: Required<SemanticClassifierConfig>;
  private readonly embeddingCache: Map<string, EmbeddingCacheEntry> = new Map();
  private referenceEmbeddings: Map<BloomsLevel, number[][]> = new Map();
  private referenceEmbeddingsLoaded = false;
  private loadingPromise: Promise<void> | null = null;

  constructor(config: SemanticClassifierConfig = {}) {
    this.config = {
      minSimilarityThreshold: config.minSimilarityThreshold ?? 0.75,
      embeddingProvider: config.embeddingProvider ?? null as unknown as EmbeddingProvider,
  ...
  setEmbeddingProvider(provider: EmbeddingProvider): void {
    (this.config as { embeddingProvider: EmbeddingProvider }).embeddingProvider = provider;
  }
   ...
   */
  getPossibleLevels(verb: string): BloomsLevel[] {
    const normalizedVerb = verb.toLowerCase().trim();
   ...
   */
  async loadReferenceEmbeddings(): Promise<void> {
  ...
  private async doLoadReferenceEmbeddings(): Promise<void> {
    const provider = this.config.embeddingProvider;
    const allPhrases: string[] = [];
    const levelIndices: Map<BloomsLevel, { start: number; end: number }> = new Map();
   ...
   */
  async classify(content: string): Promise<SemanticClassificationResult> {
    ...
    const similarityScores: Record<BloomsLevel, number> = {
      REMEMBER: 0,
      UNDERSTAND: 0,
      APPLY: 0,
      ANALYZE: 0,
 ...
 */
export function createSemanticBloomsClassifier(
 ...
 */
export function createSemanticBloomsClassifierWithProvider(
  embeddingProvider: EmbeddingProvider,
  config?: Omit<SemanticClassifierConfig, 'embeddingProvider'>
): SemanticBloomsClassifier {
