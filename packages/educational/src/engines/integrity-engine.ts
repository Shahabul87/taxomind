...
export class IntegrityEngine {
  ...
  private database?: IntegrityDatabaseAdapter;
  ...
  private detectAnomalies(current: StyleMetrics, average: StyleMetrics): StyleAnomaly[] {
    ...
    if (
      current.vocabularyLevel > average.vocabularyLevel * 1.3 &&
  ...
  private calculateOverallRisk(report: IntegrityReport): IntegrityRiskLevel {
    let riskScore = 0;
  ...
  async runBatchIntegrityCheck(
    ...
    const corpus: CorpusEntry[] = submissions.map(s => ({
      id: s.answerId,
      content: s.text,
...
export function createIntegrityEngine(config?: IntegrityEngineConfig): IntegrityEngine {
