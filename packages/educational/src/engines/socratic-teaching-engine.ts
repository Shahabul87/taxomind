...
} from '../types/socratic-teaching.types';
...
export class SocraticTeachingEngine {
  private database?: SocraticDatabaseAdapter;
  private aiAdapter?: SocraticTeachingConfig['aiAdapter'];
  private dialogueCache: Map<string, SocraticDialogue> = new Map();

  constructor(private config: SocraticTeachingConfig = {}) {
   ...
   */
  async startDialogue(input: StartDialogueInput): Promise<SocraticResponse> {
    const {
      userId,
      topic,
      learningObjective = `Understand the key concepts and implications of ${topic}`,
      priorKnowledge,
      targetBloomsLevel = 'ANALYZE' as BloomsLevel,
   ...
   */
  async continueDialogue(input: ContinueDialogueInput): Promise<SocraticResponse> {
   ...
   */
  async getHint(dialogueId: string, hintIndex: number = 0): Promise<string> {
  ...
  async endDialogue(
    dialogueId: string
  ): Promise<{ synthesis: string; performance: DialoguePerformance }> {
    const dialogue = this.dialogueCache.get(dialogueId);
    if (!dialogue) {
      throw new Error(`Dialogue ${dialogueId} not found`);
    }
   ...
   * Get dialogue by ID
   */
  async getDialogue(dialogueId: string): Promise<SocraticDialogue | null> {
    let dialogue = this.dialogueCache.get(dialogueId);
    if (!dialogue && this.database) {
      dialogue = await this.database.getDialogue(dialogueId) || undefined;
    }
   ...
   */
  async getUserDialogues(userId: string, limit: number = 10): Promise<SocraticDialogue[]> {
    if (this.database) {
  ...
  async generateQuestion(
    ...
    context?: { previousQuestions?: string[]; currentUnderstanding?: string }
...
`;
    ...
    try {
      ...
      return {
        ...
        bloomsLevel: (parsed.bloomsLevel?.toUpperCase() as BloomsLevel) || ('ANALYZE' as BloomsLevel),
  ...
  private generateTemplateQuestion(
    ...
    const templates: Record<SocraticQuestionType, (topic: string) => SocraticQuestion> = {
      clarifying: (t) => ({
        id: `q_${Date.now()}`,
        type: 'clarifying',
      ...
      questioning_viewpoints: (t) => ({
        ...
        bloomsLevel: 'EVALUATE' as BloomsLevel,
...
`;
    ...
    try {
      ...
      return {
        ...
        demonstratedBloomsLevel: (parsed.demonstratedBloomsLevel?.toUpperCase() as BloomsLevel) || ('UNDERSTAND' as BloomsLevel),
      };
    } catch {
      return this.analyzeWithRules(question, response);
    }
  ...
  private analyzeWithRules(question: SocraticQuestion, response: string): ResponseAnalysis {
    ...
    const keyTermsFound = question.keyInsights.some((insight) =>
    ...
    let recommendedNextType: SocraticQuestionType = 'probing_reasons';
    if (qualityScore < 40) {
    ...
    let demonstratedBloomsLevel: BloomsLevel = 'UNDERSTAND';
  ...
  private async generateKeyInsights(topic: string, objective: string): Promise<string[]> {
    if (this.aiAdapter) {
      try {
        const response = await this.aiAdapter.chat({
          messages: [
  ...
  private async generateSynthesis(dialogue: SocraticDialogue): Promise<string> {
  ...
  private calculatePerformance(dialogue: SocraticDialogue): DialoguePerformance {
    const exchanges = dialogue.exchanges;
    const totalExchanges = exchanges.length;
    const exchangesWithResponses = exchanges.filter((e) => e.response);

    const avgQuality =
      exchangesWithResponses.reduce((sum, e) => sum + (e.analysis?.qualityScore || 0), 0) /
      (exchangesWithResponses.length || 1);

    const avgDepth =
      exchangesWithResponses.reduce((sum, e) => sum + (e.analysis?.thinkingDepth || 0), 0) /
      (exchangesWithResponses.length || 1);
    ...
    let highestBloomsLevel: BloomsLevel = 'REMEMBER';
    ...
    const growth: { factor: string; description: string }[] = [];
    if (avgQuality > 70) {
  ...
  private calculateProgress(dialogue: SocraticDialogue): number {
  ...
  private shouldConclude(dialogue: SocraticDialogue, analysis: ResponseAnalysis): boolean {
  ...
  private concludeDialogue(dialogue: SocraticDialogue): SocraticResponse {
    dialogue.state = 'conclusion';
    dialogue.endedAt = new Date();
  ...
  private async moveToNextQuestion(
    ...
    // Determine next question type
    const nextType: SocraticQuestionType = analysis?.recommendedNextType || this.getNextQuestionType(dialogue);
  ...
  private getNextQuestionType(dialogue: SocraticDialogue): SocraticQuestionType {
  ...
  private determineDialogueState(dialogue: SocraticDialogue): DialogueState {
  ...
  private generateFeedback(analysis: ResponseAnalysis, encouraging: boolean): string {
    let feedback = '';
  ...
  private getIntroductionMessage(topic: string, style: 'gentle' | 'challenging' | 'balanced'): string {
    switch (style) {
      case 'gentle':
  ...
  private getEncouragement(context: DialogueState | 'start' | 'completion'): string {
 ...
 */
export function createSocraticTeachingEngine(config?: SocraticTeachingConfig): SocraticTeachingEngine {
