...
const REFLECTION_TEMPLATES: Record<ReflectionType, Record<ReflectionDepth, string[]>> = {
  PRE_LEARNING: {
    SHALLOW: [
...
const STRATEGY_INFO: Record<LearningStrategy, {
  description: string;
  howToUse: string;
...
export class MetacognitionEngine {
  private config: MetacognitionEngineConfig;
  private samConfig: SAMConfig;
  private logger?: { info?: (...args: unknown[]) => void };

  // In-memory caches (would use database in production)
  private sessionCache: Map<string, StudySession[]> = new Map();
  private goalCache: Map<string, LearningGoal[]> = new Map();
  private reflectionCache: Map<string, ReflectionResponse[]> = new Map();
  private strategyProfileCache: Map<string, StrategyProfile> = new Map();
  private skillAssessmentCache: Map<string, MetacognitiveSkillAssessment> = new Map();
  private confidenceCache: Map<string, KnowledgeConfidenceAssessment[]> = new Map();
  private regulationCache: Map<string, SelfRegulationProfile> = new Map();

  constructor(config: MetacognitionEngineConfig) {
    this.config = config;
    this.samConfig = config.samConfig;
    this.logger = config.samConfig.logger as { info?: (...args: unknown[]) => void } | undefined;
  }
   ...
   */
  async generateReflection(input: GenerateReflectionInput): Promise<GenerateReflectionResult> {
  ...
  private getTargetSkillForReflectionType(type: ReflectionType): MetacognitiveSkill {
    const skillMap: Record<ReflectionType, MetacognitiveSkill> = {
  ...
  private generateFollowUpQuestions(mainQuestion: string, depth: ReflectionDepth): string[] {
  ...
  private getSuggestedTime(depth: ReflectionDepth): number {
    const timeMap: Record<ReflectionDepth, number> = {
      SHALLOW: 2,
      MODERATE: 5,
      DEEP: 10,
    };
  ...
  private async generateAIReflectionPrompts(
...
Focus on metacognitive awareness and actionable insights.`;
      ...
      return parsed.map((item: { question?: string; followUpQuestions?: string[] }, idx: number) => ({
  ...
  private extractThemes(responses: ReflectionResponse[]): string[] {
   ...
   */
  async analyzeReflection(input: AnalyzeReflectionInput): Promise<ReflectionAnalysis> {
  ...
  private assessReflectionDepth(text: string): ReflectionDepth {
  ...
  private identifySkillsFromResponse(text: string, prompt: ReflectionPrompt): MetacognitiveSkill[] {
    const skills: MetacognitiveSkill[] = [prompt.targetSkill];
  ...
  private identifyGrowthAreas(text: string, prompt: ReflectionPrompt): string[] {
    const areas: string[] = [];
  ...
  private storeReflection(response: ReflectionResponse): void {
  ...
  recordStudySession(input: RecordStudySessionInput): StudySession {
    ...
    const durationMinutes = Math.round(
      (input.endedAt.getTime() - input.startedAt.getTime()) / (1000 * 60)
    );
  ...
  private updateStrategyProfile(userId: string, session: StudySession): void {
    ...
    const strategyCounts = new Map<LearningStrategy, number>();
  ...
  private analyzeOptimalTimes(sessions: StudySession[]): TimeSlot[] {
    const slotScores = new Map<string, { count: number; totalScore: number }>();
  ...
  private analyzeEffectiveEnvironments(sessions: StudySession[]): StudyEnvironment[] {
    const envScores = new Map<string, { env: StudyEnvironment; count: number; totalScore: number }>();
  ...
  private analyzeStrategyEffectiveness(sessions: StudySession[]): StrategyEffectiveness[] {
    const strategyStats = new Map<LearningStrategy, { count: number; totalScore: number }>();
  ...
  private analyzeFocusPatterns(sessions: StudySession[]): FocusPattern {
    const focusLevels = sessions
      .filter(s => s.focusLevel !== undefined)
      .map(s => s.focusLevel as number);
  ...
  private getPeakFocusTime(sessions: StudySession[]): string {
  ...
  private identifyDistractions(sessions: StudySession[]): string[] {
    const distractions = new Set<string>();
  ...
  private analyzeBreakPatterns(sessions: StudySession[]): BreakPattern {
  ...
  private calculateHabitScores(
    ...
    periodDays: number
  ): Record<StudyHabitCategory, number> {
    const scores: Record<StudyHabitCategory, number> = {
      TIME_ALLOCATION: 0,
      ENVIRONMENT: 0,
      FOCUS_MANAGEMENT: 0,
      BREAK_PATTERNS: 0,
  ...
  private generateHabitRecommendations(
    scores: Record<StudyHabitCategory, number>,
    sessions: StudySession[]
  ): StudyHabitRecommendation[] {
    const recommendations: StudyHabitRecommendation[] = [];

    // Find lowest scoring categories
    const sortedCategories = (Object.entries(scores) as [StudyHabitCategory, number][])
      .sort((a, b) => a[1] - b[1]);
  ...
  private getRecommendationForCategory(
    ...
    const recommendationMap: Record<StudyHabitCategory, StudyHabitRecommendation> = {
      TIME_ALLOCATION: {
        category,
  ...
  recommendStrategies(input: RecommendStrategiesInput): RecommendStrategiesResult {
    ...
    const highEffectStrategies = (Object.entries(STRATEGY_INFO) as [LearningStrategy, typeof STRATEGY_INFO[LearningStrategy]][])
    ...
    if (input.bloomsLevel) {
      const relevantStrategies = (Object.entries(STRATEGY_INFO) as [LearningStrategy, typeof STRATEGY_INFO[LearningStrategy]][])
        .filter(([_, info]) => info.bestFor.includes(input.bloomsLevel as BloomsLevel))
  ...
  assessConfidence(input: AssessConfidenceInput): KnowledgeConfidenceAssessment {
    ...
    const items: ConfidenceItem[] = input.items.map(item => ({
    ...
    let confidenceBias: 'OVERCONFIDENT' | 'UNDERCONFIDENT' | 'WELL_CALIBRATED' = 'WELL_CALIBRATED';
    ...
    const assessment: KnowledgeConfidenceAssessment = {
  ...
  private estimateCognitiveLoad(input: AssessCognitiveLoadInput): CognitiveLoadLevel {
  ...
  private identifyLoadFactors(
    input: AssessCognitiveLoadInput,
  ...
  private generateLoadRecommendations(
    ...
    const recommendations: LoadOptimizationRecommendation[] = [];
  ...
  private monitorGoal(goal: LearningGoal): GoalMonitoringResult {
    const now = new Date();
  ...
  private generateMotivationalMessage(progress: number, isOnTrack: boolean): string {
    if (progress === 0) {
  ...
  private isAssessmentStale(assessment: MetacognitiveSkillAssessment): boolean {
    const hoursSinceAssessment = (Date.now() - assessment.assessedAt.getTime()) / (1000 * 60 * 60);
    return hoursSinceAssessment > 24;
  }

  private calculateMetacognitiveAssessment(userId: string): MetacognitiveSkillAssessment {
    ...
    const skills: MetacognitiveSkillScore[] = [
      {
        skill: 'PLANNING',
  ...
  private calculatePlanningScore(goals: LearningGoal[], sessions: StudySession[]): number {
  ...
  private calculateMonitoringScore(reflections: ReflectionResponse[], sessions: StudySession[]): number {
    let score = 50;
    if (reflections.length > 5) score += 20;
    if (sessions.some(s => s.focusLevel !== undefined)) score += 15;
    if (sessions.some(s => s.outcome !== undefined)) score += 15;
    return Math.min(100, score);
  }

  private calculateEvaluatingScore(reflections: ReflectionResponse[], sessions: StudySession[]): number {
    let score = 50;
    ...
    if (sessions.some(s => s.outcome?.satisfactionLevel !== undefined)) score += 25;
    return Math.min(100, score);
  }

  private calculateRegulatingScore(sessions: StudySession[]): number {
    let score = 50;
  ...
  private calculateSelfQuestioningScore(reflections: ReflectionResponse[]): number {
    let score = 50;
  ...
  private calculateElaborationScore(reflections: ReflectionResponse[], sessions: StudySession[]): number {
    let score = 50;
    const elaborativeReflections = reflections.filter(
      r => typeof r.response === 'string' &&
  ...
  private calculateOrganizationScore(sessions: StudySession[]): number {
    let score = 50;
  ...
  private calculateTimeManagementScore(sessions: StudySession[], goals: LearningGoal[]): number {
    let score = 50;
    const avgDuration = sessions.length > 0
      ? sessions.reduce((s, sess) => s + sess.durationMinutes, 0) / sessions.length
      : 0;
    if (avgDuration >= 25 && avgDuration <= 60) score += 20;
    if (goals.some(g => g.deadline !== undefined)) score += 15;
  ...
  private generateMetacognitiveExercises(
    ...
    const exerciseTemplates: Record<MetacognitiveSkill, MetacognitiveExercise> = {
      PLANNING: {
        id: 'ex-planning-1',
  ...
  private createDefaultRegulationProfile(userId: string): SelfRegulationProfile {
    return {
      userId,
      emotionalRegulation: {
  ...
  recordIntervention(
    ...
    const newIntervention: RegulationIntervention = {
      type,
 ...
 */
export function createMetacognitionEngine(config: MetacognitionEngineConfig): MetacognitionEngine {
