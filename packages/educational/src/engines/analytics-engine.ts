...
// Internal types for interaction and analytics records
...
// Extended database adapter interface for analytics
interface AnalyticsDatabaseAdapter {
  // Interaction queries
  ...
  getAnalyticsRecords(params: {
    ...
    startDate?: Date;
    endDate?: Date;
  }): Promise<SAMAnalyticsRecord[]>;

  // User stats
  getUserStats(userId: string, courseId?: string): Promise<UserSAMStats>;
...
export class AnalyticsEngine implements IAnalyticsEngine {
  private database?: AnalyticsDatabaseAdapter;

  constructor(private config: AnalyticsEngineConfig) {
    this.database = config.database as unknown as AnalyticsDatabaseAdapter;
  }
  ...
  private getDefaultAnalytics(): ComprehensiveAnalytics {
    return {
      metrics: {
        totalInteractions: 0,
  ...
  private calculateLearningMetrics(
    interactions: SAMInteractionRecord[],
    analyticsRecords: SAMAnalyticsRecord[]
  ): AnalyticsLearningMetrics {
  ...
  private async calculateContentInsights(
    userId: string,
    interactions: SAMInteractionRecord[],
    courseId?: string
    ...
    const aiAssistanceRate =
      totalContentActions > 0 ? (aiAssistedActions / totalContentActions) * 100 : 0;
    ...
    const suggestionAcceptanceRate =
      suggestionsGiven > 0 ? (suggestionsApplied / suggestionsGiven) * 100 : 0;
  ...
  private async calculateTrends(
    ...
    const productivityInteractions = interactions.filter((i) =>
      ['CONTENT_GENERATE', 'FORM_SUBMIT', 'QUICK_ACTION'].includes(i.interactionType)
    );
  ...
  private calculateContentQuality(analyticsRecords: SAMAnalyticsRecord[]): number {
    if (analyticsRecords.length === 0) return 0;

    const recordsWithSatisfaction = analyticsRecords.filter(
      (r) => r.satisfactionScore !== null && r.satisfactionScore !== undefined
    );
    const recordsWithCompletion = analyticsRecords.filter(
      (r) => r.completionRate !== null && r.completionRate !== undefined
    );
    ...
    const avgCompletion =
      recordsWithCompletion.length > 0
  ...
  private calculateRecencyScore(interactions: SAMInteractionRecord[]): number {
  ...
  private calculateFrequencyScore(interactions: SAMInteractionRecord[]): number {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  ...
  private calculateDiversityScore(interactions: SAMInteractionRecord[]): number {
  ...
  private mapInteractionToFeature(interactionType: string): string {
  ...
  private estimateTimeToNextLevel(currentPoints: number, velocity: number): number {
