...
export class SAMEvaluationEngine {
  ...
  private settings: EvaluationSettings;
...
Reference the grading context and provide actionable advice.`,
  ...
  async generateAssessment(
    config: AssessmentGenerationConfig
...
Return the question as a JSON object.`,
  ...
  private getAssessmentSystemPrompt(): string {
...
Return questions as a valid JSON array.`;
  ...
  private buildAssessmentPrompt(config: AssessmentGenerationConfig): string {
  ...
  private parseGeneratedQuestions(
    ...
    if (validationResult.success && validationResult.data) {
      return validationResult.data.map((q, index) => ({
        ...
        questionType: (q.questionType as QuestionType) || config.questionTypes[0],
...
Good luck!`;
  ...
  private analyzePerformance(request: AdaptiveQuestionRequest): {
    accuracy: number;
  ...
  private determineNextDifficulty(
    ...
    analysis: { accuracy: number; trend: string },
    settings?: { targetAccuracy?: number; difficultyAdjustmentRate?: number }
  ): QuestionDifficulty {
...
Return as JSON:
{
  ...
  private parseAdaptiveQuestion(
    ...
    if (validationResult.success && validationResult.data) {
      ...
      return {
        ...
        text: validated.text,
        questionType: (validated.questionType as QuestionType) || 'MULTIPLE_CHOICE',
        bloomsLevel: validated.bloomsLevel || 'UNDERSTAND',
        difficulty,
        options: validated.options as QuestionOption[] | undefined,
  ...
  private getNextRecommendation(analysis: { accuracy: number; trend: string }): string {
  ...
  private createFallbackQuestions(config: AssessmentGenerationConfig): EnhancedQuestion[] {
  ...
  private createFallbackAdaptiveResult(
    request: AdaptiveQuestionRequest,
    difficulty: QuestionDifficulty,
    analysis: { accuracy: number; averageTimeSpent: number; trend: 'improving' | 'stable' | 'declining'; confidence: number; timeEfficiency: number }
  ): AdaptiveQuestionResult {
    return {
      question: {
        id: this.generateId(),
...
Provide a JSON response with:
{
  ...
  private parseGradingAssistance(content: string, maxScore: number): GradingAssistance {
    // Use schema validation for robust parsing
  ...
  private gradeObjectiveAnswer(answer: ObjectiveAnswer): {
    score: number;
    isCorrect: boolean;
    feedback: string;
  } {
  ...
  private countMatches(student: unknown, correct: unknown): number {
  ...
  private createPendingEvaluation(context: EvaluationContext): SubjectiveEvaluationResult {
    return {
      score: 0,
      maxScore: context.maxPoints,
      accuracy: 0,
  ...
  private createDefaultGradingAssistance(maxScore: number): GradingAssistance {
    return {
      suggestedScore: 0,
      maxScore,
      confidence: 0,
  ...
  private createDefaultStudentExplanation(result: EvaluationResult, studentName: string): string {
...
export function createEvaluationEngine(config: EvaluationEngineConfig): SAMEvaluationEngine {
