...
export class AchievementEngine implements IAchievementEngine {
  ...
  private database: AchievementDatabaseAdapter;

  constructor(private config: AchievementEngineConfig) {
  ...
  async getActiveChallenges(userId: string): Promise<Challenge[]> {
    try {
      ...
      return this.challenges.filter(challenge =>
        userChallenges.activeChallenges.includes(challenge.id)
      );
    } catch (error) {
  ...
  private isStreakAction(action: string): boolean {
    return STREAK_ACTIONS.includes(action);
  }

  private getStreakType(action: string): string {
    return STREAK_TYPE_MAP[action] ?? 'DAILY_INTERACTION';
  }

  private getChallengeTimeframe(challenge: Challenge): { start: Date; end: Date } {
    const now = new Date();
 ...
 */
export function createAchievementEngine(config: AchievementEngineConfig): AchievementEngine {
