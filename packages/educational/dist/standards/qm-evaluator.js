...
const LEARNER_CENTERED_PATTERN = /\b(you will|learners? will|students? will|be able to|can|will be able|upon completion|by the end|after completing)\b/i;
// ═══════════════════════════════════════════════════════════════
// QM EVALUATOR CLASS
// ═══════════════════════════════════════════════════════════════
export class QMEvaluator {
    ...
    evaluate2_3_LearnerCenteredObjectives(standard, data) {
        ...
        const learnerCenteredCount = data.objectives.filter(obj => LEARNER_CENTERED_PATTERN.test(obj)).length;
    ...
    evaluate3_3_EvaluationCriteria(standard, data) {
        ...
        const assessmentsWithFeedback = data.assessments.filter(a => a.questions?.some(q => q.explanation || q.feedback)).length;
    ...
    evaluate3_5_ProgressTracking(standard, data) {
        ...
        const formativeCount = data.assessments.filter(a => a.type === 'quiz' || a.type === 'practice').length;
        let score;
        if (formativeCount >= 3)
            score = 2;
        else if (formativeCount >= 1)
            score = 1;
        else
            score = 0;
        return {
    ...
    evaluate4_5_MaterialVariety(standard, data) {
        ...
        const hasText = data.chapters.some(ch => ch.sections?.some(s => s.description && s.description.length > 50));
        if (hasText)
            materialTypes.add('text');
    ...
    evaluate8_1_Navigation(standard, data) {
        ...
        if (titledChapters === data.chapters.length && data.chapters.length > 0)
            score++;
    ...
    evaluate8_2_Readability(standard, data) {
        ...
        const chaptersWithDesc = data.chapters.filter(ch => ch.learningOutcome && ch.learningOutcome.length > 20).length;
        if (chaptersWithDesc >= data.chapters.length * 0.5)
            score++;
        ...
        const totalSections = data.chapters.reduce((sum, ch) => sum + (ch.sections?.length ?? 0), 0);
        if (sectionsWithDesc >= totalSections * 0.3)
            score++;
        return {
            standardId: standard.id,
            status: score >= 3 ? 'met' : score >= 2 ? 'partially_met' : 'not_met',
    ...
    evaluate8_3_AccessibleContent(standard, data) {
        ...
        const hasTextContent = data.chapters.some(ch => ch.sections?.some(s => s.description && s.description.length > 50));
        let score;
    ...
    evaluate8_5_MultimediaUsability(standard, data) {
        ...
        let score;
        if (videosCount >= 1 && hasAttachments)
            score = 2;
        else if (videosCount >= 1 || hasAttachments)
            score = 1;
        else
            score = 0;
        return {
