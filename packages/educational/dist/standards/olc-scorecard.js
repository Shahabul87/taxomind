...
export class OLCEvaluator {
    ...
    evaluateCD1_InstructionalDesign(indicator, data) {
        ...
        if (totalSections >= data.objectives.length && data.objectives.length > 0)
            score++;
        // Check for assessments aligned with objectives
        if (data.assessments.length >= Math.ceil(data.objectives.length * 0.5))
            score++;
    ...
    evaluateCD3_ActiveLearning(indicator, data) {
        ...
        if (data.assessments.some(a => a.type === 'quiz' || a.type === 'practice'))
            elements++;
        // Check for projects/assignments
        if (data.assessments.some(a => a.type === 'project' || a.type === 'assignment'))
            elements++;
        // Check for video content (interactive viewing)
        if (data.chapters.some(ch => ch.sections?.some(s => s.videoUrl)))
            elements++;
        // Check for multiple assessments (learning progression)
        if (data.assessments.length >= 3)
            elements++;
        let score;
        if (elements >= 4)
            score = 3;
    ...
    evaluateCD4_MaterialVariety(indicator, data) {
        ...
        if (data.chapters.some(ch => ch.sections?.some(s => s.videoUrl)))
            formats.add('video');
        if (data.chapters.some(ch => ch.sections?.some(s => s.description && s.description.length > 50)))
            formats.add('text');
        if ((data.attachments?.length ?? 0) > 0)
            formats.add('attachments');
        if (data.assessments.length > 0)
            formats.add('assessments');
        if (data.description && data.description.length > 100)
            formats.add('introduction');
        let score;
        if (formats.size >= 4)
            score = 3;
    ...
    evaluateCD5_CognitiveLoad(indicator, data) {
        ...
        if (avgSectionsPerChapter >= 2 && avgSectionsPerChapter <= 5)
            score++;
        ...
        if (chaptersWithOutcomes >= data.chapters.length * 0.5)
            score++;
    ...
    evaluateCS1_LogicalOrganization(indicator, data) {
        ...
        if (data.chapters.length >= 3)
            score++;
        if (data.chapters.every(ch => ch.title && ch.title.length > 3))
            score++;
        return this.createResult(indicator, Math.min(score, 3));
    }
    evaluateCS2_ContentChunking(indicator, data) {
        const totalSections = data.chapters.reduce((sum, ch) => sum + (ch.sections?.length ?? 0), 0);
        let score;
        if (totalSections >= data.chapters.length * 2 && data.chapters.length >= 3)
            score = 3;
        else if (totalSections >= data.chapters.length)
            score = 2;
    ...
    evaluateTL2_InstructionalMethods(indicator, data) {
        ...
        if (data.chapters.some(ch => ch.sections?.some(s => s.videoUrl)))
            methods.add('video');
        if (data.chapters.some(ch => ch.sections?.some(s => s.description)))
            methods.add('text');
        if (data.assessments.some(a => a.type === 'quiz'))
            methods.add('quiz');
        if (data.assessments.some(a => a.type === 'project' || a.type === 'assignment'))
            methods.add('project');
        if ((data.attachments?.length ?? 0) > 0)
    ...
    evaluateTL3_PracticeOpportunities(indicator, data) {
        const practiceCount = data.assessments.filter(a => a.type === 'quiz' || a.type === 'practice').length;
        const projectCount = data.assessments.filter(a => a.type === 'project' || a.type === 'assignment').length;
        const totalPractice = practiceCount + projectCount;
        let score;
        if (totalPractice >= 5)
            score = 3;
        else if (totalPractice >= 3)
            score = 2;
    ...
    evaluateEA3_FormativeAssessment(indicator, data) {
        const formativeAssessments = data.assessments.filter(a => a.type === 'quiz' || a.type === 'practice');
        const withFeedback = formativeAssessments.filter(a => a.questions?.some(q => q.explanation || q.feedback)).length;
        let score;
        if (withFeedback >= 3)
            score = 3;
        else if (withFeedback >= 2)
            score = 2;
        else if (formativeAssessments.length >= 1)
            score = 1;
        else
            score = 0;
    ...
    evaluateEA4_ClearCriteria(indicator, data) {
        const assessmentsWithCriteria = data.assessments.filter(a => a.questions?.some(q => q.explanation)).length;
    ...
    evaluateAU2_AccessibleMultimedia(indicator, data) {
        const videoCount = data.chapters.reduce((count, ch) => count + (ch.sections?.filter(s => s.videoUrl).length ?? 0), 0);
        const hasTextAlternatives = data.chapters.some(ch => ch.sections?.some(s => s.description && s.description.length > 50));
        let score;
        if (videoCount >= 3 && hasTextAlternatives)
            score = 3;
        else if (videoCount >= 1 && hasTextAlternatives)
            score = 2;
        else if (videoCount >= 1 || hasTextAlternatives)
            score = 1;
        else
    ...
    evaluateAU3_Readability(indicator, data) {
        ...
        if (data.description && data.description.length >= 100)
            score++;
        // Chapter outcomes
        const chaptersWithOutcomes = data.chapters.filter(ch => ch.learningOutcome && ch.learningOutcome.length > 20).length;
        if (chaptersWithOutcomes >= data.chapters.length * 0.5)
            score++;
