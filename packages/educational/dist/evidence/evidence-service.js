...
export class EvidenceService {
     ...
     */
    async storeEvidence(analysisId, evidence) {
     ...
     */
    async deleteEvidenceForAnalysis(analysisId) {
     ...
     */
    async aggregateEvidence(analysisId) {
     ...
     */
    determineConflictSeverity(evidence) {
        ...
        const classIndices = Array.from(classifications).map((c) => bloomsOrder.indexOf(c)).filter((i) => i >= 0);
...
export class ConfidenceCalculator {
     ...
     */
    calculate(keywordMatches, context, position) {
        ...
        // Combine scores
        const rawConfidence = keywordScore * this.options.keywordBaseWeight +
            (contextScore - keywordScore) * this.options.contextWeight +
            (positionScore - keywordScore) * this.options.positionWeight;
        // Apply bounds
        const confidence = Math.max(this.options.minConfidence, Math.min(this.options.maxConfidence, rawConfidence));
        // Build breakdown
        const breakdown = {
            keywordScore,
...
export class KeywordAnalyzer {
    ...
    analyzeText(text) {
        ...
        const secondaryLevels = Object.entries(levelCounts)
     ...
     */
    extractTextPosition(text, sentence, paragraphIndex = 0) {
 ...
 */
export function createConfidenceCalculator(options) {
    return new ConfidenceCalculator(options);
}
/**
 * Create a keyword analyzer instance
 */
export function createKeywordAnalyzer() {
