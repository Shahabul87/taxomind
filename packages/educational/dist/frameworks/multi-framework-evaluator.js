 ...
 */
import { COURSE_TYPE_FRAMEWORK_WEIGHTS, getFramework, getIdealDistribution, } from './framework-definitions';
...
export class MultiFrameworkEvaluator {
    ...
    async evaluate(content) {
        ...
        for (const frameworkType of this.options.frameworks) {
            const analysis = await this.analyzeWithFramework(content.content, frameworkType, courseType, content.existingBloomsAnalysis, content.existingDOKAnalysis);
            frameworkAnalyses.push(analysis);
        }
        // Separate primary and secondary analyses
        const primaryAnalysis = frameworkAnalyses.find((a) => a.framework === this.options.primaryFramework) || frameworkAnalyses[0];
        const secondaryAnalyses = frameworkAnalyses.filter((a) => a.framework !== primaryAnalysis.framework);
        ...
        const recommendations = this.generateRecommendations(frameworkAnalyses, courseType, crossFrameworkAlignment);
     ...
     */
    async analyzeWithFramework(content, frameworkType, courseType, existingBlooms, existingDOK) {
        ...
        if (frameworkType === 'blooms' && existingBlooms) {
            return this.buildAnalysisFromExisting(frameworkType, existingBlooms.distribution, existingBlooms.dominantLevel, courseType);
        }
        if (frameworkType === 'dok' && existingDOK) {
            ...
            return this.buildAnalysisFromExisting(frameworkType, dokDist, existingDOK.dominantLevel.toString(), courseType);
        }
        // Analyze content
        const distribution = {};
     ...
     */
    buildAnalysisFromExisting(frameworkType, distribution, dominantLevel, courseType) {
     ...
     */
    calculateAlignmentWithIdeal(actual, ideal) {
     ...
     */
    buildLevelAnalysis(actual, ideal) {
     ...
     * Calculate cross-framework alignment
     */
    calculateCrossFrameworkAlignment(analyses) {
     ...
     */
    generateRecommendations(analyses, courseType, crossAlignment) {
        ...
        if (crossAlignment < 70) {
            recommendations.push({
                framework: 'blooms', // Generic recommendation
                type: 'rebalance',
                priority: 'medium',
     ...
     */
    calculateCompositeScore(analyses, weights) {
        let weightedSum = 0;
        let totalWeight = 0;
        for (const analysis of analyses) {
            const weight = weights[analysis.framework] || 0;
            if (weight > 0) {
     ...
     */
    mapToFramework(fromFramework, toFramework, level) {
 ...
 */
export function createMultiFrameworkEvaluator(options) {
