...
// src/analyzers/video-cognitive-analyzer.ts
...
var VideoCognitiveAnalyzer = class {
  config;
  transcriptFetcher;
  contentAnalyzer;
  ...
  chunkTranscript(segments) {
    ...
    if (currentChunk.length > 0) {
      if (currentDuration < this.config.minChunkDurationSeconds && chunks.length > 0) {
  ...
  detectTransitions(chunkAnalyses) {
    ...
    for (let i = 1; i < chunkAnalyses.length; i++) {
      ...
      if (prev.confidence < this.config.transitionConfidenceThreshold || curr.confidence < this.config.transitionConfidenceThreshold) {
        continue;
      }
    ...
    for (const transition of transitions) {
      if (transition.magnitude >= this.config.significantTransitionThreshold) {
        pausePoints.push({
          ...
          suggestedActivity: transition.direction === "up" ? "Take a moment to reflect on the previous concepts before moving to more complex material." : "Good opportunity to practice what you just learned.",
  ...
  buildTimeline(chunkAnalyses) {
    ...
    for (let minute = 0; minute < totalMinutes; minute++) {
      ...
      const chunk = chunkAnalyses.find(
        (a) => timeInSeconds >= a.chunk.startTime && timeInSeconds < a.chunk.endTime
      );
      if (chunk) {
        timeline.push({
          minute,
...
function createVideoCognitiveAnalyzer(contentAnalyzer, config, transcriptFetcher) {
  return new VideoCognitiveAnalyzer(contentAnalyzer, config, transcriptFetcher);
}

// src/analyzers/image-cognitive-analyzer.ts
var BLOOMS_LEVEL_ORDER2 = [
  "REMEMBER",
  "UNDERSTAND",
  "APPLY",
  "ANALYZE",
...
var ImageCognitiveAnalyzer = class {
  config;
  contentAnalyzer;
  ...
  async analyzeImage(imageData, metadata = {}) {
    ...
    const textRegions = this.config.enableOCR ? await this.extractText(imageData) : [];
    const extractedText = textRegions.filter((r) => r.confidence >= this.config.ocrConfidenceThreshold).map((r) => r.text).join(" ");
    const visualElements = this.config.enableVisualAnalysis ? await this.detectVisualElements(imageData) : [];
    ...
    const accessibilityAssessment = this.config.enableAccessibilityCheck ? this.assessAccessibility(fullMetadata, textRegions, colorAnalysis) : this.getDefaultAccessibilityAssessment();
    const recommendations = this.generateRecommendations(
      cognitiveAssessment,
      structuralAnalysis,
    ...
    return {
      ...
      processingMetadata: {
        ...
        modelUsed: this.config.visionProvider ? "vision-model" : void 0
      }
    };
  }
  /**
  ...
  analyzeStructure(visualElements, textRegions) {
    ...
    const flowDirection = hasVisualFlow ? this.detectFlowDirection(visualElements) : void 0;
    return {
      imageType,
      complexity,
      sectionCount,
  ...
  async assessCognitive(extractedText, structuralAnalysis, metadata) {
    ...
    const additionalText = [metadata.altText, metadata.caption].filter(Boolean).join(" ");
  ...
  calculateDistribution(baseLevel, textDistribution, additionalDistribution, structuralAnalysis) {
    const baseLevelIndex = BLOOMS_LEVEL_ORDER2.indexOf(baseLevel);
    const distribution = {
      REMEMBER: 0,
      UNDERSTAND: 0,
      APPLY: 0,
    ...
    for (let i = 0; i < BLOOMS_LEVEL_ORDER2.length; i++) {
      const distance = Math.abs(i - baseLevelIndex);
      distribution[BLOOMS_LEVEL_ORDER2[i]] = Math.max(0, 40 - distance * 15);
    }
    if (textDistribution) {
      for (const level of BLOOMS_LEVEL_ORDER2) {
  ...
  calculateConfidence(textConfidence, structuralAnalysis) {
    let confidence = 0.5;
    if (textConfidence !== void 0) {
      confidence = textConfidence * 0.6 + 0.4;
    }
  ...
  assessAccessibility(metadata, textRegions, colorAnalysis) {
    ...
    const altTextQuality = hasAltText ? Math.min(100, metadata.altText.length * 2) : 0;
    if (!hasAltText) {
      issues.push({
        severity: "critical",
  ...
  generateActivities(cognitive, structural) {
    ...
    const levelIndex = BLOOMS_LEVEL_ORDER2.indexOf(cognitive.primaryLevel);
    if (levelIndex > 0) {
      const lowerLevel = BLOOMS_LEVEL_ORDER2[levelIndex - 1];
      activities.push({
    ...
    if (levelIndex < BLOOMS_LEVEL_ORDER2.length - 1) {
      const higherLevel = BLOOMS_LEVEL_ORDER2[levelIndex + 1];
      activities.push({
...
function createImageCognitiveAnalyzer(contentAnalyzer, config) {
