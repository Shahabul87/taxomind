...
import { MicrolearningEngine, createMicrolearningEngine, } from '../engines/microlearning-engine';
...
`;
const createMockMicroModule = (overrides = {}) => ({
...
describe('MicrolearningEngine', () => {
    ...
    describe('Content Chunking', () => {
        ...
        describe('chunkContent (AI-Based)', () => {
            let aiEngine;
            beforeEach(() => {
                aiEngine = new MicrolearningEngine(createMockEngineConfig({ enableAIChunking: true }));
            });
    ...
    describe('Delivery Scheduling', () => {
        ...
        it('should include course ID when provided', () => {
            const schedule = engine.createSchedule('user-123', modules, {}, 'course-456');
    ...
    describe('Learning Sessions', () => {
        ...
        it('should respect max duration limit', async () => {
            ...
            const session = await engine.createSession(input);
            const totalDuration = session.modules.reduce((sum, m) => sum + m.module.durationMinutes, 0);
        ...
        it('should filter by focus concepts', async () => {
            ...
            for (const sm of session.modules) {
                const hasMatch = sm.module.keywords.some(k => k.includes('Machine')) ||
    ...
    describe('Mobile Optimization', () => {
        ...
        it('should adjust for reading speed', () => {
            ...
            expect(slowResult.content.estimatedTimeSeconds).toBeGreaterThan(fastResult.content.estimatedTimeSeconds);
        });
        ...
        it('should prioritize initial loading chunks', () => {
            ...
            if (result.loadingChunks && result.loadingChunks.length > 2) {
                expect(result.loadingChunks[0].priority).toBeLessThanOrEqual(result.loadingChunks[2].priority);
            }
        });
    });
    // ============================================================================
