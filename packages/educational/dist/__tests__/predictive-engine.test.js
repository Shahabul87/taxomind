...
// ============================================================================
function createSamplePredictiveStudentProfile(overrides = {}) {
    return {
        userId: 'user-1',
        courseId: 'course-1',
        learningHistory: {
...
function createLowEngagementStudent(overrides = {}) {
...
function createHighPerformingStudent(overrides = {}) {
    return createSamplePredictiveStudentProfile({
...
function createSampleStudentCohort(overrides = {}) {
    return {
        courseId: 'course-1',
        students: [
...
function createSampleLearningContext(overrides = {}) {
    return {
...
describe('PredictiveEngine', () => {
    ...
    describe('predictLearningOutcomes', () => {
        ...
        it('should predict higher success for high performing students', async () => {
            ...
            expect(highResult.successProbability).toBeGreaterThan(lowResult.successProbability);
        });
        it('should predict completion date in the future', async () => {
            const student = createSamplePredictiveStudentProfile();
            const result = await engine.predictLearningOutcomes(student);
            expect(result.predictedCompletionDate.getTime()).toBeGreaterThan(Date.now());
        });
    ...
    describe('identifyAtRiskStudents', () => {
        ...
        it('should calculate risk distribution correctly', async () => {
            const cohort = createSampleStudentCohort();
            const result = await engine.identifyAtRiskStudents(cohort);
            const total = result.riskDistribution.high +
    ...
    describe('factory function', () => {
        ...
        it('should create engine with custom AI adapter', () => {
            const customAdapter = createMockAIAdapter(() => createMockAIResponse('0.05'));
            const customConfig = {
                samConfig: {
                    ...createMockSAMConfig(),
                    ai: customAdapter,
    ...
    describe('AI integration', () => {
        ...
        it('should handle invalid AI response gracefully', async () => {
            const invalidAdapter = createMockAIAdapter(() => createMockAIResponse('not-a-number'));
            const invalidConfig = {
                samConfig: {
                    ...createMockSAMConfig(),
                    ai: invalidAdapter,
