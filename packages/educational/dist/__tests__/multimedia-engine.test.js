...
import { MultimediaEngine, createMultimediaEngine, } from '../engines/multimedia-engine';
...
const createMockVideoContent = (overrides = {}) => ({
...
const createMockAudioContent = (overrides = {}) => ({
...
const createMockInteractiveContent = (overrides = {}) => ({
    type: 'quiz',
    elements: [
        {
            id: 'element-1',
...
const createMockVideoAnalysis = (overrides = {}) => ({
...
const createMockAudioAnalysis = (overrides = {}) => ({
...
const createMockInteractiveAnalysis = (overrides = {}) => ({
...
describe('MultimediaEngine', () => {
    ...
    describe('Multi-Modal Insights', () => {
        ...
        it('should predict engagement based on variety', async () => {
            ...
            const singleInsights = await engine.generateMultiModalInsights('course-123', singleType);
            const multipleInsights = await engine.generateMultiModalInsights('course-123', multipleTypes);
            expect(multipleInsights.engagementPrediction).toBeGreaterThan(singleInsights.engagementPrediction);
        });
    ...
    describe('Content Recommendations', () => {
        ...
        it('should include navigation recommendations', async () => {
            const recommendations = await engine.getContentRecommendations('course-123');
            const hasNavigation = recommendations.some((r) => r.toLowerCase().includes('navigation') || r.toLowerCase().includes('markers'));
            expect(hasNavigation).toBe(true);
        });
        it('should include accessibility recommendations', async () => {
            const recommendations = await engine.getContentRecommendations('course-123');
            const hasAccessibility = recommendations.some((r) => r.toLowerCase().includes('caption') ||
                r.toLowerCase().includes('accessible'));
            expect(hasAccessibility).toBe(true);
        });
        it('should include interactive recommendations', async () => {
            const recommendations = await engine.getContentRecommendations('course-123');
            const hasInteractive = recommendations.some((r) => r.toLowerCase().includes('quiz') || r.toLowerCase().includes('interactive'));
    ...
    describe('Accessibility Report', () => {
        ...
        it('should include caption recommendations', async () => {
            const report = await engine.getAccessibilityReport('course-123');
            const hasCaptions = report.recommendations.some((r) => r.toLowerCase().includes('caption'));
            expect(hasCaptions).toBe(true);
        });
        it('should include screen reader recommendations', async () => {
            const report = await engine.getAccessibilityReport('course-123');
            const hasScreenReader = report.recommendations.some((r) => r.toLowerCase().includes('screen reader'));
            expect(hasScreenReader).toBe(true);
        });
        it('should include keyboard accessibility recommendations', async () => {
            const report = await engine.getAccessibilityReport('course-123');
            const hasKeyboard = report.recommendations.some((r) => r.toLowerCase().includes('keyboard'));
    ...
    describe('Integration Scenarios', () => {
        ...
        it('should analyze complete course content', async () => {
            ...
            // Analyze interactive
            const interactiveAnalysis = await engine.analyzeInteractive(createMockInteractiveContent());
