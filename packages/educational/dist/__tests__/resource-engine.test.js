...
import { ResourceEngine, createResourceEngine, } from '../engines/resource-engine';
...
describe('ResourceEngine', () => {
    ...
    describe('Resource Discovery', () => {
        ...
        it('should sort resources by quality and relevance', async () => {
            ...
            if (resources.length > 1) {
                const scores = resources.map((r) => (r.qualityScore || 0) * 0.5 + (r.relevanceScore || 0) * 0.5);
    ...
    describe('License Checking', () => {
        ...
        it('should flag modification restrictions', async () => {
            ...
            const status = await engine.checkLicenseCompatibility(resource, 'modify');
    ...
    describe('ROI Analysis', () => {
        ...
        it('should consider learner profile for efficiency', async () => {
            ...
            expect(roiWithProfile.learningEfficiency).toBeGreaterThanOrEqual(roiWithoutProfile.learningEfficiency * 0.9);
        });
    ...
    describe('Personalized Recommendations', () => {
        ...
        it('should personalize recommendations', async () => {
            ...
            const recommendations = await engine.personalizeRecommendations(profile, resources);
        ...
        it('should prefer free resources within budget', async () => {
            ...
            const recommendations = await engine.personalizeRecommendations(profile, resources);
            const freeRec = recommendations.find((r) => r.resource.cost?.type === 'free');
            const paidRec = recommendations.find((r) => r.resource.cost?.type === 'paid');
    ...
    describe('Get Resource Recommendations', () => {
        ...
        it('should get recommendations for a user and topic', async () => {
            const recommendations = await engine.getResourceRecommendations('user-123', 'Machine Learning');
            expect(Array.isArray(recommendations)).toBe(true);
        });
        it('should build default profile for user', async () => {
            const recommendations = await engine.getResourceRecommendations('new-user', 'Web Development');
            expect(recommendations).toBeDefined();
        });
        it('should handle multi-word topics', async () => {
            const recommendations = await engine.getResourceRecommendations('user-123', 'React Native Mobile Development');
            expect(Array.isArray(recommendations)).toBe(true);
        });
        it('should handle short topics', async () => {
            const recommendations = await engine.getResourceRecommendations('user-123', 'AI');
            expect(Array.isArray(recommendations)).toBe(true);
        });
    });
    // ============================================================================
    ...
    describe('Resource Type Handling', () => {
        ...
        it('should suggest next steps for course type', async () => {
            ...
            const recommendations = await engine.personalizeRecommendations(profile, resources);
            expect(recommendations[0].nextSteps).toContain('Complete all modules and assessments');
        });
        it('should suggest next steps for tutorial type', async () => {
            const profile = createMockProfile();
            const resources = [createMockResource({ type: 'tutorial' })];
            const recommendations = await engine.personalizeRecommendations(profile, resources);
            expect(recommendations[0].nextSteps).toContain('Practice the demonstrated techniques');
        });
