  ...
  "relations": [
    {
      ...
      "reasoning": "why this relationship exists"
            ...
            const filteredConcepts = parsed.concepts.filter((c) => c.confidence >= this.confidenceThreshold);
            const filteredRelations = parsed.relations.filter((r) => r.confidence >= this.confidenceThreshold);
            const overallConfidence = filteredConcepts.length > 0
    ...
    buildGraph(courseId, concepts, relations) {
        ...
        const targetIds = new Set(relations
    ...
    calculateGraphStats(concepts, relations) {
        ...
        for (const relation of relations) {
            connectionCounts.set(relation.sourceConceptId, (connectionCounts.get(relation.sourceConceptId) ?? 0) + 1);
            connectionCounts.set(relation.targetConceptId, (connectionCounts.get(relation.targetConceptId) ?? 0) + 1);
        }
        const totalConnections = Array.from(connectionCounts.values()).reduce((sum, c) => sum + c, 0);
    ...
    calculateMaxDepth(concepts, relations) {
        const prerequisiteRels = relations.filter((r) => r.relationType === 'PREREQUISITE');
        if (prerequisiteRels.length === 0)
            return 0;
    ...
    async analyzePrerequisites(input) {
        ...
        const directPrereqs = graph.relations
            .filter((r) => r.targetConceptId === input.conceptId &&
            r.relationType === 'PREREQUISITE')
            .map((r) => {
            const prereqConcept = graph.concepts.find((c) => c.id === r.sourceConceptId);
        ...
        const chain = this.buildPrerequisiteChain(input.conceptId, graph, input.maxDepth ?? this.maxPrerequisiteDepth);
        // Find dependent concepts
        const dependentConcepts = graph.relations
            .filter((r) => r.sourceConceptId === input.conceptId &&
            r.relationType === 'PREREQUISITE')
        ...
        const estimatedTime = chain.reduce((total, node) => {
            const bloomsMultiplier = (BLOOMS_HIERARCHY.indexOf(node.concept.bloomsLevel) + 1) * 0.2;
        ...
        if (input.userId && input.includeMastery) {
            result.gapAnalysis = await this.analyzePrerequisiteGaps(input.userId, chain, concept);
        }
        return result;
    }
    buildPrerequisiteChain(conceptId, graph, maxDepth) {
        ...
        const traverse = (nodeId, depth) => {
            ...
            const prereqs = graph.relations.filter((r) => r.targetConceptId === nodeId && r.relationType === 'PREREQUISITE');
        ...
        const directPrereqs = graph.relations.filter((r) => r.targetConceptId === conceptId && r.relationType === 'PREREQUISITE');
    ...
    isBottleneck(conceptId, graph) {
        ...
        const dependentCount = graph.relations.filter((r) => r.sourceConceptId === conceptId && r.relationType === 'PREREQUISITE').length;
    ...
    async generateLearningPath(input) {
        ...
        for (const prereqId of allPrereqs) {
            ...
            sequence.push(this.createLearningPathNode(concept, position++, 'PREREQUISITE'));
        }
        // Add target concepts
        ...
        // Calculate totals
        const totalTime = adjustedSequence.reduce((sum, node) => sum + node.estimatedTimeMinutes, 0);
        return {
    ...
    async analyzeCourse(input) {
        ...
        if (input.includeFullContent && course.chapters) {
            for (const chapter of course.chapters) {
                ...
                for (const extracted of extraction.concepts) {
                    allConcepts.push(this.convertToFullConcept(extracted, input.courseId, chapter.id));
                }
                for (const relation of extraction.relations) {
                    const sourceId = allConcepts.find((c) => c.name.toLowerCase() === relation.sourceConcept.toLowerCase())?.id;
                    const targetId = allConcepts.find((c) => c.name.toLowerCase() === relation.targetConcept.toLowerCase())?.id;
    ...
    assessStructureQuality(graph) {
        ...
        const orphans = graph.concepts.filter((c) => {
            const hasRelations = graph.relations.some((r) => r.sourceConceptId === c.id || r.targetConceptId === c.id);
            return !hasRelations;
        });
        if (orphans.length > 0) {
            issues.push({
        ...
        const overallScore = Math.round((prerequisiteOrdering + conceptContinuity + depthBalance) / 3);
        return {
    ...
    hasCircularDependencies(graph) {
        ...
        const hasCycle = (nodeId) => {
            ...
            const children = graph.relations
                .filter((r) => r.sourceConceptId === nodeId && r.relationType === 'PREREQUISITE')
                .map((r) => r.targetConceptId);
            for (const child of children) {
    ...
    generateCourseRecommendations(graph) {
        ...
        const bloomsTotal = Object.values(graph.stats.conceptsByBloomsLevel).reduce((sum, c) => sum + c, 0);
        if (bloomsTotal > 0) {
            const applyAndAbove = graph.stats.conceptsByBloomsLevel.APPLY +
    ...
    async updateConceptMastery(userId, conceptId, score, evidenceType) {
        ...
        mastery.masteryLevel = this.determineMasteryLevel(mastery.score, mastery.practiceCount);
    ...
    determineMasteryLevel(score, practiceCount) {
        ...
        if (practiceCount === 1)
            return 'INTRODUCED';
        if (score >= 90 && practiceCount >= 3)
            return 'MASTERED';
        if (score >= 70 && practiceCount >= 2)
            return 'PROFICIENT';
        return 'PRACTICING';
    }
    // ============================================================================
    ...
    findRelatedConcepts(conceptId, graph, maxDepth = 3) {
        ...
        const dfs = (currentId, depth, relationPath) => {
            if (depth > maxTraversalDepth || visited.has(currentId))
                return;
            visited.add(currentId);
            // Find all relations involving this concept
            const relations = graph.relations.filter((rel) => rel.sourceConceptId === currentId || rel.targetConceptId === currentId);
    ...
    calculateConceptCentrality(conceptId, graph) {
        ...
        const inDegree = graph.relations.filter((rel) => rel.targetConceptId === conceptId).length;
        const outDegree = graph.relations.filter((rel) => rel.sourceConceptId === conceptId).length;
...
// FACTORY FUNCTION
// ============================================================================
export function createKnowledgeGraphEngine(config) {
