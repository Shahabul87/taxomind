...
export class PersonalizationEngine {
    ...
    database;
    logger;
    learningStyleCache = new Map();
    emotionalStateCache = new Map();
...
Respond in JSON format with keys: emphasizedElements, simplifiedConcepts, additionalExplanations`;
                ...
                if (parsed.emphasizedElements)
                    emphasizedElements.push(...parsed.emphasizedElements);
                if (parsed.simplifiedConcepts)
                    simplifiedConcepts.push(...parsed.simplifiedConcepts);
                if (parsed.additionalExplanations)
                    additionalExplanations.push(...parsed.additionalExplanations);
            }
            catch (error) {
    ...
    async storeLearningStyleProfile(userId, profile) {
        ...
        this.logger?.debug?.('[PersonalizationEngine] Learning style profile cached for user:', userId, profile.primaryStyle);
    }
    async storeEmotionalState(userId, state) {
        ...
        this.logger?.debug?.('[PersonalizationEngine] Emotional state cached for user:', userId, state.currentEmotion);
    }
    async storeMotivationProfile(userId, profile) {
        ...
        this.logger?.debug?.('[PersonalizationEngine] Motivation profile computed for user:', userId, `level: ${profile.currentLevel}`);
    }
    async storeLearningPath(path) {
        ...
        this.logger?.debug?.('[PersonalizationEngine] Learning path generated:', path.pathId, `${path.nodes.length} nodes`);
    }
    async storePersonalizationResult(userId, result) {
        ...
        this.logger?.debug?.('[PersonalizationEngine] Personalization applied for user:', userId, `${result.recommendations.length} recommendations`);
    }
    // ============================================================================
    // UTILITY HELPERS
    // ============================================================================
