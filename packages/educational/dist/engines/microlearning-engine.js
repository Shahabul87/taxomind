  ...
  "chunks": [
    ...
    scheduleModules(modules, preferences) {
        ...
        for (const microMod of modules) {
            ...
            while (!preferences.preferredDays.includes(currentDate.getDay()) ||
    ...
    async createSession(input) {
        ...
        if (input.courseId) {
            availableModules = availableModules.filter(m => m.sourceContext?.courseId === input.courseId);
        }
        // Filter by type if specified
        if (input.moduleTypes && input.moduleTypes.length > 0) {
            availableModules = availableModules.filter(m => input.moduleTypes.includes(m.type));
        }
        // Filter by concepts if specified
        if (input.focusConcepts && input.focusConcepts.length > 0) {
            availableModules = availableModules.filter(m => input.focusConcepts.some(c => m.keywords.includes(c) || m.description.includes(c)));
        }
    ...
    calculateSpacedRepetition(input, progress) {
        ...
        if (input.quality < 3) {
            ...
            if (repetitions === 0) {
                ...
                interval = Math.min(config.maxInterval, Math.round(interval * easeFactor * config.intervalModifier));
            }
            repetitions++;
        }
        // Update ease factor
        easeFactor = Math.max(config.minEaseFactor, easeFactor + (0.1 - (5 - input.quality) * (0.08 + (5 - input.quality) * 0.02)));
        // Apply easy bonus
        if (input.quality === 5) {
    ...
    optimizeForMobile(input) {
        ...
        const mobileContent = this.createMobileContent(contentMod.content.primary, input.networkCondition ?? 'FAST', input.readingSpeed ?? 'NORMAL');
        ...
        // Optimize media
        const optimizedMedia = this.optimizeMedia(contentMod.content.media ?? [], input.networkCondition ?? 'FAST');
        // Calculate data size
    ...
    createMobileContent(content, networkCondition, readingSpeed) {
        ...
        const estimatedTime = Math.round((text.split(/\s+/).length / WORDS_PER_MINUTE) * 60 * speedMultiplier);
        return {
            format: 'MARKDOWN',
            content: text,
    ...
    calculateOverallStats(progress) {
        ...
        const totalTime = progress.reduce((sum, p) => sum + (p.performance?.timeSpentSeconds ?? 0), 0) / 60;
    ...
    calculateStreakStats(progress) {
        ...
        const daysSinceLastActivity = Math.round((today.getTime() - lastDate.getTime()) / (1000 * 60 * 60 * 24));
...
// FACTORY FUNCTION
// ============================================================================
export function createMicrolearningEngine(config) {
