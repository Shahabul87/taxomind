...
export class CourseGuideEngine {
    ...
    calculateDepthMetrics(course) {
        ...
        const totalExams = course.chapters.reduce((sum, ch) => sum + ch.sections.reduce((s, sec) => s + sec.exams.length, 0), 0);
        const totalQuestions = course.chapters.reduce((sum, ch) => sum + ch.sections.reduce((s, sec) => s + sec.questions.length, 0), 0);
        const contentRichness = Math.min(((totalSections * 10) + (totalExams * 20) + (totalQuestions * 5)) / 10, 100);
        ...
        const learningPathClarity = course.chapters.every((ch) => ch.sections.length > 0)
            ? 80
            : 50;
    ...
    async calculateEngagementMetrics(course) {
        ...
        // Completion rate
        const completedCount = course.enrollments.filter((e) => e.progress?.isCompleted).length;
        const completionRate = (completedCount / totalEnrollments) * 100;
        // Average progress
        const totalProgress = course.enrollments.reduce((sum, e) => sum + (e.progress?.percentage || 0), 0);
        const averageProgress = totalProgress / totalEnrollments;
        // Interaction frequency
        let interactionFrequency = 0;
        if (this.databaseAdapter) {
            const recentActivity = await this.databaseAdapter.getRecentInteractionCount(course.id, 7);
        ...
        const activeStudents = course.enrollments.filter((e) => e.progress?.lastAccessedAt &&
            new Date(e.progress.lastAccessedAt) > thirtyDaysAgo).length;
        const retentionRate = totalEnrollments > 0 ? (activeStudents / totalEnrollments) * 100 : 0;
        const overallEngagement = (completionRate +
            averageProgress +
    ...
    calculateMarketAcceptanceMetrics(course) {
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const recentPurchases = course.purchases.filter((p) => new Date(p.createdAt) > thirtyDaysAgo).length;
        const olderPurchases = course.purchases.filter((p) => new Date(p.createdAt) <= thirtyDaysAgo).length;
        const enrollmentGrowth = olderPurchases > 0
        ...
        const recommendationRate = course.reviews.length > 0
            ...
            : 0;
        const overallAcceptance = (Math.max(0, Math.min(100, enrollmentGrowth)) +
            competitivePosition +
            pricingOptimality +
            reviewScore +
            recommendationRate) /
    ...
    identifyGaps(course, competitors) {
        ...
        const strengthCounts = competitorStrengths.reduce((acc, strength) => {
    ...
    async predictSuccess(course, metrics) {
        ...
        const projectedEnrollments = Math.round(currentEnrollments * (1 + growthRate) * 3);
    ...
    calculateSuccessProbability(metrics, riskFactors) {
        const baseScore = (metrics.depth.overallDepth +
 ...
 */
export function createCourseGuideEngine(config = {}) {
