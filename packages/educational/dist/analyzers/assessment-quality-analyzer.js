...
export class AssessmentQualityAnalyzer {
    ...
    analyzeBloomsCoverage(questions) {
        ...
        const higherOrderCoverage = (distribution.ANALYZE > 0 ? 1 : 0) +
    ...
    analyzeFeedbackQuality(questions) {
        ...
        if (explanationRatio > 0.8)
            score += 15;
        if (explanationDepth === 'detailed' || explanationDepth === 'comprehensive')
            score += 20;
        if (providesRemediation)
            score += 15;
    ...
    analyzeDistractors(questions) {
        ...
        const averagePlausibility = plausibilityScores.length > 0
    ...
    determineDifficultyPattern(difficulties) {
        ...
        if (ascendingRatio > 0.6)
            return 'ascending';
        if (descendingRatio > 0.6)
            return 'descending';
        if (ascendingRatio < 0.3 && descendingRatio < 0.3)
            return 'plateaued';
        return 'random';
    }
    /**
    ...
    calculateOverallScore(metrics) {
        ...
        let weightedSum = metrics.questionVariety.score * weights.questionVariety +
