 ...
 */
import { COURSE_TYPE_PROFILES, } from '../types/depth-analysis.types';
// ═══════════════════════════════════════════════════════════════
...
const LEARNER_CENTERED_PATTERN = /\b(you will|learners? will|students? will|be able to|can|will be able|upon completion|by the end)\b/i;
// ═══════════════════════════════════════════════════════════════
...
export class DeterministicRubricEngine {
    ...
    analyze(input) {
        ...
        const questionsCount = input.assessments.reduce((sum, a) => sum + (a.questions?.length ?? 0), 0);
        return {
        ...
        return [
            ...
            // ═══════════════════════════════════════════════════════════════
            {
                ...
                condition: (data) => {
                    if (data.objectives.length === 0)
                        return false;
                    const measurableCount = data.objectives.filter((obj) => MEASURABLE_VERBS_PATTERN.test(obj)).length;
            ...
            },
            {
                ...
                condition: (data) => {
                    if (data.objectives.length === 0)
                        return false;
                    const learnerCentered = data.objectives.filter((obj) => LEARNER_CENTERED_PATTERN.test(obj)).length;
            ...
            },
            {
                ...
                condition: (data) => {
                    const formative = data.assessments.filter((a) => a.type === 'quiz' ||
                        a.type === 'practice' ||
            ...
            },
            {
                ...
                condition: (data) => {
                    const assessmentsWithQuestions = data.assessments.filter((a) => a.questions && a.questions.length > 0);
                    if (assessmentsWithQuestions.length === 0)
                        return false;
                    const withFeedback = assessmentsWithQuestions.filter((a) => a.questions?.some((q) => q.explanation || q.feedback));
            ...
            },
            {
                ...
                condition: (data) => {
                    const totalQuestions = data.assessments.reduce((sum, a) => sum + (a.questions?.length ?? 0), 0);
            ...
            },
            {
                ...
                condition: (data) => {
                    if (data.chapters.length === 0)
                        return false;
                    const withOutcomes = data.chapters.filter((ch) => ch.learningOutcome && ch.learningOutcome.length > 20);
            ...
            },
            {
                ...
                condition: (data) => {
                    ...
                    const variance = sectionCounts.reduce((sum, c) => sum + Math.pow(c - avg, 2), 0) /
            ...
            // ═══════════════════════════════════════════════════════════════
            {
                ...
                condition: (data) => {
                    if (!data.contentAnalysis)
                        return false;
                    const higherOrder = (data.contentAnalysis.bloomsDistribution.ANALYZE ?? 0) +
            ...
            },
            {
                ...
                condition: (data) => {
                    if (!data.contentAnalysis?.dokDistribution)
                        return false;
            ...
            },
            {
                ...
                condition: (data) => {
                    const hasVideo = data.chapters.some((ch) => ch.sections?.some((s) => s.videoUrl));
            ...
            },
            {
                ...
                condition: (data) => {
                    ...
                    if (sections.length === 0)
                        return true;
                    const withDesc = sections.filter((s) => s.description && s.description.length > 10);
    ...
    estimateEffort(rule) {
        ...
        if (highEffortCategories.includes(rule.category))
            return 'high';
        if (lowEffortCategories.includes(rule.category))
            return 'low';
        return 'medium';
    }
    generateActionSteps(rule) {
        ...
        switch (rule.id) {
            ...
            case 'AS-002':
                ...
                steps.push('Add different formats: quizzes, projects, discussions, essays');
 ...
 */
export function calculateCourseTypeAlignment(actual, courseType) {
