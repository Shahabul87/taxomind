...
export class DeepContentAnalyzer {
    ...
    async analyzeContent(sources) {
        ...
        const verbFrequencyMap = new Map();
        const contextCounts = {
            instructional: 0,
            assessment: 0,
            activity: 0,
        ...
        for (const [ctx, count] of Object.entries(contextCounts)) {
            contextDistribution[ctx] = totalContexts > 0
                ? Math.round((count / totalContexts) * 100)
                : 0;
        }
        // Identify content gaps
        const contentGaps = this.identifyContentGaps(bloomsDistribution, dokDistribution, contextDistribution);
        // Generate recommendations
        const recommendations = this.generateRecommendations(bloomsDistribution, dokDistribution, contentGaps, overallConfidence);
        return {
            bloomsDistribution,
            dokDistribution,
    ...
    splitIntoSentences(text) {
        ...
        return rawSentences.filter(s => s.length >= this.MIN_SENTENCE_LENGTH &&
    ...
    generateRecommendations(blooms, dok, gaps, confidence) {
        ...
        if (blooms.REMEMBER + blooms.UNDERSTAND > 50) {
            recommendations.push('Reduce recall-focused content; transform definitions into application exercises');
        }
        if (blooms.CREATE < 5) {
        ...
        if (confidence < 50) {
            recommendations.push('[Note] Analysis confidence is low. Consider adding more structured content with clear learning objectives');
        }
