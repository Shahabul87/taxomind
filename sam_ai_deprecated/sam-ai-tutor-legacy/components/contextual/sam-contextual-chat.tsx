...
import { SAMContextualIntelligence, type ContextualResponse } from '@/sam/utils/sam-contextual-intelligence';
...
export function SAMContextualChat({ 
  ...
  useEffect(() => {
    if (currentContext) {
      SAMContextualIntelligence.generateContextualResponse(currentContext).then((response) => {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    setTimeout(async () => {
      try {
        ...
        const localContextualResponse = currentContext
          ? await SAMContextualIntelligence.generateContextualResponse(currentContext)
          : null;

        // Create contextually aware prompt
        const contextualPrompt = buildContextualPrompt(content, currentContext, localContextualResponse);
